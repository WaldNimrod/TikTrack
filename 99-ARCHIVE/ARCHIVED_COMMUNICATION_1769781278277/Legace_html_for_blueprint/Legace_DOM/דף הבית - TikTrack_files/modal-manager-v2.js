...
class ModalManagerV2 {
    ...
    init() {
        try {
            // Debug: Log initialization and check for lingering modals
            console.log('[ModalManagerV2] Initializing - checking for lingering modals on page load');

            // Check for any lingering modals/backdrops on page load
            const openModals = document.querySelectorAll('.modal.show');
            const allBackdrops = document.querySelectorAll('.modal-backdrop');

            if (openModals.length > 0 || allBackdrops.length > 0) {
                console.warn('[ModalManagerV2] Found lingering modals/backdrops on page load:', {
                ...
                openModals.forEach(modal => {
                    console.log('[ModalManagerV2] Force hiding lingering modal:', modal.id);
                ...
                allBackdrops.forEach(backdrop => {
                    console.log('[ModalManagerV2] Force removing lingering backdrop');
                    ...
                    <div class="modal-dialog modal-xl modal-dialog-centered attachment-preview-dialog">
                        <div class="modal-content">
                            ...
                            <div class="modal-body text-center">
                                ...
                                <iframe src="" class="attachment-preview-pdf-frame w-100 border rounded d-none"  title="תצוגת קובץ"></iframe>
    ...
    loadDefaultConfigurations() {
        // רק cash_flows-config נטען כרגע
                ...
                <div class="modal-dialog modal-${config.size || 'lg'}">
                    <div class="modal-content">
                        <div class="modal-header modal-header-colored" 
                            ...
                            <h5 class="modal-title" id="${config.id}Label" >${config.title?.add || defaultTitle}</h5>
        ...
        switch (field.type) {
            ...
            case 'tag-multi-select':
                ...
                const feeCurrencyLabelHTML = field.feeCurrencyLabel ? 
                    `<small class="text-muted ms-2" id="${field.id}CurrencyLabel" >-</small>` : '';
    ...
    async showModal(modalId, mode = 'add', entityData = null, options = {}) {
        ...
        try {
            ...
            // בדיקה שהאלמנט קיים ב-DOM - עם retry mechanism בגלל בעיות תזמון
            let domCheckAttempts = 0;
            const maxDomCheckAttempts = 5;
            let modalInDom = false;

            while (!modalInDom && domCheckAttempts < maxDomCheckAttempts) {
                modalInDom = document.body.contains(modalElement);
                if (!modalInDom) {
                    domCheckAttempts++;
                    if (domCheckAttempts < maxDomCheckAttempts) {
    ...
    getPluralEndpoint(entityType) {
        const pluralMap = {
            'cash_flow': 'cash_flows',  // Fixed: matches url_prefix='/api/cash_flows' in backend
            'trade': 'trades',
    ...
    initializeCurrencyExchangeCalculations(modalElement) {
        ...
        const updateDescription = () => {
            ...
            if (feeSymbol === fromSymbol && feeAmount > 0) {
                summaryHTML += `<div ><strong>נטו במטבע החשבון:</strong> ${netInAccountCurrency.toFixed(2)}${fromSymbol}</div>`;
            } else if (feeAmount > 0) {
                summaryHTML += `<div ><strong>נטו במטבע החשבון:</strong> ${fromAmount.toFixed(2)}${fromSymbol} (עמלה במטבע אחר)</div>`;
            }
            
            // נטו יתקבל במטבע המבוקש
            if (toSymbol) {
                if (canCalculateNet && feeAmount > 0) {
                    summaryHTML += `<div ><strong>נטו במטבע המבוקש:</strong> ${netInTargetCurrency.toFixed(2)}${toSymbol}</div>`;
                } else {
                    summaryHTML += `<div ><strong>נטו במטבע המבוקש:</strong> ${calculatedToAmount.toFixed(2)}${toSymbol}</div>`;
                }
            }
    ...
    applyUserColors(modalElement, entityType) {
        ...
        if (headerElement) {
            ...
            const validEntityTypes = ['trade', 'ticker', 'account', 'trading_account', 'alert', 'cash_flow', 'cash-flow', 'note', 'trade_plan', 'trade_plan', 'execution', 'preference', 'research', 'design', 'constraint', 'development'];
        ...
        if (bootstrapSelector) {
            ...
            for (const iconData of bootstrapIcons) {
                ...
                if (iconClass === '') {
                    ...
                    const colorStyle = iconColor ? `style="font-size: 24px; color: ${iconColor};"` : '';
    ...
    async selectWatchListIcon(iconName, library, modalElement) {
        ...
        if (iconName === '') {
            ...
            preview.innerHTML = `<i class="${iconName}" ></i>`;
        } else {
            try {
                if (window.IconSystem) {
                    const iconHTML = await window.IconSystem.renderIcon('button', iconName, {
    ...
    async populateAlertRelatedObjects(form, relatedTypeId, selectedRelatedId = null) {
        ...
        try {
            ...
            switch (parseInt(relatedTypeId)) {
                case 1: // account
                    endpoint = '/api/trading_accounts/'; // Fixed: use snake_case instead of kebab-case
                    valueField = 'id';
                    textField = 'name';
                    break;
                case 2: // trade
...
if (typeof window.showModalSafe === 'undefined') {
    window.showModalSafe = async function(modalId, mode = 'add') {
        try {
            ...
            // #region agent log - showModalSafe called
            fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({
                location: 'modal-manager-v2.js:showModalSafe:called',
                message: 'showModalSafe function called',
                data: {
                  timestamp: Date.now(),
                  page: window.location.pathname,
                  modalId: modalId,
                  mode: mode,
                  modalManagerV2Available: !!window.ModalManagerV2,
...
// אתחול אוטומטי - מידי וגם ב-DOMContentLoaded
