...
class ModalNavigationUI {
  ...
  _renderBreadcrumb(stack, activeModalId) {
    const items = stack.map(entry => {
      ...
      if (isActive) {
        return `<span class="breadcrumb-item breadcrumb-current" data-modal-id="${entry.modalId}" >${label}</span>`;
      }
      return `<a href="#" class="breadcrumb-item breadcrumb-link" data-modal-id="${entry.modalId}" >${label}</a>`;
    });

    return `<span class="modal-history-count-label" >[מערך: ${stack.length}]</span><div class="modal-breadcrumb-trail">${items.join(' / ')}</div>`;
  }
