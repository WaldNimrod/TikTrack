...
;(function () {
  ...
  function renderTrades(trades, currencySymbol) {
    ...
    // #region agent log - clearing trades loading
    fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'trading-ui/scripts/widgets/recent-items-widget.js:renderTrades:clearLoading',message:'Clearing trades loading indicator',data:{hasLoadingElement:!!elements.tradesLoading,loadingElementId:elements.tradesLoading?.id,loadingElementVisible:!elements.tradesLoading?.classList.contains('d-none'),tradesCount:trades?.length||0,timestamp:Date.now()},sessionId:'debug-session',runId:'widget_render_fixes',hypothesisId:'recent_items_loader'})}).catch(()=>{});
    // #endregion

    if (elements.tradesLoading) {
      elements.tradesLoading.classList.add('d-none');
      elements.tradesLoading.style.display = 'none';
      elements.tradesLoading.hidden = true;
      // Also hide all SPAN elements inside the loading container
      const spans = elements.tradesLoading.querySelectorAll('span');
      spans.forEach(span => {
      ...
      // #region agent log - trades loading cleared
      fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'trading-ui/scripts/widgets/recent-items-widget.js:renderTrades:loadingCleared',message:'Trades loading indicator cleared',data:{afterVisible:!elements.tradesLoading.classList.contains('d-none'),spansHidden:spans.length,timestamp:Date.now()},sessionId:'debug-session',runId:'widget_render_fixes',hypothesisId:'recent_items_loader'})}).catch(()=>{});
      // #endregion
    } else {
      // #region agent log - trades loading element not found
      fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'trading-ui/scripts/widgets/recent-items-widget.js:renderTrades:loadingNotFound',message:'Trades loading element not found',data:{containerExists:!!elements.container,allElementIds:Array.from(elements.container?.querySelectorAll('[id*="Loading"]')||[]).map(el=>el.id),timestamp:Date.now()},sessionId:'debug-session',runId:'widget_render_fixes',hypothesisId:'recent_items_loader'})}).catch(()=>{});
      // #endregion
    }
  ...
  function renderTradePlans(tradePlans, currencySymbol) {
    ...
    // Hide loading/error states
    // #region agent log - clearing plans loading
    fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'trading-ui/scripts/widgets/recent-items-widget.js:renderTradePlans:clearLoading',message:'Clearing plans loading indicator',data:{hasLoadingElement:!!elements.plansLoading,loadingElementId:elements.plansLoading?.id,loadingElementVisible:!elements.plansLoading?.classList.contains('d-none'),tradePlansCount:tradePlans?.length||0,timestamp:Date.now()},sessionId:'debug-session',runId:'widget_render_fixes',hypothesisId:'recent_items_loader'})}).catch(()=>{});
    // #endregion
    ...
    if (elements.plansLoading) {
      ...
      // Also hide all SPAN elements inside the loading container
      const spans = elements.plansLoading.querySelectorAll('span');
      spans.forEach(span => {
        span.classList.add('d-none');
        span.style.display = 'none';
        span.hidden = true;
      ...
      // #region agent log - plans loading cleared
      fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'trading-ui/scripts/widgets/recent-items-widget.js:renderTradePlans:loadingCleared',message:'Plans loading indicator cleared',data:{afterVisible:!elements.plansLoading.classList.contains('d-none'),spansHidden:spans.length,timestamp:Date.now()},sessionId:'debug-session',runId:'widget_render_fixes',hypothesisId:'recent_items_loader'})}).catch(()=>{});
      // #endregion
    } else {
      // #region agent log - plans loading element not found
      fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'trading-ui/scripts/widgets/recent-items-widget.js:renderTradePlans:loadingNotFound',message:'Plans loading element not found',data:{containerExists:!!elements.container,allElementIds:Array.from(elements.container?.querySelectorAll('[id*="Loading"]')||[]).map(el=>el.id),timestamp:Date.now()},sessionId:'debug-session',runId:'widget_render_fixes',hypothesisId:'recent_items_loader'})}).catch(()=>{});
      // #endregion
    }
  ...
  const RecentItemsWidget = {
    ...
    render(data = {}) {
      ...
      // #region agent log - render function called
      fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'trading-ui/scripts/widgets/recent-items-widget.js:render:entry',message:'RecentItemsWidget.render() called',data:{tradesCount:tradesToRender.length,tradePlansCount:tradePlansToRender.length,hasTradesLoading:!!elements.tradesLoading,hasPlansLoading:!!elements.plansLoading,timestamp:Date.now()},sessionId:'debug-session',runId:'widget_render_fixes',hypothesisId:'recent_items_loader'})}).catch(()=>{});
      // #endregion
      ...
      // Completed rendering
      // Force clear loaders one more time after rendering (in case they were re-shown)
      setTimeout(() => {
        if (elements.tradesLoading) {
          elements.tradesLoading.classList.add('d-none');
          elements.tradesLoading.style.display = 'none';
