...
const CACHE_DEPENDENCIES = {
    ...
    'trading_accounts-data': ['user-preferences'],
    'trading-account-{id}': ['trading_accounts-data'],
    ...
    // Trade Plans Level
    'trade_plans-data': ['accounts-data'],
    'trade_plan-{id}': ['trade_plans-data'],
    
    // Cash Flows Level
    'cash_flows-data': ['accounts-data'],
    'cash-flow-{id}': ['cash_flows-data'],
...
const TTL_POLICIES = {
    ...
    'dashboard_data': 'medium',      // 30 minutes
...
class UnifiedCacheManager {
    constructor() {
        ...
        this.defaultPolicies = {
            ...
            'trade_history-data': { layer: 'backend', ttl: 300000, compress: false, dependencies: ['trades', 'executions', 'trade_plans'] },
            'trade_history-statistics-*': { layer: 'backend', ttl: 300000, compress: false },
            'trade_history-aggregated-*': { layer: 'backend', ttl: 300000, compress: false },
            'trade_history-plan-vs-execution-*': { layer: 'backend', ttl: 300000, compress: false },
            ...
            'trading_journal-*': { layer: 'backend', ttl: 180000, compress: false, dependencies: ['notes', 'trades', 'executions'] },
            'trading_accounts-data': { layer: 'memory', ttl: 60000, compress: false, dependencies: ['user-preferences'] },
            'trading-account-{id}': { layer: 'memory', ttl: 60000, compress: false, dependencies: ['trading_accounts-data'] },
            ...
            'dashboard_data': { layer: 'backend', ttl: 300000, compress: false },
            'trade-positions': { layer: 'memory', ttl: 300000, compress: false, maxSize: 500 * 1024, validate: true, syncToBackend: false },
            'account_activity-data': { layer: 'backend', ttl: 60000, compress: false, dependencies: ['accounts-data', 'cash_flows-data', 'executions-data'] },
            'account_activity-*': { layer: 'backend', ttl: 60000, compress: false },
            'account-balance-*': { layer: 'backend', ttl: 60000, compress: false, dependencies: ['accounts-data', 'cash_flows-data', 'executions-data'] },
            ...
            'trade_history-timeline-*': { 
                layer: 'indexedDB', 
                ttl: 172800000, // 2 days (prod) / 86400000 (dev - 1 day)
                compress: true, 
                dependencies: ['trades', 'executions', 'trade_plans', 'notes', 'alerts', 'cash_flows'] 
            },
            'trade_history-chart-data-*': { 
                layer: 'indexedDB', 
                ttl: 172800000, // 2 days (prod) / 86400000 (dev - 1 day)
                compress: true, 
                dependencies: ['trades', 'executions', 'market_data_quotes'] 
            },
            'trade_history-position-data-*': { 
                layer: 'indexedDB', 
                ttl: 172800000, // 2 days
                compress: true, 
                dependencies: ['trades', 'executions'] 
            },
            'trade_history-pl-data-*': { 
                layer: 'indexedDB', 
                ttl: 172800000, // 2 days
                compress: true, 
                dependencies: ['trades', 'executions', 'market_data_quotes'] 
            ...
            'trade_history-full-analysis-*': { 
                layer: 'indexedDB', 
                ttl: 172800000, // 2 days (prod) / 86400000 (dev - 1 day)
                compress: true, 
                dependencies: ['trades', 'executions', 'trade_plans', 'notes', 'alerts', 'cash_flows', 'market_data_quotes'] 
            }
        };
     ...
     */
    async initialize() {
        // #region agent log - H1/H3: UCM initialization start
        fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'trading-ui/scripts/unified-cache-manager.js:initialize:entry',message:'UCM initialize called',data:{ucmInitialized:this.initialized,modalManagerExists:!!window.ModalManagerV2,loggerExists:!!window.Logger,indexedDBAvailable:!!window.indexedDB,sessionTokenBefore:sessionStorage.getItem('authToken'),timestamp:Date.now()},sessionId:'debug-session',runId:'homepage_auth_persistence',hypothesisId:'H1_H3'})}).catch(()=>{});
        // #endregion
        try {
            ...
            // #region agent log - H1/H3/H4: UCM initialization complete - check for bootstrap auth
            fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'trading-ui/scripts/unified-cache-manager.js:initialize:complete',message:'UCM initialization complete',data:{sessionTokenExists:!!sessionStorage.getItem('authToken'),sessionUserExists:!!sessionStorage.getItem('currentUser'),windowAuthTokenExists:!!window.authToken,windowCurrentUserExists:!!window.currentUser,timestamp:Date.now()},sessionId:'debug-session',runId:'auth_cache_consistency',hypothesisId:'H1_H3_H4'})}).catch(()=>{});
            // #endregion
            
            // Trigger bootstrap auth sync if it exists (bootstrap ran before UCM initialized)
            if (typeof window._bootstrapAuthSync === 'function') {
              // #region agent log - H1/H4: UCM triggering bootstrap sync
              fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'trading-ui/scripts/unified-cache-manager.js:initialize:triggerBootstrapSync',message:'UCM triggering bootstrap auth sync',data:{timestamp:Date.now()},sessionId:'debug-session',runId:'auth_cache_consistency',hypothesisId:'H1_H4'})}).catch(()=>{});
              // #endregion
              window._bootstrapAuthSync().catch(() => {});
            }
            
            // Also sync window globals to UCM if they exist but aren't in sessionStorage
            if (window.authToken && window.currentUser) {
              const sessionToken = sessionStorage.getItem('authToken');
              const sessionUser = sessionStorage.getItem('currentUser');
              if (!sessionToken || !sessionUser) {
                // #region agent log - H5: UCM syncing window globals to sessionStorage
                fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'trading-ui/scripts/unified-cache-manager.js:initialize:syncWindowGlobals',message:'UCM syncing window globals to sessionStorage',data:{hasWindowToken:!!window.authToken,hasWindowUser:!!window.currentUser,hasSessionToken:!!sessionToken,hasSessionUser:!!sessionUser,timestamp:Date.now()},sessionId:'debug-session',runId:'auth_cache_consistency',hypothesisId:'H5'})}).catch(()=>{});
                // #endregion
                this.save('authToken', window.authToken, { layer: 'sessionStorage', includeUserId: false }).catch(() => {});
                this.save('currentUser', window.currentUser, { layer: 'sessionStorage', includeUserId: false }).catch(() => {});
              }
            }
            // #region agent log - H1/H3: UCM initialization complete
            fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'trading-ui/scripts/unified-cache-manager.js:initialize:exit',message:'UCM initialized successfully',data:{ucmInitialized:this.initialized,hasAuthToken:!!sessionStorage.getItem('authToken'),authTokenValue:sessionStorage.getItem('authToken')?.substring(0,10),dashboardDataExists:!!window.DashboardData,timestamp:Date.now()},sessionId:'debug-session',runId:'homepage_auth_persistence',hypothesisId:'H1_H3'})}).catch(()=>{});
            // #endregion
    ...
    async clear(type = 'all', options = {}) {
        // #region agent log - H1: clear() entry
        fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'trading-ui/scripts/unified-cache-manager.js:clear:entry',message:'H1: clear() called',data:{type,initialized:this.initialized,caller:new Error().stack.split('\n')[2]?.trim(),timestamp:Date.now()},sessionId:'debug-session',runId:'run6_dashboard_fix',hypothesisId:'H1'})}).catch(()=>{});
        // #endregion
...
UnifiedCacheManager.prototype.clearAllCache = async function(options = {}) {
    try {
        // שלב 0: בדיקת initialization
        if (!this.initialized) {
            window.Logger.warn('⚠️ UnifiedCacheManager not initialized yet - deferring clear request', { 
                page: "unified-cache-manager",
                options
            });
            return { 
                success: false, 
                error: 'UnifiedCacheManager not initialized', 
                clearedLayers: [],
                errors: ['UnifiedCacheManager not initialized']
            };
        }
        ...
        const cacheKeys = [
            ...
            'trade-data', 'dashboard_data', 'tikTrack_preferences'
        ];
