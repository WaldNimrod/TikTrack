...
;(function () {
  ...
  async function getDataForCombination(combination) {
    ...
    try {
      if (combination === 'createTrades') {
        ...
        const dismissed = await window.PendingActionsCacheService.getDismissed('trade_creation_clusters');
  ...
  async function loadCombinationData(action, entity) {
    ...
    try {
      ...
      // ✅ הבטח שה-loading מתנקה תמיד
      setLoading(combination, false);
      
      try {
        await renderCombination(combination);
      } catch (renderError) {
        window.Logger?.error?.('Failed to render combination', { error: renderError, combination, page: 'unified-pending-actions-widget' });
        // ✅ גם אם render נכשל, ה-loading כבר התנקה ב-setLoading למעלה
      }
      
      try {
        await checkTabsVisibility();
      } catch (visibilityError) {
        window.Logger?.warn?.('Failed to check tabs visibility', { error: visibilityError, combination, page: 'unified-pending-actions-widget' });
      }
        ...
        if (window.ExecutionClusterHelpers?.renderClusterListItem) {
          ...
          return window.ExecutionClusterHelpers.renderClusterListItem(item, selectedIds, {
            ...
            onDismiss: async (clusterIdToDismiss) => {
              await window.PendingActionsCacheService.dismissItem('trade_creation_clusters', clusterIdToDismiss);
  ...
  async function renderCombination(combination) {
    // #region agent log - renderCombination called
    fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'trading-ui/scripts/widgets/unified-pending-actions-widget.js:renderCombination:entry',message:'renderCombination called',data:{combination:combination,paneExists:!!elements.panes?.[combination],listExists:!!elements.list[combination],timestamp:Date.now()},sessionId:'debug-session',runId:'widget_render_fixes',hypothesisId:'unified_pending_render'})}).catch(()=>{});
    // #endregion

    const pane = elements.panes?.[combination];
    if (!pane) {
      // #region agent log - pane not found
      fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'trading-ui/scripts/widgets/unified-pending-actions-widget.js:renderCombination:noPane',message:'Pane not found for combination',data:{combination:combination,availablePanes:Object.keys(elements.panes||{}),timestamp:Date.now()},sessionId:'debug-session',runId:'widget_render_fixes',hypothesisId:'unified_pending_render'})}).catch(()=>{});
      // #endregion
      // ✅ הבטח שה-loading מתנקה גם אם pane לא נמצא
      setLoading(combination, false);
      return;
    }
    ...
    if (!list) {
      // #region agent log - list not found
      fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'trading-ui/scripts/widgets/unified-pending-actions-widget.js:renderCombination:noList',message:'List not found for combination',data:{combination:combination,availableLists:Object.keys(elements.list||{}),timestamp:Date.now()},sessionId:'debug-session',runId:'widget_render_fixes',hypothesisId:'unified_pending_render'})}).catch(()=>{});
      // #endregion
      // ✅ הבטח שה-loading מתנקה גם אם list לא נמצא
      setLoading(combination, false);
      return;
    }

    // ✅ הבטח שה-loading מתנקה גם אם יש שגיאה
    try {
      const data = await getDataForCombination(combination);

      // #region agent log - data loaded
      fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'trading-ui/scripts/widgets/unified-pending-actions-widget.js:renderCombination:dataLoaded',message:'Data loaded for combination',data:{combination:combination,dataLength:data?.length||0,hasData:!!data&&data.length>0,timestamp:Date.now()},sessionId:'debug-session',runId:'widget_render_fixes',hypothesisId:'unified_pending_render'})}).catch(()=>{});
      // #endregion
      ...
      // ✅ הבטח שה-loading מתנקה גם אם יש שגיאה
      window.Logger?.error?.('Error rendering combination', { error, combination, page: 'unified-pending-actions-widget' });
      setLoading(combination, false);
      showError(combination, error.message || 'שגיאה בטעינת נתונים');
    }
      ...
      if (list) {
        ...
        list.addEventListener('click', async (event) => {
          ...
          if (dismissBtn) {
            ...
            await window.PendingActionsCacheService.dismissItem('trade_creation_clusters', clusterId);
            await loadCombinationData(state.activeAction, state.activeEntity);
            return;
          }
  ...
  function setLoading(combination, isLoading) {
    ...
    if (loadingEl) {
      ...
      const spans = loadingEl.querySelectorAll('span');
      spans.forEach(span => {
        span.classList.toggle('d-none', !isLoading);
        span.style.display = isLoading ? '' : 'none';
      });
    }
  }
