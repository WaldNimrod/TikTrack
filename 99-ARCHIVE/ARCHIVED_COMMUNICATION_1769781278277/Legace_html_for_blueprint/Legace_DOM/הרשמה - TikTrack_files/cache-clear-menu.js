...
(() => {
    ...
    async function clearMemoryLayer(source) {
        // #region agent log - H2: clearMemoryLayer before clear
        fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'trading-ui/scripts/cache-clear-menu.js:clearMemoryLayer:beforeClear',message:'H2: clearMemoryLayer before clear',data:{source,managerExists:!!getCacheManager(),managerInitialized:getCacheManager()?.initialized,hasClearMethod:!!getCacheManager()?.clear,timestamp:Date.now()},sessionId:'debug-session',runId:'run6_dashboard_fix',hypothesisId:'H2'})}).catch(()=>{});
        // #endregion
