...
async function runFilePrecheck(file, connectorValueOverride = null) {
    ...
    try {
        ...
        const response = await fetch('/api/user_data_import/precheck', {
            method: 'POST',
            body: formData
        });
...
async function confirmAutoLinkedAccount() {
    ...
    try {
        ...
        const response = await fetch(`/api/user_data_import/session/${pendingAccountLinking.sessionId}/account_link/confirm`, {
            method: 'POST',
            headers: {
...
async function submitAccountLinkSelection(forceOverride = false) {
    ...
    try {
        ...
        const response = await fetch(`/api/user_data_import/session/${pendingAccountLinking.sessionId}/account_link/select`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
...
async function openAccountLinkingModalManually() {
    ...
    try {
        const response = await fetch(`/api/user_data_import/session/${currentSessionId}/account_link/status`);
        const data = await response.json();
        if (!response.ok || !data.success) {
...
async function linkExternalAccountToTradingAccount() {
    ...
    setAccountLinkingLoading(true);
    try {
        const response = await fetch(`/api/user_data_import/session/${pendingAccountLinking.sessionId}/link_account`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
...
async function fetchExistingSessionDetails(sessionId) {
    ...
    try {
        const response = await fetch(`/api/user_data_import/session/${sessionId}`);
        if (!response.ok) {
        ...
        if (!linkedAccountInfo && (linkingMatchedAccountId || tradingAccountId)) {
            ...
            try {
                const accountResponse = await fetch(`/api/trading_accounts/${accountId}`);
...
async function openImportUserDataModal() {
    ...
    try {
        ...
        try {
            ...
            try {
                const response = await fetch('/api/user_data_import/sessions/active', {
                    method: 'GET',
        ...
        if (hasActiveSession && activeSessionId) {
            try {
                ...
                try {
                    const resetResponse = await fetch(`/api/user_data_import/session/${activeSessionId}/reset`, {
                        method: 'POST',
...
async function loadStep1Content() {
    ...
    if (!dropZone || !fileInput) {
        window.Logger.error('[Import Modal] File upload elements not found', { 
            dropZone: !!dropZone, 
            fileInput: !!fileInput,
            page: 'import-user-data' 
        });
...
async function reanalyseSessionForTask(taskKey, loadingMessage = 'טוען ומעבד נתונים...') {
    ...
    try {
        ...
        const analysisResponse = await fetch(`/api/user_data_import/session/${currentSessionId}/analyze?task_type=${encodedTask}`);
        ...
        const previewResponse = await fetch(`/api/user_data_import/session/${currentSessionId}/preview?task_type=${encodedTask}`);
...
async function importExistingRecord(index) {
    try {
        ...
        const response = await fetch(`/api/user_data_import/session/${sessionId}/allow_existing`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
...
async function skipExistingRecord(index) {
    try {
        ...
        const response = await fetch(`/api/user_data_import/session/${sessionId}/reject_duplicate`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
...
function analyzeFile() {
    ...
    console.log('📡 [ANALYZE_FILE] Sending POST request to /api/user_data_import/upload');
    
    fetch('/api/user_data_import/upload', {
            method: 'POST',
        body: formData
    })
    .then(response => {
    ...
    .then(data => {
        ...
        if (data.success || data.status === 'success') {
            ...
            setAnalysisLoadingState(false);
            showImportUserDataNotification(`שגיאה בניתוח הקובץ: ${data.error || data.message || 'תגובה לא תקינה מהשרת'}`, 'error');
            console.groupEnd();
        }
    })
    .catch(error => {
        ...
        showImportUserDataNotification(`שגיאה בניתוח הקובץ: ${error?.message || 'שגיאה לא ידועה'}`, 'error');
        console.groupEnd();
    });
}
...
async function loadProblemResolution(autoTriggered = false) {
    ...
    try {
        const response = await fetch(`/api/user_data_import/session/${currentSessionId}/preview?task_type=${encodeURIComponent(taskKey)}`, {
        method: 'GET'
        });
        const data = await response.json();
...
function loadPreviewData() {
    ...
    fetch(`/api/user_data_import/session/${currentSessionId}/preview?task_type=${encodedTask}`, {
        method: 'GET'
    })
    .then(response => response.json())
    .then(data => {
...
function acceptDuplicate(index, type) {
    ...
    fetch(`/api/user_data_import/session/${currentSessionId}/accept_duplicate`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
        },
...
function rejectDuplicate(index, type) {
    ...
    fetch(`/api/user_data_import/session/${currentSessionId}/reject_duplicate`, {
            method: 'POST',
        headers: {
            'Content-Type': 'application/json',
        },
...
function generatePreview() {
    ...
    fetch(`/api/user_data_import/session/${currentSessionId}/preview`, {
        method: 'GET'
    })
    .then(response => response.json())
    .then(data => {
...
function performImport(generateReport = false) {
    ...
    fetch(`/api/user_data_import/session/${currentSessionId}/execute`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
        },
...
async function handleSessionReset(sessionId, skipGoToStep = false) {
    ...
    try {
        ...
        let response;
        try {
            response = await fetch(`/api/user_data_import/session/${sessionId}/reset`, {
                method: 'POST',
                signal: controller.signal
            });
            clearTimeout(timeoutId);
                        ...
                        detailsBtn.onclick = () => {
                            if (typeof window.showDetailsModal === 'function') {
                                window.showDetailsModal(
                                    ...
                                    `<div >${detailedErrors.map((item, idx) => `(${idx + 1}) ${item}`).join('\n')}</div>`,
...
function refreshPreviewData() {
    ...
    fetch(`/api/user_data_import/session/${currentSessionId}/refresh_preview`, {
        method: 'POST'
    })
    .then(response => {
