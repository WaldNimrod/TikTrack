...
class ActionsMenuSystem {
    ...
    createActionsMenu(buttons) {
        ...
        const menuButtons = buttons.map((button, index) => {
            ...
            const buttonHTML = `<button class="btn actions-menu-item" data-variant="small" data-button-type="${buttonType}" data-onclick='${escapedOnclick}' ${tooltipAttrs}>${icon}</button>`;
            return buttonHTML;
        }).join('');
...
window.createActionsMenu = function(buttons) {
    ...
    const menuButtons = buttons.map(button => {
        ...
        return `<button class="btn actions-menu-item" data-variant="small" data-button-type="${buttonType}" data-onclick='${escapedOnclick}' ${tooltipAttrs}>${icon}</button>`;
    }).join('');
