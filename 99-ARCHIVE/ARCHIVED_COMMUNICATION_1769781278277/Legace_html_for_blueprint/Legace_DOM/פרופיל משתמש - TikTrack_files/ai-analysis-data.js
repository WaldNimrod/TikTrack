...
(function aiAnalysisDataService() {
  ...
  async function validateVariables(variables) {
    ...
    try {
      ...
      if (window.CacheTTLGuard?.ensure) {
        return await window.CacheTTLGuard.ensure(cacheKey, async () => {
          const response = await fetch(buildUrl('/api/business/ai_analysis/validate_variables'), {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
    ...
    try {
      ...
      if (window.CacheTTLGuard?.ensure) {
        return await window.CacheTTLGuard.ensure('ai-analysis-providers', async () => {
          const response = await fetch(buildUrl('/api/ai_analysis/llm_provider'), { });
...
<body>
  ...
  async function deleteAllAnalyses() {
    try {
      const apiUrl = buildUrl('/api/ai_analysis/delete_all');
