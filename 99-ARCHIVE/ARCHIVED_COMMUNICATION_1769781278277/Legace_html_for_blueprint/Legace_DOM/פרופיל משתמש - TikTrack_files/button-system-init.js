...
class AdvancedButtonSystem {
    ...
    static ENTITY_COLOR_MAP = {
        'trade_plan': { 
            color: 'entity-trade_plan-color', 
            hover: 'entity-trade_plan-color' 
        },
        'trade': { 
    ...
    applyEntityColors(button, entityType) {
        ...
        // ITCSS Compliance: Add CSS class instead of inline styles
        const className = `btn-entity-${entityType.replace('_', '-')}`;
        button.classList.add(className);

        // Remove any existing inline styles for these properties
        button.style.removeProperty('--current-entity-color');
        button.style.removeProperty('--current-entity-hover');

        this.logger.debug(`Applied entity colors for ${entityType} using CSS class: ${className}`);
    }
