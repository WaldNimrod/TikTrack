...
/**
 * ==========================================
 * RUNTIME FIXES FOR CONSOLE ERROR REDUCTION
 * ==========================================
 */

// Conditions System Fallback (Runtime Fix)
function ensureConditionsTranslations() {
    if (!window.ConditionsTranslations) {
        console.warn('🛠️ [Runtime Fix] ConditionsTranslations not available, loading fallback');
        window.ConditionsTranslations = {
            get: (key) => key, // Identity function fallback
            translate: (key) => key,
            has: (key) => true,
            // Add other common methods as needed
        };
        console.log('✅ [Runtime Fix] ConditionsTranslations fallback loaded');
    }
}

// Page Config Fallbacks (Runtime Fix)
function ensurePageConfigs() {
    const pageConfigs = {
        'trades': {
            name: 'trades',
            displayName: 'Trades',
    ...
    Object.keys(pageConfigs).forEach(pageKey => {
        if (!window.pageConfigs[pageKey]) {
            window.pageConfigs[pageKey] = pageConfigs[pageKey];
            console.log(`✅ [Runtime Fix] Added fallback config for ${pageKey}`);
        }
    });
}

// Apply runtime fixes on script load
(function applyRuntimeFixes() {
    console.log('🔧 [Runtime Fix] Applying console error reduction fixes...');

    ensureConditionsTranslations();
    ensurePageConfigs();

    console.log('✅ [Runtime Fix] Console error reduction fixes applied');
})();
  ...
  class UnifiedAppInitializer {
    ...
    async executeInitialization(config) {
      ...
      try {
        ...
        // Do this BEFORE validation to avoid false errors
        ...
        // Validate required dependencies AFTER waiting for scripts to load
        if (window.Logger?.debug) {
...
/**
 * תלויות:
 * - linked_items.js (לפונקציות הצגת מודלים)
            ...
            <div class="modal fade" id="clearCacheConfirmationModal" tabindex="-1" data-bs-backdrop="static">
                <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        <div class="modal-header text-white" data-entity-type="${config.entityType || ''}">
                        ...
                        <div class="modal-footer rtl-modal-footer">
          ...
          <div class="modal-body">
            ...
            <div class="row">
              <div class="col-md-6">
                ...
                <pre class="bg-light p-2 rounded">${JSON.stringify(successInfo.details, null, 2)}</pre>
              </div>
              <div class="col-md-6">
...
async function showDetailsModal(title, content, options = {}) {
  ...
  const modalHTML = `
    <div class="modal fade" id="${modalId}" tabindex="-1" aria-labelledby="${modalId}-label">
      <div class="modal-dialog modal-lg">
        <div class="modal-content">
          <div class="modal-header modal-header-info text-white d-flex justify-content-between align-items-center">
...
if (false && typeof window.pageInitializationConfigs === 'undefined') {
  ...
  // ✅ REMOVED 2026-01-22: ensureAuthenticatedForPage function completely deleted
  // 
  // Single Source of Truth: trading-ui/scripts/page-initialization-configs.js
  // All pages must load page-initialization-configs.js to get authentication functionality.
  // No backward compatibility needed - TikTrack is a modern system.
  //
  // The function window.ensureAuthenticatedForPage is exported ONLY from:
  //   → page-initialization-configs.js
  //
  // This ensures:
  //   1. Single source of truth for authentication logic
  //   2. No confusion about which implementation is used
  //   3. Clean, maintainable codebase
  //   4. No legacy code burden
  ...
  // ✅ CRITICAL FIX: Don't export ensureAuthenticatedForPage from core-systems.js
  // The correct implementation is in page-initialization-configs.js which checks
  // sessionStorage, localStorage, window globals, and calls checkAuthentication()
  // This old implementation only checks sessionStorage and doesn't call checkAuthentication()
  // 
  // REMOVED: Export completely - let page-initialization-configs.js handle it
  // The old implementation is kept for backward compatibility but should not be exported
  // window.ensureAuthenticatedForPage = ensureAuthenticatedForPage; // REMOVED
