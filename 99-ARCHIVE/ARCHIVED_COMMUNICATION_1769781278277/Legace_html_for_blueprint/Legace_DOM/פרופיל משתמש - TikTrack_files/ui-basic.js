    ...
    if (icon) {
      ...
      if (window.BUTTON_ICONS && window.BUTTON_ICONS.TOGGLE) {
        ...
        if (toggleIconPath.startsWith('/') || toggleIconPath.startsWith('http')) {
          ...
          const transformStyle = isCollapsed ? '' : '';
...
function generateActionButtons(entityId, entityType, status, detailsFunction, linkedFunction, editFunction, cancelFunction, restoreFunction, deleteFunction, showDetails = true, showLinked = true, showEdit = true, showCancel = true, showDelete = true) {
  ...
  if (showDelete) {
    buttonsHtml += `
      <button class="btn btn-sm btn-outline-danger ui-basic-small-btn" onclick="${deleteFunction}('${entityType}', ${entityId})" title="מחק">
        🗑️
      </button>`;
  }
...
// REMOVED: viewLinkedItems demo function - use window.viewLinkedItems from linked_items.js directly
...
// REMOVED: window.viewLinkedItems - use window.viewLinkedItems from linked_items.js directly
