...
def setup_logging() -> logging.Logger:
    """Setup logging system - FIXED: No duplicate handlers"""
    ...
    # Set main logger - CLEAR EXISTING HANDLERS FIRST
    ...
    # Console log - ONLY ONE CONSOLE HANDLER
    console_handler = logging.StreamHandler()
    console_handler.setLevel(logging.INFO)
    console_handler.setFormatter(log_format)
    logger.addHandler(console_handler)
    
    # File log - ONLY ONE MAIN FILE HANDLER
    ...
    # SQL queries log (only in development) - SEPARATE LOGGER
    if os.getenv('FLASK_ENV') == 'development':
        sql_logger = logging.getLogger('sqlalchemy.engine')
        sql_logger.setLevel(logging.INFO)
        sql_logger.propagate = False  # Don't propagate to main logger
