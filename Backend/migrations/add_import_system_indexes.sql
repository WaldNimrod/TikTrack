...
CREATE INDEX IF NOT EXISTS idx_import_session_account_id ON import_sessions(account_id);
...
CREATE INDEX IF NOT EXISTS idx_import_session_account_status ON import_sessions(account_id, status);
CREATE INDEX IF NOT EXISTS idx_import_session_account_created ON import_sessions(account_id, created_at DESC);

-- Indexes for Execution table (for duplicate detection optimization)
CREATE INDEX IF NOT EXISTS idx_execution_account_date ON executions(account_id, date);
