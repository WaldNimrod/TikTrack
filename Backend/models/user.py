...
class User(BaseModel):
    ...
    @classmethod
    def get_default_user(cls, db) -> Optional['User']:
        """
        ...
        Returns:
            Optional[User]: Default user if found, None otherwise
            
        Example:
            >>> default_user = User.get_default_user(db_session)
            >>> if default_user:
            ...     print(f"Default user: {default_user.username}")
        """
        return db.query(cls).filter(cls.is_default == True).first()
    
    @classmethod
    def create_default_user(cls, db) -> 'User':
        """
        ...
        Returns:
            User: Default user (created or existing)
            
        Example:
            >>> default_user = User.create_default_user(db_session)
            >>> print(f"Default user: {default_user.username}")
        """
        # Check if default user exists
        default_user = cls.get_default_user(db)
        if default_user:
            return default_user
        ...
        default_user = cls(
            ...
            is_active=True,
            is_default=True,
            preferences='{"primaryCurrency": "USD", "defaultStopLoss": 5, "defaultTargetPrice": 10, "defaultCommission": 1.0, "consoleCleanupInterval": 60000, "timezone": "Asia/Jerusalem", "defaultStatusFilter": "open", "defaultTypeFilter": "swing", "defaultAccountFilter": "all", "defaultDateRangeFilter": "this_week", "defaultSearchFilter": ""}'
        )
