...
class User(BaseModel):
    ...
    preference_profiles = relationship("PreferenceProfile", back_populates="user", foreign_keys="PreferenceProfile.user_id")
    user_preferences_v2 = relationship("UserPreferencesV2", back_populates="user")
    ...
    def get_preferences(self) -> Dict[str, Any]:
        """
        Get user preferences as dictionary (legacy method)
        
        Returns:
            Dict[str, Any]: User preferences from legacy JSON field
            
        Example:
            >>> user = User(preferences_json='{"theme": "dark"}')
    ...
    def set_preferences(self, preferences: Dict[str, Any]) -> None:
        """
        Set user preferences from dictionary (legacy method)
