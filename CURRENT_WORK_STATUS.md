...
### **1. מסד נתונים פגום - בעיה קריטית!**
- **סטטוס:** 🚨 דחוף
- **בעיה:** `(sqlite3.DatabaseError) database disk image is malformed`
- **תוצאה:** השרת רץ אבל לא יכול לכתוב למסד הנתונים
- **פעולה נדרשת:** תיקון או שחזור מסד הנתונים

### **2. השרת רץ אבל עם שגיאות**
- **סטטוס:** ⚠️ חלקי
- **בעיה:** השרת עולה אבל יש שגיאות במסד הנתונים
- **תוצאה:** API endpoints לא עובדים כמו שצריך

### **3. בדיקת API endpoints חדשים**
- **סטטוס:** ⏳ ממתין
- **תלוי ב:** תיקון מסד הנתונים
...
### **עדיפות גבוהה:**
1. **תיקון מסד נתונים** - פתרון בעיית "database disk image is malformed"
2. **שחזור נתונים** - שחזור מסד נתונים מגיבוי או יצירת חדש
3. **בדיקת השרת** - וידוא שהשרת עובד ללא שגיאות
