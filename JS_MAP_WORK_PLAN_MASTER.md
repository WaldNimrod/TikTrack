...
- **פונקציות לא קוראות ל-APIs** - הנתונים לא נטענים מהשרת
- **שגיאות HTTP 500** ב-preferences שלא קיימים
- **נתונים סטטיים** במקום דינמיים מהשרת

#### **2. פונקציות רינדור:**
- **`renderSystemStats()`** - קיימת אבל לא מציגה תוכן (רק "טוען...")
- **`renderPageMapping()`** - קיימת אבל לא מציגה תוכן
- **`renderFunctionsData()`** - קיימת אבל לא מציגה תוכן
- **`loadLocalFunctionsAnalysis()`** - קיימת אבל לא מציגה תוכן
