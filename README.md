...
- **קובץ שרת:** `Backend/dev_server.py`
...
- ✅ **Auto-reload** - השרת מתעדכן אוטומטית בשינויים
- ✅ **Monitoring** - בדיקת בריאות אוטומטית
- ✅ **Restart אוטומטי** - במקרה של קריסה
- ✅ **לוגים מפורטים** - לפיתוח ובדיקת בעיות
- ✅ **יציבות** - מבוסס על Waitress המתוקן

### 📁 **ארכיון קונפיגורציות ישנות:**
- **מיקום:** `Backend/backups/20250819_server_configurations/`
- **תאריך:** 19 באוגוסט 2025
- **סיבה:** מעבר לקונפיגורציה אחידה
...
#### 🔄 שינויי קונפיגורציה:
- **ארכיון קונפיגורציות ישנות:** העברת `run_stable.py`, `run_waitress_fixed.py`, `SERVER_CONFIGURATIONS.md` לארכיון
- **קונפיגורציה אחידה:** רק `dev_server.py` ו-`./start_dev.sh` בשימוש
- **פישוט דוקומנטציה:** עדכון הדוקומנטציה להציג רק את הקונפיגורציה החדשה
- **מניעת כפילויות:** עדכון `app.py` למנוע הפעלה ישירה ולהשתמש רק ב-`dev_server.py`
- **עדכון dev_server.py:** הסרת תלות ב-`run_waitress_fixed.py` ושימוש ב-`app.py` ישירות

#### 📁 קבצים שעודכנו:
- ✅ `README.md` - עדכון להציג רק את הקונפיגורציה החדשה
- ✅ `backups/20250819_server_configurations/` - ארכיון הקונפיגורציות הישנות
- ✅ `backups/20250819_server_configurations/README_BACKUP.md` - תיעוד הגיבוי
- ✅ `Backend/app.py` - מניעת הפעלה ישירה והפניה ל-`dev_server.py`
- ✅ `Backend/dev_server.py` - הסרת תלות ב-`run_waitress_fixed.py` ושימוש ב-`app.py` ישירות
...
- ✅ **לוגים מפורטים** - לפיתוח ובדיקת בעיות
- ✅ **יציבות** - מבוסס על Waitress המתוקן
- ✅ **מניעת כפילויות** - `app.py` לא מאפשר הפעלה ישירה
...
| README ראשי | מערכת הבדיקות | ארכיון קונפיגורציות |
|-------------|----------------|---------------------|
| תיעוד כללי | בדיקות קוד | קונפיגורציות ישנות |
| ארכיטקטורה | אמצעי אבטחה | היסטוריית שינויים |
| פיתוח | דוחות תוצאות | שחזור (אם נדרש) |
| UI/UX | איתור באגים | ארכיון |
