...
**Root Cause:** `transformers.js` זיהה את `commissionType` כשדה פיננסי (כי הוא מכיל `commission` מ-`FINANCIAL_FIELDS`) והמיר אותו למספר, בעוד שהוא ENUM ולא מספר.

**תיקון:** הוספתי `commissionType`, `commission_type`, ו-`type` ל-`STRING_ONLY_FIELDS` ב-`transformers.js`. כמו כן, הוספתי `minimum` ל-`FINANCIAL_FIELDS` כדי להבטיח שהוא מומר למספר.

**אימות:** ביצעתי tests שמראים שהתיקון עובד — `commission_type` נשאר `"TIERED"`, `commission_value` נשאר `0.0035` (number), ו-`minimum` מומר ל-`1` (number).
...
**תרחיש:**
1. הטופס שולח: `{ commissionType: "TIERED", commissionValue: 0.0035, minimum: 1 }`
...
4. ה-API מקבל: `{ commission_type: 0, commission_value: 0.0035, minimum: 1 }`
...
### 2.1. שינוי ב-`transformers.js` — הוספת `commissionType` ל-`STRING_ONLY_FIELDS`
...
### 2.2. שינוי ב-`transformers.js` — הוספת `minimum` ל-`FINANCIAL_FIELDS`

**קובץ:** `ui/src/cubes/shared/utils/transformers.js`

**שינוי:**
- **לפני:** `FINANCIAL_FIELDS = [..., 'margin']`
- **אחרי:** `FINANCIAL_FIELDS = [..., 'margin', 'minimum']`
...
const FINANCIAL_FIELDS = [
  'balance', 'price', 'amount', 'total', 'value', 'quantity', 'cost', 
  'fee', 'commission', 'profit', 'loss', 'equity', 'margin', 'minimum'
];
```
...
ביצעתי tests עם `reactToApi()`:

**Test 1: מספרים מהטופס**
```javascript
Input: {
  commissionType: "TIERED",
  commissionValue: 0.0035,
...
**Test 2: מחרוזות מהטופס (backward compatibility)**
```javascript
Input: {
  commissionType: "TIERED",
  commissionValue: "0.0035",
...
Output: {
  commission_type: "TIERED",    // ✅ string (ENUM)
  commission_value: 0.0035,     // ✅ number (converted from string)
  minimum: 1                    // ✅ number (converted from string)
}
```
...
| קובץ | שינוי | סטטוס |
|------|-------|--------|
| `ui/src/cubes/shared/utils/transformers.js` | הוספת `commissionType` ל-`STRING_ONLY_FIELDS` + הוספת `minimum` ל-`FINANCIAL_FIELDS` | ✅ |
...
2. **בדיקת Network Tab:**
   ...
   - **צפוי:** 
     - `commission_type: "TIERED"` (string)
     - `commission_value: 0.0035` (number)
     - `minimum: 1` (number)
...
3. **תיקון נוסף:** הוספתי `minimum` ל-`FINANCIAL_FIELDS` כדי להבטיח שהוא מומר למספר גם אם נשלח כמחרוזת.

4. **אימות:** ביצעתי tests manual שמראים שהתיקון עובד.

---

## 7. התנצלות

מתנצל על התיקונים הקודמים שלא פתרו את הבעיה. הבעיה הייתה ב-`transformers.js` ולא בטופס או ב-handler, ולכן התיקונים הקודמים לא עזרו. עכשיו התיקון נכון ואומת ב-tests.

---

**Team 30 (Frontend Execution)**  
**log_entry | TO_TEAM_50 | COMMISSION_VALUE_422_FIX_ROOT_CAUSE | SENT | 2026-02-10**
