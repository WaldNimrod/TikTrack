...
class CRUDResponseHandler {
    ...
    static async handleSaveResponse(response, options = {}) {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                // שגיאת אימות (HTTP 401/403) - טיפול מיוחד לבדיקות
                if (response.status === 401 || response.status === 403) {
                    const message = 'נדרש אימות - יש להתחבר למערכת';

                    // #region agent log - auth error during testing
                    fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41', {
                      method: 'POST',
                      headers: { 'Content-Type': 'application/json' },
                      body: JSON.stringify({
                        location: 'crud-response-handler.js:handleSaveResponse:auth_error',
                        message: '401/403 auth error during CRUD operation - user-friendly handling',
                        data: {
                          timestamp: Date.now(),
                          page: window.location.pathname,
                          operation: options.entityName || 'entity',
                    ...
                    window.Logger?.warn(`⚠️ Auth required for ${options.entityName || 'operation'}: ${response.status}`);

                    // Show user-friendly auth message
                    if (typeof window.showInfoNotification === 'function') {
                        window.showInfoNotification('אימות נדרש', message);
                    }
    ...
    static handleError(error, operation = 'פעולה') {
        // Enhanced error handling for auth/API errors in testing scenarios
        const isAuthError = error?.message?.includes('401') || error?.message?.includes('Unauthorized') ||
                           error?.status === 401 || error?.status === 403;
        const isValidationError = error?.message?.includes('400') || error?.status === 400 ||
                                 error?.message?.includes('Bad Request');

        // Reduce console noise for expected auth/API errors during testing
        if (isAuthError || isValidationError) {
            // #region agent log - expected auth/api error during testing
            fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({
                location: 'crud-response-handler.js:handleError:expected_error',
                message: 'Expected auth/API error detected - reducing console noise',
                data: {
                  timestamp: Date.now(),
                  page: window.location.pathname,
                  operation: operation,
                  errorType: isAuthError ? 'auth' : 'validation',
            ...
            window.Logger?.warn(`⚠️ Expected ${isAuthError ? 'auth' : 'validation'} error in ${operation}:`, error?.message || error);

            // Show user-friendly messages instead of technical errors
            if (isAuthError) {
                if (typeof window.showInfoNotification === 'function') {
                    window.showInfoNotification('אימות נדרש', 'יש להתחבר כדי לבצע פעולה זו');
                }
            } else if (isValidationError) {
                if (typeof window.showWarningNotification === 'function') {
                    window.showWarningNotification('נתונים לא תקינים', 'יש לתקן את השדות המסומנים ולנסות שוב');
                }
            }
            return; // Don't show generic error notification
        }

        // For unexpected errors, show full logging and notification
    ...
    static async refreshEntityTables(entityType) {
        try {
            ...
            const actionMap = {
                ...
                'trade_plan': 'trade_plan-updated',
                'trade_plans': 'trade_plan-updated',
                'alert': 'alert-updated',
