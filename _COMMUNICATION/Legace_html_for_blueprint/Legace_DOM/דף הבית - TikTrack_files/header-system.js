...
window.FilterManager = class FilterManager {
  ...
  async init() {
    ...
    // #region agent log - HeaderSystem.init start
    fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        location: 'header-system.js:HeaderSystem.init:start',
        message: 'HeaderSystem.init called',
        data: {
          timestamp: Date.now(),
          page: window.location.pathname,
          documentReadyState: document.readyState,
          hasUnifiedHeader: !!document.getElementById('unified-header'),
          unifiedHeaderContent: document.getElementById('unified-header')?.innerHTML?.length || 0,
          authState: {
        ...
        sessionId: 'header_debug_session',
        runId: 'p0_redirect_loop_fix',
        hypothesisId: 'H4_header_auth_state'
      })
    }).catch(() => {});
    // #endregion
    ...
    // If auth verification is still in progress and no token, skip to avoid 401 redirect loops
    if (window.isAuthVerified === false && !window.authToken && !sessionStorage.getItem('authToken')) {
      window.Logger?.warn?.('⏳ Skipping account filter load until auth verified', { page: 'header-system' });
      return;
    }
  ...
  async loadAccountsForFilter() {
    ...
    // Priority 1: Use DataImportData service (only on data_import page)
    const currentPageKey = document.body.getAttribute('data-page-key') ||
                          document.querySelector('meta[name="page-key"]')?.getAttribute('content') ||
                          'default';
    if (currentPageKey === 'data_import' && window.DataImportData?.loadTradingAccountsForImport) {
      try {
    ...
    if (!accounts || accounts.length === 0) {
      try {
        ...
        // Graceful 401 handling - return empty array instead of blocking page
        if (response.status === 401 || response.status === 403) {
          window.Logger?.debug?.('⚠️ Authentication required for trading accounts (expected)', { page: 'header-system', status: response.status });
          accounts = [];
        } else if (!response.ok) {
...
class HeaderSystem {
  ...
  static createHeader() {
    ...
    // Header creation logging
    console.log('[HeaderSystem] createHeader: Starting header creation', {
      page: window.location.pathname,
      hasUnifiedHeader: !!document.getElementById('unified-header'),
      documentReadyState: document.readyState
    });
    ...
    // #region agent log - header element check
    fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        location: 'header-system.js:createHeader:element_check',
        message: 'Checking for unified-header element',
        data: {
          timestamp: Date.now(),
          page: window.location.pathname,
          headerElementExists: !!headerElement,
          documentBodyExists: !!document.body,
          headerElementContentLength: headerElement?.innerHTML?.length || 0
        },
        sessionId: 'header_debug_session',
        hypothesisId: 'H3_header_creation'
      })
    }).catch(() => {});
    // #endregion
    ...
    if (!headerElement) {
      ...
      // #region agent log - header element created
      fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          location: 'header-system.js:createHeader:element_created',
          message: 'Header element created dynamically',
    ...
    // #region agent log - header HTML generated
    fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        location: 'header-system.js:createHeader:html_generated',
        message: 'Header HTML generated',
        data: {
          timestamp: Date.now(),
          htmlLength: headerHTML.length,
          hasNavLinks: headerHTML.includes('tiktrack-nav-link')
        },
        sessionId: 'header_debug_session',
        hypothesisId: 'H3_header_creation'
      })
    ...
    // #region agent log - DOM parser execution
    fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        location: 'header-system.js:createHeader:dom_parser',
        message: 'DOM Parser execution',
        data: {
          timestamp: Date.now(),
        ...
        sessionId: 'header_debug_session',
        hypothesisId: 'H4_dom_parser_issue'
      })
    }).catch(() => {});
    // #endregion
    ...
    // #region agent log - DOM parser result
    fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        location: 'header-system.js:createHeader:dom_parsed',
        message: 'DOM Parser completed',
        data: {
          timestamp: Date.now(),
          docBodyExists: !!doc.body,
          childNodesCount: doc.body ? doc.body.childNodes.length : 0,
          firstChildType: doc.body && doc.body.childNodes.length > 0 ? doc.body.childNodes[0].nodeType : null,
          firstChildTag: doc.body && doc.body.childNodes.length > 0 ? doc.body.childNodes[0].tagName : null
        },
        sessionId: 'header_debug_session',
        hypothesisId: 'H4_dom_parser_issue'
      })
    ...
    // VERIFICATION: Confirm header was populated
    console.log('[HEADER VERIFICATION] Header creation completed for:', window.location.pathname);
    console.log('[HEADER VERIFICATION] Final header content length:', existingHeader.innerHTML.length);
    console.log('[HEADER VERIFICATION] Header contains navigation:', existingHeader.innerHTML.includes('tiktrack-nav-link'));
    console.log('[HEADER VERIFICATION] Header contains menu:', existingHeader.innerHTML.includes('tiktrack-dropdown'));

    // #region agent log - DOM injection completed
    fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        location: 'header-system.js:createHeader:dom_injection',
        message: 'DOM injection completed',
        data: {
          timestamp: Date.now(),
          page: window.location.pathname,
          injectedNodesCount: doc.body.childNodes.length,
          finalHeaderContentLength: existingHeader.innerHTML.length,
          hasNavigation: existingHeader.innerHTML.includes('nav'),
          hasMenu: existingHeader.innerHTML.includes('menu'),
          firstChildTag: existingHeader.firstElementChild?.tagName
        },
        sessionId: 'header_debug_session',
        hypothesisId: 'H5_dom_injection'
      })
    }).catch(() => {});
    // #endregion

    // #region agent log - header creation completed
    fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        location: 'header-system.js:createHeader:completed',
        message: 'Header creation completed',
        data: {
          timestamp: Date.now(),
          finalContentLength: existingHeader.innerHTML.length,
          navLinksCount: existingHeader.querySelectorAll('.tiktrack-nav-link').length
        },
        sessionId: 'header_debug_session',
        hypothesisId: 'H3_header_creation'
      })
                  ...
                  <ul class="tiktrack-nav-list">
                    ...
                    <li class="tiktrack-nav-item dropdown">
                      ...
                      <ul class="tiktrack-dropdown-menu">
                        ...
                        <li><a class="tiktrack-dropdown-item" href="/user_ticker">הטיקרים שלי</a></li>
                        <li><a class="tiktrack-dropdown-item" href="/tickers">טיקרים (מנהל)</a></li>
...
window.HeaderSystemClass = HeaderSystem;
  ...
  // HeaderSystem class is fully implemented above
  // No minimal version override needed

  // Define window.initializeHeaderSystem for core-systems compatibility
  window.initializeHeaderSystem = async function() {
    // #region agent log - header initialization start
    fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        location: 'header-system.js:initializeHeaderSystem:start',
        message: 'initializeHeaderSystem called',
        data: {
          timestamp: Date.now(),
          page: window.location.pathname,
          hasUnifiedHeader: !!document.getElementById('unified-header'),
          unifiedHeaderContentBefore: document.getElementById('unified-header')?.innerHTML?.length || 0
        },
        sessionId: 'header_debug_session',
        hypothesisId: 'H6_header_clearing_investigation'
      })
    }).catch(() => {});
    // #endregion
  console.log('🎯 window.initializeHeaderSystem() called');

  // Header system initialization logging
  console.log('[HeaderSystem] initializeHeaderSystem called', {
    page: window.location.pathname,
    HeaderSystemExists: typeof HeaderSystem !== 'undefined',
    headerSystemInstanceExists: !!window.headerSystemInstance,
    documentReadyState: document.readyState,
    hasUnifiedHeader: !!document.getElementById('unified-header')
  });

    try {
      // If HeaderSystem class exists, create an instance and initialize
      if (typeof HeaderSystem !== 'undefined') {
        console.log('🔧 Initializing HeaderSystem with full class...');
        ...
        await window.headerSystemInstance.init();
        console.log('✅ HeaderSystem initialized successfully');

        // #region agent log - header initialization complete - check content
        fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            location: 'header-system.js:initializeHeaderSystem:end',
            message: 'initializeHeaderSystem completed - checking header content',
            data: {
              timestamp: Date.now(),
              page: window.location.pathname,
              hasUnifiedHeader: !!document.getElementById('unified-header'),
              unifiedHeaderContentAfter: document.getElementById('unified-header')?.innerHTML?.length || 0,
              hasNavigation: document.getElementById('unified-header')?.innerHTML?.includes('nav') || false,
              hasMenu: document.getElementById('unified-header')?.innerHTML?.includes('menu') || false
            },
            sessionId: 'header_debug_session',
            hypothesisId: 'H6_header_clearing_investigation'
          })
      ...
      console.warn('⚠️ HeaderSystem class not available, using minimal version');
      // Fallback to minimal version if available
      if (window.HeaderSystem && typeof window.HeaderSystem.initialize === 'function') {
        return await window.HeaderSystem.initialize();
      }
      console.log('✅ Using minimal HeaderSystem fallback');
      return true;
    }
  } catch (error) {
    console.error('❌ Error initializing HeaderSystem:', error);
    // Don't throw - allow page to continue loading
    return false;
  }
};

// Header initialization is handled by core-systems.js
// No DOM event listeners needed here

// #region Header Content Watcher - Debug header clearing
(function() {
  ...
  function checkHeaderContent() {
    ...
    if (currentContent !== lastHeaderContent) {
      // #region agent log - header content changed
      fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          location: 'header-system.js:headerWatcher:content_changed',
          message: 'Header content changed',
          data: {
            timestamp: Date.now(),
            page: window.location.pathname,
            previousLength: lastHeaderContent.length,
            currentLength: currentContent.length,
            wasCleared: lastHeaderContent.length > 0 && currentContent.length === 0,
      ...
      lastHeaderContent = currentContent;
    }
  }
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => {
      watcherInterval = setInterval(checkHeaderContent, 500); // Check every 500ms
    });
  } else {
    watcherInterval = setInterval(checkHeaderContent, 500);
  }
