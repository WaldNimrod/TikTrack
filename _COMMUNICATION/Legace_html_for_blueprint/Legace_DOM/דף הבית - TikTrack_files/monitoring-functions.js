...
async function checkForMismatches(pageName, pageConfig, htmlScripts = null) {
    ...
    const knownDynamicScripts = [
        ...
        'trading_journal-page.js',
        'date-comparison-modal.js',
        'trade_history-page.js',
        'portfolio-state.js',
    ...
    loadedScripts.forEach(loadedScript => {
        ...
        // Also check if script is part of entity_details package (modal-navigation-manager, modal-manager-v2)
        ...
        // Check if script is part of entity_details package (even if modules package is not included)
        // modal-navigation-manager and modal-manager-v2 are often loaded with entity_details
        if (!isAllowedSystemScript && pageConfig.packages && pageConfig.packages.includes('entity_details')) {
            ...
            if (entityDetailsScripts.some(name => scriptName.includes(name))) {
                isAllowedSystemScript = true; // Consider it allowed if entity_details is included
            }
        }
...
async function runDetailedPageScan(pageName, pageConfig) {
    ...
    } else if (path.includes('trade_history-page')) {
        currentPage = 'trade_history-page';
    } else if (path.includes('portfolio-state')) {
        currentPage = 'portfolio-state';
    } else if (path.includes('trading_journal-page')) {
        currentPage = 'trading_journal-page';
    }
