...
(function tagServiceFactory() {
    ...
    async function fetchCategories(options = {}) {
        ...
        const categories = await requestJSON('/api/tags/categories');
    ...
    async function createCategory(payload) {
        const category = await requestJSON('/api/tags/categories', {
            method: 'POST',
    ...
    async function updateCategory(categoryId, payload) {
        const category = await requestJSON(`/api/tags/categories/${categoryId}`, {
            method: 'PUT',
            body: JSON.stringify(payload)
        });
        await removeCached(CACHE_KEYS.categories);
    ...
    async function deleteCategory(categoryId) {
        await requestJSON(`/api/tags/categories/${categoryId}`, { method: 'DELETE' });
        await removeCached(CACHE_KEYS.categories);
    ...
    async function validateTagCategoryViaAPI(category) {
        ...
        try {
            ...
            if (window.CacheTTLGuard?.ensure) {
                return await window.CacheTTLGuard.ensure(cacheKey, async () => {
                    const response = await fetch('/api/business/tag/validate_category', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
