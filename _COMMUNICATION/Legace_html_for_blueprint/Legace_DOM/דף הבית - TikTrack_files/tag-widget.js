...
;(function () {
  ...
  async function refreshTagCloud({ force = false } = {}) {
    // #region agent log - refreshTagCloud started
    fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'trading-ui/scripts/widgets/tag-widget.js:refreshTagCloud:start',message:'refreshTagCloud started',data:{force,timestamp:Date.now()},sessionId:'debug-session',runId:'widget_data_loading',hypothesisId:'widget_loading'})}).catch(()=>{});
    // #endregion
    ...
    try {
      // #region agent log - TagService.getTagCloudData call
      fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'trading-ui/scripts/widgets/tag-widget.js:refreshTagCloud:getTagCloudData',message:'Calling TagService.getTagCloudData',data:{force:force,hasTagService:!!window.TagService,tagServiceMethods:Object.keys(window.TagService||{}),timestamp:Date.now()},sessionId:'debug-session',runId:'widget_render_fixes',hypothesisId:'tag_widget_data'})}).catch(()=>{});
      // #endregion
      ...
      // #region agent log - refreshTagCloud success
      fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'trading-ui/scripts/widgets/tag-widget.js:refreshTagCloud:success',message:'refreshTagCloud success',data:{tagsCount:Array.isArray(data)?data.length:0,timestamp:Date.now()},sessionId:'debug-session',runId:'widget_data_loading',hypothesisId:'widget_loading'})}).catch(()=>{});
      // #endregion
      renderTagCloud(Array.isArray(data) ? data : []);
    } catch (error) {
      // #region agent log - refreshTagCloud error
      fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'trading-ui/scripts/widgets/tag-widget.js:refreshTagCloud:error',message:'refreshTagCloud error',data:{error:error?.message,timestamp:Date.now()},sessionId:'debug-session',runId:'widget_data_loading',hypothesisId:'widget_loading'})}).catch(()=>{});
      // #endregion
      ...
      // #region agent log - refreshTagCloud loading cleared
      fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'trading-ui/scripts/widgets/tag-widget.js:refreshTagCloud:loadingCleared',message:'refreshTagCloud loading state cleared',data:{timestamp:Date.now()},sessionId:'debug-session',runId:'widget_data_loading',hypothesisId:'widget_loading'})}).catch(()=>{});
      // #endregion
    }
  }
  ...
  function toggleTagCloudState({ loading = false } = {}) {
    if (elements.cloudLoading) {
      ...
      elements.cloudLoading.style.display = loading ? '' : 'none';
      // Also hide/show all SPAN elements inside the loading container
      const spans = elements.cloudLoading.querySelectorAll('span');
      spans.forEach(span => {
        span.classList.toggle('d-none', !loading);
  ...
  function renderTagCloud(tags) {
    // #region agent log - renderTagCloud called
    fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'trading-ui/scripts/widgets/tag-widget.js:renderTagCloud:entry',message:'renderTagCloud called',data:{tagsCount:tags?.length||0,tagsType:typeof tags,hasContainer:!!elements.cloudContainer,timestamp:Date.now()},sessionId:'debug-session',runId:'widget_render_fixes',hypothesisId:'tag_widget_render'})}).catch(()=>{});
    // #endregion

    // ✅ נקה loading לפני רינדור
    if (elements.cloudLoading) {
      elements.cloudLoading.classList.add('d-none');
      elements.cloudLoading.style.display = 'none';
      elements.cloudLoading.hidden = true;
      const spans = elements.cloudLoading.querySelectorAll('span');
      spans.forEach(span => {
  ...
  function addPaginationControls(tableId, totalItems, pageSize, currentPage, totalPages) {
    ...
    const existingPagination = tableWrapper.parentElement.querySelector('.linked_items-pagination');
    ...
    paginationContainer.className = 'linked_items-pagination d-flex justify-content-between align-items-center mt-3';
  ...
  function renderDrawerRows(results) {
    ...
    const existingSection = elements.drawerBody.querySelector('section.content-section[data-section^="linked_items-"]');
    if (existingSection) {
    ...
    if (linkedItemsHtml) {
      ...
      if (linkedItemsSection) {
        ...
        const existingLinkedSection = elements.drawerBody.querySelector('section.content-section[data-section^="linked_items-"]');
  ...
  const TagWidget = {
    ...
    init(containerId = CONTAINER_ID, config = {}) {
      ...
      // #region agent log - H3: TagWidget init before refresh
      fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'trading-ui/scripts/widgets/tag-widget.js:init:beforeRefresh',message:'H3: TagWidget init - before refreshTagCloud',data:{timestamp:Date.now(),tagServiceExists:!!window.TagService,tagServiceMethods:window.TagService?Object.keys(window.TagService):[],containerExists:!!elements.cloudContainer},sessionId:'debug-session',runId:'tag_widget_debug',hypothesisId:'H3'})}).catch(()=>{});
      // #endregion
      
      window.Logger?.info?.('TagWidget: Starting tag cloud refresh...', { page: 'tag-widget' });
      refreshTagCloud().catch((error) => {
        // #region agent log - H3: refreshTagCloud error
        fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'trading-ui/scripts/widgets/tag-widget.js:init:refreshError',message:'H3: refreshTagCloud error in init',data:{timestamp:Date.now(),error:error?.message},sessionId:'debug-session',runId:'tag_widget_debug',hypothesisId:'H3'})}).catch(()=>{});
        // #endregion
