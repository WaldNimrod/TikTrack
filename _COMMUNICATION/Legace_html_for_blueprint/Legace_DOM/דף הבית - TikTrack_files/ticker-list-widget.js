...
;(function () {
  ...
  async function renderTickersList(tab, tickers) {
    // #region agent log - renderTickersList started
    fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'trading-ui/scripts/widgets/ticker-list-widget.js:renderTickersList:start',message:'renderTickersList started',data:{tab,tickersCount:tickers?.length||0,timestamp:Date.now()},sessionId:'debug-session',runId:'widget_data_loading',hypothesisId:'widget_loading'})}).catch(()=>{});
    // #endregion
    const listEl = elements[`${tab}List`];
    if (!listEl) return;
    ...
    if (loadingEl) {
      ...
      // #region agent log - loading indicator hidden
      fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'trading-ui/scripts/widgets/ticker-list-widget.js:renderTickersList:hideLoading',message:'Loading indicator hidden',data:{tab,loadingElId:loadingEl.id,spansHidden:spans.length,timestamp:Date.now()},sessionId:'debug-session',runId:'widget_data_loading',hypothesisId:'widget_loading'})}).catch(()=>{});
      // #endregion
    }
    if (emptyEl) emptyEl.classList.add('d-none');
  ...
  async function loadActiveTickers() {
    // #region agent log - loadActiveTickers started
    fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'trading-ui/scripts/widgets/ticker-list-widget.js:loadActiveTickers:start',message:'loadActiveTickers started',data:{timestamp:Date.now()},sessionId:'debug-session',runId:'widget_data_loading',hypothesisId:'widget_loading'})}).catch(()=>{});
    // #endregion
    showLoading('active');
    state.activeTickers = [];
    ...
    try {
      ...
      if (data.status === 'success' && Array.isArray(data.data)) {
        ...
        // Filter only fulfilled results
        const fulfilledTickers = enrichedTickers
          .filter(result => result.status === 'fulfilled')
          .map(result => result.value);
        
        state.activeTickers = enrichedTickers;
        // #region agent log - loadActiveTickers success
        fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'trading-ui/scripts/widgets/ticker-list-widget.js:loadActiveTickers:success',message:'loadActiveTickers success',data:{tickersCount:enrichedTickers.length,timestamp:Date.now()},sessionId:'debug-session',runId:'widget_data_loading',hypothesisId:'widget_loading'})}).catch(()=>{});
        // #endregion
      ...
      // #region agent log - loadActiveTickers error
      fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'trading-ui/scripts/widgets/ticker-list-widget.js:loadActiveTickers:error',message:'loadActiveTickers error',data:{error:error?.message,timestamp:Date.now()},sessionId:'debug-session',runId:'widget_data_loading',hypothesisId:'widget_loading'})}).catch(()=>{});
      // #endregion
  ...
  async function loadAllTickers() {
    ...
    try {
      const response = await fetch('/api/tickers/with_initial_data', {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
          'Accept': 'application/json'
