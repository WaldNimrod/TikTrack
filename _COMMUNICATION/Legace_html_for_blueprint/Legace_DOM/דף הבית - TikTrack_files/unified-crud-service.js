...
class UnifiedCRUDService {
    ...
    static async saveEntity(entityType, entityData, options = {}) {
        ...
        try {
            ...
            const crudOptions = {
                ...
                returnErrorDetails: true // ALWAYS return error details for better error messages
            };
                    ...
                    try {
                        ...
                        if (isModalOpen) {
                            ...
                            if (linkedItemsEntry && linkedItemsEntry.entityType && linkedItemsEntry.entityId) {
                                ...
                                const cacheKey = `linked_items-${linkedItemsEntry.entityType}-${linkedItemsEntry.entityId}`;
                                ...
                                if (window.entityDetailsAPI?.clearEntityCache) {
                                    try {
                                        window.entityDetailsAPI.clearEntityCache('linked_items', `${linkedItemsEntry.entityType}-${linkedItemsEntry.entityId}`);
                                    } catch (e) {
                                ...
                                try {
                                    const url = `/api/linked_items/${linkedItemsEntry.entityType}/${linkedItemsEntry.entityId}?_t=${Date.now()}`;
    ...
    static _getEntityAPIEndpoint(entityType) {
        const endpointMap = {
            'trade': '/api/trades',
            'trade_plan': '/api/trade_plans',
            'alert': '/api/alerts',
            'ticker': '/api/tickers',
            'trading_account': '/api/trading_accounts',
            'execution': '/api/executions',
            'cash_flow': '/api/cash_flows',
            'note': '/api/notes',
            'watch_list': '/api/watch_lists',
            'import_session': '/api/user_data_import/session',
            'tag': '/api/tags',
            'tag_category': '/api/tags/categories',
            'trading_journal': '/api/trading_journal',
            'preference_profile': '/api/preference_profiles',
