...
async function getAccounts() {
  try {
    ...
    // Graceful 401/403 handling - return empty array without blocking
    if (response.status === 401 || response.status === 403) {
      window.Logger?.debug?.('⚠️ Authentication required for trading accounts (expected)', { 
    ...
    if (response.ok) {
      ...
      window.Logger?.warn?.('Failed to fetch accounts', { status: response.status, statusText: response.statusText });
      return [];
    }
  } catch (error) {
    // Error fetching accounts
    window.Logger?.warn?.('Error fetching accounts', error);
    return [];
  }
}
