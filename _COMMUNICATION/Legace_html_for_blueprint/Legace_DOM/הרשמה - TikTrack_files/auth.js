...
// Global auth verification flag (used by wrapper/header to avoid race redirects)
if (typeof window !== 'undefined' && typeof window.isAuthVerified === 'undefined') {
...
(function bootstrapAuthFromSessionStorage() {
  ...
  try {
    ...
    let storedUserRaw = sessionStorage.getItem(SESSION_USER_KEY);

    // If sessionStorage is empty but window globals exist, restore from window globals (H5 fix)
    if ((!storedToken || !storedUserRaw) && window.authToken && window.currentUser) {
      // #region agent log - H5: Restoring from window globals
      fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'trading-ui/scripts/auth.js:bootstrapAuthFromSessionStorage:restoreFromWindow',message:'Restoring auth from window globals',data:{hasWindowToken:!!window.authToken,hasWindowUser:!!window.currentUser,hasSessionToken:!!storedToken,hasSessionUser:!!storedUserRaw,timestamp:Date.now()},sessionId:'debug-session',runId:'auth_cache_consistency',hypothesisId:'H5'})}).catch(()=>{});
      // #endregion
      storedToken = window.authToken;
      storedUserRaw = typeof window.currentUser === 'string' ? window.currentUser : JSON.stringify(window.currentUser);
      // Restore to sessionStorage
      sessionStorage.setItem(SESSION_TOKEN_KEY, storedToken);
      sessionStorage.setItem(SESSION_USER_KEY, storedUserRaw);
    }
    ...
    fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41', {
      ...
      body: JSON.stringify({
        ...
        data: {
          ...
          restoredFromWindow: (!sessionStorage.getItem(SESSION_TOKEN_KEY) && !!window.authToken),
    ...
    // #region agent log - H1/H4: Bootstrap UCM sync attempt
    fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'trading-ui/scripts/auth.js:bootstrapAuthFromSessionStorage:ucmSync',message:'Bootstrap checking UCM for sync',data:{ucmExists:!!window.UnifiedCacheManager,ucmInitialized:window.UnifiedCacheManager?.initialized,tokenExists:!!storedToken,userExists:!!storedUser,timestamp:Date.now()},sessionId:'debug-session',runId:'auth_cache_consistency',hypothesisId:'H1_H4'})}).catch(()=>{});
    // #endregion
    if (window.UnifiedCacheManager) {
      // Try immediate sync if already initialized
      if (window.UnifiedCacheManager.initialized) {
        // #region agent log - H1/H4: Bootstrap UCM sync executing
        fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'trading-ui/scripts/auth.js:bootstrapAuthFromSessionStorage:ucmSyncExec',message:'Bootstrap syncing to UCM',data:{timestamp:Date.now()},sessionId:'debug-session',runId:'auth_cache_consistency',hypothesisId:'H1_H4'})}).catch(()=>{});
        // #endregion
        ...
        // #region agent log - H1/H4: Bootstrap UCM deferred sync starting
        fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'trading-ui/scripts/auth.js:bootstrapAuthFromSessionStorage:ucmDeferred',message:'Bootstrap starting deferred UCM sync',data:{timestamp:Date.now()},sessionId:'debug-session',runId:'auth_cache_consistency',hypothesisId:'H1_H4'})}).catch(()=>{});
        // #endregion
        
        // Store sync function on window so UCM can call it when ready
        window._bootstrapAuthSync = async function() {
          if (window.UnifiedCacheManager?.initialized && storedToken && storedUser) {
            // #region agent log - H1/H4: Bootstrap UCM deferred sync executing
            fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'trading-ui/scripts/auth.js:bootstrapAuthFromSessionStorage:ucmDeferredExec',message:'Bootstrap deferred sync executing',data:{timestamp:Date.now()},sessionId:'debug-session',runId:'auth_cache_consistency',hypothesisId:'H1_H4'})}).catch(()=>{});
            // #endregion
            try {
              await window.UnifiedCacheManager.save('authToken', storedToken, { 
                layer: 'sessionStorage', 
                includeUserId: false 
              });
              await window.UnifiedCacheManager.save('currentUser', storedUser, { 
                layer: 'sessionStorage', 
                includeUserId: false 
              });
              // Clean up after successful sync
              delete window._bootstrapAuthSync;
            } catch (error) {
              window.Logger?.warn?.('⚠️ Bootstrap auth sync to UCM failed', { error: error?.message }, { page: 'auth' });
            }
          }
        };
        ...
        const syncInterval = setInterval(() => {
          if (window.UnifiedCacheManager?.initialized) {
            // #region agent log - H1/H4: Bootstrap UCM deferred sync executing (interval)
            fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'trading-ui/scripts/auth.js:bootstrapAuthFromSessionStorage:ucmDeferredExecInterval',message:'Bootstrap deferred sync executing via interval',data:{timestamp:Date.now()},sessionId:'debug-session',runId:'auth_cache_consistency',hypothesisId:'H1_H4'})}).catch(()=>{});
            // #endregion
            clearInterval(syncInterval);
            if (window._bootstrapAuthSync) {
              window._bootstrapAuthSync();
            }
          }
        }, 100);
        // Timeout after 5 seconds to prevent infinite polling
        setTimeout(() => {
          // #region agent log - H1/H4: Bootstrap UCM deferred sync timeout
          fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'trading-ui/scripts/auth.js:bootstrapAuthFromSessionStorage:ucmDeferredTimeout',message:'Bootstrap deferred sync timeout',data:{ucmInitialized:window.UnifiedCacheManager?.initialized,timestamp:Date.now()},sessionId:'debug-session',runId:'auth_cache_consistency',hypothesisId:'H1_H4'})}).catch(()=>{});
          // #endregion
          clearInterval(syncInterval);
        }, 5000);
      }
...
async function saveAuthToCache(user, token = 'session_based') {
  // #region agent log - H1/H2: UCM initialization timing
  fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'trading-ui/scripts/auth.js:saveAuthToCache:entry',message:'saveAuthToCache called',data:{userId:user?.id,username:user?.username,tokenType:typeof token,ucmExists:!!window.UnifiedCacheManager,ucmInitialized:window.UnifiedCacheManager?.initialized,sessionTokenBefore:sessionStorage.getItem('authToken'),sessionUserBefore:!!sessionStorage.getItem('currentUser'),timestamp:Date.now()},sessionId:'debug-session',runId:'homepage_auth_persistence',hypothesisId:'H1_H2'})}).catch(()=>{});
  // #endregion
  ...
  try {
    // #region agent log - H2: Checking for key mismatch (dev_* vs authToken/currentUser)
    const devTokenExists = typeof sessionStorage !== 'undefined' && !!sessionStorage.getItem('dev_authToken');
    const devUserExists = typeof sessionStorage !== 'undefined' && !!sessionStorage.getItem('dev_currentUser');
    fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'trading-ui/scripts/auth.js:saveAuthToCache:keyCheck',message:'Checking for key mismatch',data:{devTokenExists,devUserExists,currentTokenKey:SESSION_TOKEN_KEY,currentUserKey:SESSION_USER_KEY,timestamp:Date.now()},sessionId:'debug-session',runId:'auth_cache_consistency',hypothesisId:'H2'})}).catch(()=>{});
    // #endregion
    ...
    // #region agent log - H1/H2: Token saved verification
    fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'trading-ui/scripts/auth.js:saveAuthToCache:exit',message:'saveAuthToCache completed',data:{userId:user?.id,verifyUser:!!verifyUser,verifyToken:!!verifyToken,sessionTokenAfter:sessionStorage.getItem('authToken'),sessionUserAfter:!!sessionStorage.getItem('currentUser'),ucmInitialized:window.UnifiedCacheManager?.initialized,usedUCM:window.UnifiedCacheManager?.initialized,timestamp:Date.now()},sessionId:'debug-session',runId:'homepage_auth_persistence',hypothesisId:'H1_H2'})}).catch(()=>{});
    // #endregion
...
async function getAuthFromCache() {
  ...
  // #region agent log - H1/H5: getAuthFromCache entry
  fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'trading-ui/scripts/auth.js:getAuthFromCache:entry',message:'getAuthFromCache called',data:{ucmExists:!!window.UnifiedCacheManager,ucmInitialized:window.UnifiedCacheManager?.initialized,sessionTokenKey:SESSION_TOKEN_KEY,sessionUserKey:SESSION_USER_KEY,sessionTokenExists:!!sessionStorage.getItem(SESSION_TOKEN_KEY),sessionUserExists:!!sessionStorage.getItem(SESSION_USER_KEY),timestamp:Date.now()},sessionId:'debug-session',runId:'auth_cache_consistency',hypothesisId:'H1_H5'})}).catch(()=>{});
  // #endregion
  
  try {
    // Try SessionStorageLayer through UnifiedCacheManager first (preferred method)
    if (window.UnifiedCacheManager?.initialized) {
      // #region agent log - H1/H5: getAuthFromCache UCM read attempt
      fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'trading-ui/scripts/auth.js:getAuthFromCache:ucmRead',message:'getAuthFromCache reading from UCM',data:{timestamp:Date.now()},sessionId:'debug-session',runId:'auth_cache_consistency',hypothesisId:'H1_H5'})}).catch(()=>{});
      // #endregion
      ...
      // #region agent log - H1/H5: getAuthFromCache UCM read result
      fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'trading-ui/scripts/auth.js:getAuthFromCache:ucmResult',message:'getAuthFromCache UCM read result',data:{userFound:!!user,tokenFound:!!token,userId:user?.id,timestamp:Date.now()},sessionId:'debug-session',runId:'auth_cache_consistency',hypothesisId:'H1_H5'})}).catch(()=>{});
      // #endregion
    ...
    // Fallback: direct sessionStorage (bootstrap mode - before UnifiedCacheManager initializes)
    // #region agent log - H1/H5: getAuthFromCache fallback to sessionStorage
    fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'trading-ui/scripts/auth.js:getAuthFromCache:fallback',message:'getAuthFromCache falling back to sessionStorage',data:{ucmInitialized:window.UnifiedCacheManager?.initialized,sessionTokenKey:SESSION_TOKEN_KEY,sessionUserKey:SESSION_USER_KEY,timestamp:Date.now()},sessionId:'debug-session',runId:'auth_cache_consistency',hypothesisId:'H1_H5'})}).catch(()=>{});
    // #endregion
    if (typeof sessionStorage !== 'undefined') {
      const user = JSON.parse(sessionStorage.getItem(SESSION_USER_KEY) || 'null');
      const token = sessionStorage.getItem(SESSION_TOKEN_KEY);
      // #region agent log - H1/H5: getAuthFromCache sessionStorage result
      fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'trading-ui/scripts/auth.js:getAuthFromCache:sessionResult',message:'getAuthFromCache sessionStorage read result',data:{userFound:!!user,tokenFound:!!token,userId:user?.id,timestamp:Date.now()},sessionId:'debug-session',runId:'auth_cache_consistency',hypothesisId:'H1_H5'})}).catch(()=>{});
      // #endregion
      if (user && token) {
        return { user, token };
      }
...
async function forceLogoutAndPrompt(reason = 'unauthorized') {
  // #region agent log - H4: forceLogoutAndPrompt called - this redirects to login
  fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'trading-ui/scripts/auth.js:forceLogoutAndPrompt',message:'forceLogoutAndPrompt called - redirecting to login',data:{reason:reason,currentPage:window.location.pathname,tokensBeforeClear:{sessionStorage:!!sessionStorage.getItem('authToken')},timestamp:Date.now()},sessionId:'debug-session',runId:'homepage_auth_persistence',hypothesisId:'H4_force_logout'})}).catch(()=>{});
  // #endregion
  try {
...
async function login(username, password) {
  // #region agent log - H2/H5: Login called
  fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'trading-ui/scripts/auth.js:login:entry',message:'Login function called',data:{username:!!username,hasUCM:!!window.UnifiedCacheManager,ucmInitialized:window.UnifiedCacheManager?.initialized,sessionTokenBefore:sessionStorage.getItem('authToken'),timestamp:Date.now()},sessionId:'debug-session',runId:'homepage_auth_persistence',hypothesisId:'H2_H5'})}).catch(()=>{});
  // #endregion
  ...
  if (data.data?.user) {
    ...
    try {
      ...
      // ✅ CRITICAL FIX: Also set window globals immediately
      window.authToken = authToken;
      window.currentUser = currentUser;
      
      // #region agent log - auth.js login token saved
      fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'trading-ui/scripts/auth.js:login:tokenSaved',message:'Tokens saved in auth.js login',data:{sessionToken:!!sessionStorage.getItem('authToken'),sessionUser:!!sessionStorage.getItem('currentUser'),windowToken:!!window.authToken,windowUser:!!window.currentUser,timestamp:Date.now()},sessionId:'debug-session',runId:'p0_redirect_loop_fix_v3',hypothesisId:'H9_localStorage_backup'})}).catch(()=>{});
      // #endregion
    } catch (e) {
      // Silently ignore sessionStorage errors
      console.error('[auth.js] Error saving tokens:', e);
    }
    ...
    // #region agent log - H2/H5: Login completed - token saved
    fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'trading-ui/scripts/auth.js:login:exit',message:'Login completed',data:{userId:currentUser?.id,username:currentUser?.username,hasAuthToken:!!authToken,authTokenType:typeof authToken,ucmInitialized:window.UnifiedCacheManager?.initialized,sessionTokenAfter:sessionStorage.getItem('authToken'),sessionUserAfter:!!sessionStorage.getItem('currentUser'),timestamp:Date.now()},sessionId:'debug-session',runId:'homepage_auth_persistence',hypothesisId:'H2_H5'})}).catch(()=>{});
    // #endregion
  }
...
if (typeof window !== 'undefined') {
  ...
  fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41', {
    ...
    headers: {'Content-Type': 'application/json'},
    body: JSON.stringify({
      location: 'auth.js:750',
      message: 'Login function export verification',
      data: {
        loginFunctionType: typeof login,
        windowLoginBefore: typeof window.login,
        exportSuccess: typeof window.login === 'function',
        timestamp: Date.now()
      },
      sessionId: 'homepage_script_chain_debug',
      runId: 'auth_export_check'
    })
  }).catch(() => {});
  // #endregion
...
function setLoadingState(isLoading, buttonId = 'loginBtn', textId = 'loginBtnText', spinnerId = 'loginBtnSpinner') {
  ...
  if (loginBtn && loginBtnText && loginBtnSpinner) {
    if (isLoading) {
      ...
      loginBtnSpinner.classList.add('show');
    } else {
      loginBtn.disabled = false;
      loginBtnText.style.display = 'inline';
      loginBtnSpinner.classList.remove('show');
    }
  }
}
...
async function logout() {
  ...
  try {
    ...
    if (window.UnifiedCacheManager?.clearByPattern) {
      await window.UnifiedCacheManager.clearByPattern('dashboard_data');
    }
...
async function checkAuthentication(onAuthenticated = null, onNotAuthenticated = null) {
    ...
    // Auth verification in progress (used by wrapper/header to avoid premature redirects)
    window.isAuthVerified = false;
  ...
  try {
    ...
    // ✅ CRITICAL FIX V3: Also check window globals as last resort
    if (!tokenAvailable && window.authToken) {
      tokenAvailable = true;
      effectiveToken = window.authToken;
      authToken = window.authToken;
      currentUser = window.currentUser;
      // Save to sessionStorage for persistence
      if (typeof sessionStorage !== 'undefined') {
        sessionStorage.setItem(SESSION_TOKEN_KEY, window.authToken);
        sessionStorage.setItem(SESSION_USER_KEY, JSON.stringify(window.currentUser));
      }
    }
    ...
    console.log('[auth.js] checkAuthentication: window.authToken =', window.authToken ? 'present' : 'null');

    // #region agent log - checkAuthentication token availability
    fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'trading-ui/scripts/auth.js:checkAuthentication:tokenAvailability',message:'Token availability check',data:{tokenAvailable,hasEffectiveToken:!!effectiveToken,hasSessionToken:!!sessionStorage.getItem(SESSION_TOKEN_KEY),hasWindowToken:!!window.authToken,ucmInitialized:window.UnifiedCacheManager?.initialized,timestamp:Date.now()},sessionId:'debug-session',runId:'p0_redirect_loop_fix_v3',hypothesisId:'H10_token_check'})}).catch(()=>{});
    // #endregion
    ...
    if (!tokenAvailable) {
      ...
      // #region agent log - checkAuthentication no token
      fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'trading-ui/scripts/auth.js:checkAuthentication:noToken',message:'No token found in checkAuthentication',data:{hasSessionToken:!!sessionStorage.getItem(SESSION_TOKEN_KEY),hasWindowToken:!!window.authToken,ucmInitialized:window.UnifiedCacheManager?.initialized,timestamp:Date.now()},sessionId:'debug-session',runId:'p0_redirect_loop_fix_v3',hypothesisId:'H10_no_token'})}).catch(()=>{});
      // #endregion
    ...
    // #region agent log - H2: Client sending /api/auth/me request
    fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'trading-ui/scripts/auth.js:checkAuthentication:server_check',message:'Client sending /api/auth/me request',data:{effectiveToken:effectiveToken ? effectiveToken.substring(0,50) + '...' : 'NO_TOKEN',hasAuthHeader:!!effectiveToken,ucmInitialized:window.UnifiedCacheManager?.initialized},sessionId:'debug-session',runId:'homepage_auth_persistence',hypothesisId:'H2_client_request'})}).catch(()=>{});
    // #endregion
    const response = await fetch('/api/auth/me', {
      method: 'GET',
    ...
    // #region agent log - U3: auth check API response
    fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'trading-ui/scripts/auth.js:checkAuthentication:apiResponse',message:'U3: /api/auth/me response received',data:{status:response.status,ok:response.ok,statusText:response.statusText,hasToken:!!effectiveToken,tokenLength:effectiveToken?.length,url:response.url,timestamp:Date.now()},sessionId:'debug-session',runId:'run6_user_mgmt_fix',hypothesisId:'U3'})}).catch(()=>{});
    // #endregion
    // #region agent log - H2/H3: Client received response
    fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'trading-ui/scripts/auth.js:checkAuthentication:response',message:'Client received /api/auth/me response',data:{status:response.status,statusText:response.statusText,headers: Object.fromEntries(response.headers.entries())},sessionId:'debug-session',runId:'homepage_auth_persistence',hypothesisId:'H2_client_response'})}).catch(()=>{});
    // #endregion
    // #region agent log - P0 regression debug
    fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'trading-ui/scripts/auth.js:checkAuthentication:p0_debug',message:'P0 regression debug - response status',data:{status:response.status,ok:response.ok,hasEffectiveToken:!!effectiveToken,tokenLength:effectiveToken?.length,url:response.url,timestamp:Date.now()},sessionId:'debug-session',runId:'p0_auth_regression',hypothesisId:'H11_response_status'})}).catch(()=>{});
    // #endregion
    ...
    await saveAuthToCache(currentUser, authToken);
      ...
      // #region agent log - P0 regression debug 401
      fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'trading-ui/scripts/auth.js:checkAuthentication:p0_debug_401',message:'P0 regression debug - 401 response',data:{status:401,hasEffectiveToken:!!effectiveToken,tokenLength:effectiveToken?.length,hasSessionToken:!!sessionStorage.getItem(SESSION_TOKEN_KEY),hasWindowToken:!!window.authToken,timestamp:Date.now()},sessionId:'debug-session',runId:'p0_auth_regression',hypothesisId:'H11_401_response'})}).catch(()=>{});
      // #endregion
      console.log('[auth.js] checkAuthentication: 401 response - token present but rejected');
    } else {
      // #region agent log - P0 regression debug other error
      fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'trading-ui/scripts/auth.js:checkAuthentication:p0_debug_error',message:'P0 regression debug - non-401 error',data:{status:response.status,statusText:response.statusText,hasEffectiveToken:!!effectiveToken,timestamp:Date.now()},sessionId:'debug-session',runId:'p0_auth_regression',hypothesisId:'H11_other_error'})}).catch(()=>{});
      // #endregion
      console.log('[auth.js] checkAuthentication: Non-401 error:', response.status, response.statusText);
    }
  } catch (error) {
    ...
    if (resolvePromise && !finalResult) {
      // #region agent log - P0 regression debug no finalResult
      fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'trading-ui/scripts/auth.js:checkAuthentication:p0_debug_no_result',message:'P0 regression debug - no finalResult set',data:{hasTokenAvailable:tokenAvailable,hasEffectiveToken:!!effectiveToken,hasSessionToken:!!sessionStorage.getItem(SESSION_TOKEN_KEY),hasWindowToken:!!window.authToken,timestamp:Date.now()},sessionId:'debug-session',runId:'p0_auth_regression',hypothesisId:'H11_no_final_result'})}).catch(()=>{});
      // #endregion
      console.log('[auth.js] checkAuthentication: No finalResult set, returning authenticated: false');
    ...
    // #region agent log - P0 regression debug finally
    fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'trading-ui/scripts/auth.js:checkAuthentication:p0_debug_finally',message:'P0 regression debug - finally block',data:{hasFinalResult:!!finalResult,authenticated:finalResult?.authenticated,error:finalResult?.error,timestamp:Date.now()},sessionId:'debug-session',runId:'p0_auth_regression',hypothesisId:'H11_finally_block'})}).catch(()=>{});
    // #endregion
  }
...
// Export checkAuthentication immediately for early access (before window.TikTrackAuth is defined)
// This allows ensureAuthenticatedForPage to use it even if auth.js is still loading
if (typeof window !== 'undefined') {
  ...
  const loginHTML = `
    ...
    <form id="loginForm">
      ...
      <button type="submit" class="btn-login" id="loginBtn">
        ...
        <span id="loginBtnSpinner" >⏳ מתחבר...</span>
      </button>
    </form>
    
    <div class="forgot_password-wrapper">
      <a href="forgot_password" class="forgot_password-link">שכחת סיסמה?</a>
    </div>
...
async function showForgotPasswordModal(onSuccess = null) {
  ...
  if (form) {
    form.addEventListener('submit', async e => {
      ...
      try {
        const response = await fetch('/api/auth/password_reset/request', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
...
async function showResetPasswordModal(tokenFromUrl = null, onSuccess = null) {
  ...
  if (form) {
    form.addEventListener('submit', async e => {
      ...
      try {
        const validateResponse = await fetch('/api/auth/password_reset/validate', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
        ...
        const response = await fetch('/api/auth/password_reset/reset', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
