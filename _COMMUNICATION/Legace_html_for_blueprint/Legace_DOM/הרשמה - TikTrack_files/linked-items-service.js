 ...
 * איחוד לוגיקה מ-linked_items.js ו-entity_details-renderer.js
 * 
 * תכונות:
...
class LinkedItemsService {
        ...
        if (typeof window.IconSystem !== 'undefined' && window.IconSystem.getEntityIcon) {
            try {
                ...
                if (typeof window.Logger !== 'undefined') {
                    window.Logger.warn('⚠️ Error getting linked item icon from IconSystem, using fallback', { entityType, error, page: 'linked_items-service' });
                }
            }
        }
     ...
     * 
     * @example
     * const actionsHtml = LinkedItemsService.generateLinkedItemActions(item, 'table', { entityColors: this.entityColors, sourceInfo: { sourceModal: 'entity_details', sourceType: 'ticker', sourceId: 123 } });
     */
    static generateLinkedItemActions(item, context = 'modal', options = {}) {
        if (!item || !item.type || !item.id) return '';
        
        let actionsHtml = '<div class="btn-group btn-group-sm linked_items-actions" role="group">';
    ...
    static _getEditFunctionForType(type, id) {
        ...
        const configFileMap = {
            ...
            'trade_plan': '/scripts/modal-configs/trade_plans-config.js?v=1.0.0',
            'ticker': '/scripts/modal-configs/tickers-config.js?v=1.0.0',
            'trading_account': '/scripts/modal-configs/trading_accounts-config.js?v=1.0.0',
            'alert': '/scripts/modal-configs/alerts-config.js?v=1.0.0',
            'cash_flow': '/scripts/modal-configs/cash_flows-config.js?v=1.0.0',
