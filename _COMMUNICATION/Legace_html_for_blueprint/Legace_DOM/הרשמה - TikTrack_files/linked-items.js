 ...
 * - CSS separation: Dedicated linked_items.css file with no inline styles
 *
 * COLOR CODING SYSTEM:
 * ===================
 *
 ...
 * - linked_items.css: Dedicated styling for the modal
 *
 * File: trading-ui/scripts/linked_items.js
 * Version: 3.1
...
function viewLinkedItems(itemId, itemType = null) {
  ...
  if (!detectedItemType) {
    ...
    if (currentPath.includes('/accounts') || currentPath.includes('/trading_accounts')) {detectedItemType = 'trading_account';}
...
function showLinkedItemsModal(data, itemType, itemId, mode = 'view') {
  ...
  if (itemType === 'ticker') {
    ...
    window.Logger.error('❌ DEPRECATED: account entity type used in showLinkedItemsModal', { itemType, itemId }, { page: "linked_items" });
  ...
  if (window.ModalManagerV2 && typeof window.ModalManagerV2.showModal === 'function') {
    ...
    window.ModalManagerV2.showModal(modalElement.id, 'view').catch(error => {
      window.Logger?.error('Error showing linked items modal via ModalManagerV2', { error, modalId: modalElement.id, page: 'linked_items' });
...
function createLinkedItemsModalContent(data, itemType, itemId, mode = 'view') {
  ...
  case 'account':
    ...
    window.Logger.error('❌ DEPRECATED: account entity type used in createLinkedItemsModalContent', { itemType, itemId }, { page: "linked_items" });
  ...
  let content = `
    <div class="linked_items-container">
      <style>
...
function createDetailedItemInfo(item) {
  ...
  case 'account':
    ...
    window.Logger.error('❌ DEPRECATED: account entity type used in createDetailedItemInfo', { item }, { page: "linked_items" });
...
function exportLinkedItemsData(itemType, itemId) {
  try {
    // קבלת הנתונים מהשרת
    fetch(`/api/linked_items/${itemType}/${itemId}`)
  ...
  switch (item.related_type_id) {
    case 1: { // חשבון מסחר
      ...
      if (account) {
        ...
        if (window.Logger && window.Logger.info) {
          window.Logger.info('✅ חשבון נמצא:', { name, currency, id: account.id }, { page: "linked_items" });
        }
      ...
      if (!tradingAccountColor) {
        window.Logger.error('❌ trading_account color not found!', {}, { page: "linked_items" });
      }
