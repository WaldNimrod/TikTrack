...
(function() {
  ...
  // Skip heavy init on dynamic-colors-display and trading_journal pages to avoid recursion issues
  // CRITICAL: Check pathname safely to prevent RangeError
  try {
    if (typeof window !== 'undefined' && window.location && window.location.pathname) {
      const path = window.location.pathname;
      if (path && (path.includes('dynamic-colors-display') || path.includes('trading_journal'))) {
...
function updateCSSVariablesFromPreferences(preferences) {
  try {
      ...
      if (preferences.entityTradePlanColor) {
        document.documentElement.style.setProperty('--entity-trade_plan-color', preferences.entityTradePlanColor);
        document.documentElement.style.setProperty('--entity-trade_plan-bg', preferences.entityTradePlanColorLight || '#d4edda');
        document.documentElement.style.setProperty('--entity-trade_plan-text', preferences.entityTradePlanColorDark || '#155724');
        document.documentElement.style.setProperty('--entity-trade_plan-border', preferences.entityTradePlanColorDark || '#155724');
      }
      ...
      if (preferences.chartSecondaryColor) {
        // ITCSS Compliance: Use CSS class instead of inline style properties
        // CSS variables are now defined in _global-settings.css
        document.documentElement.classList.add('preferences-loaded');
      }
    ...
    if (preferences.entityTradePlanColor) {
      document.documentElement.style.setProperty('--entity-trade_plan-color', preferences.entityTradePlanColor);
      document.documentElement.style.setProperty('--entity-trade_plan-bg', preferences.entityTradePlanColorLight || 'rgba(255, 169, 0, 0.1)');
        document.documentElement.style.setProperty('--entity-trade_plan-text', '#b35d00');
      document.documentElement.style.setProperty('--entity-trade_plan-border', `rgba(${hexToRgb(preferences.entityTradePlanColor)?.r || 255}, ${hexToRgb(preferences.entityTradePlanColor)?.g || 169}, ${hexToRgb(preferences.entityTradePlanColor)?.b || 0}, 0.3)`);
    }
    if (preferences.entityExecutionColor) {
      document.documentElement.style.setProperty('--entity-execution-color', preferences.entityExecutionColor);
      ...
      if (prefPayload && prefPayload.preferences) {
        ...
        if (bodyClass) {
          const entityType = bodyClass.split(' ').find(cls => 
            ['tickers-page', 'trades-page', 'accounts-page', 'trading_accounts-page', 'alerts-page', 'cash_flows-page', 'notes-page', 'executions-page', 'trade_plans-page', 'preferences-page'].includes(cls)
          );
          ...
          if (entityType) {
            ...
            else if (entity === 'cash_flows') entity = 'cash_flow';
            else if (entity === 'trade_plans') entity = 'trade_plan';
            else if (entity === 'trading_accounts' || entity === 'accounts') entity = 'trading_account';
    ...
    try {
      ...
      requestAnimationFrame(() => {
        ...
        try {
          ...
          // ITCSS Compliance: CSS variables are now defined in _global-settings.css
          // No inline styles needed - all colors are defaults in CSS
          ...
          // Removed setProperty calls - ITCSS compliance
          ...
          // ITCSS Compliance: Removed setProperty calls - all numeric colors are defaults in CSS
          ...
          const entityMap = {
            ...
            trade_plan: getEntityColorFromPrefs('entityTradePlanColor', '--entity-trade_plan-color'),
          ...
          // ITCSS Compliance: Removed setProperty calls for entity colors - all defined in CSS
          ...
          const ensureVar = (target, value) => {
            if (!getVar(target) && value) {
              // ITCSS Compliance: Removed setProperty call - all synonyms are defaults in CSS
            }
          };
