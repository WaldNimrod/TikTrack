...
class ConstraintsMonitor {
    ...
    async loadData() {
        try {
            // #region agent log - API integration marker: constraints LIST
            fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({
                location: 'constraints.js:loadConstraintsData:list_api_call',
                message: 'API integration marker: constraints LIST operation initiated',
                data: {
                  timestamp: Date.now(),
                  page: window.location.pathname,
                  function: 'loadConstraintsData',
                  endpoint: `${this.apiBase}/`,
                  method: 'GET',
                  operation: 'LIST'
                },
                sessionId: 'batch_e_api_integration',
            ...
            // #region agent log - API integration marker: constraints tables LIST
            fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({
                location: 'constraints.js:loadConstraintsData:tables_api_call',
                message: 'API integration marker: constraints tables LIST operation initiated',
                data: {
                  timestamp: Date.now(),
                  page: window.location.pathname,
                  function: 'loadConstraintsData',
                  endpoint: `${this.apiBase}/tables`,
                  method: 'GET',
                  operation: 'LIST'
                },
                sessionId: 'batch_e_api_integration',
            ...
            // Clear all loaders after data load (P0 fix)
            this.clearAllLoaders();
        }
    }

    /**
     * Clear all loading indicators (P0 fix for stuck loaders)
     */
    clearAllLoaders() {
        try {
            // Clear spinner-border elements
            document.querySelectorAll('.spinner-border, .spinner-grow, [class*="spinner"]').forEach(el => {
                const text = el.textContent || '';
                if (text.includes('טוען') || text.includes('בודק')) {
            ...
            document.querySelectorAll('*').forEach(el => {
                const text = el.textContent || '';
                if ((text.includes('טוען') || text.includes('בודק')) && (el.classList.contains('spinner-border') || el.classList.contains('spinner-grow'))) {
                    el.classList.add('d-none');
                    el.style.display = 'none';
                    el.hidden = true;
                }
            ...
            // Clear validation modal spinner if exists
            const validationModal = document.getElementById('validationModal');
            if (validationModal) {
                const spinner = validationModal.querySelector('.spinner-border');
                if (spinner && spinner.textContent && spinner.textContent.includes('בודק')) {
                    spinner.remove();
                }
            }
            
            // Mark cleanup as complete (for validation script)
            window.__constraintsCleanupDone = true;
            
            // Also set DOM attribute for alternative detection
            const pageContainer = document.getElementById('constraints') || document.body;
            if (pageContainer) {
                pageContainer.setAttribute('data-loaders-cleared', 'true');
            }
        } catch (error) {
            window.Logger?.error('Error clearing constraints loaders:', error);
        }
    }
...
async function validateConstraintData(constraint) {
    ...
    try {
        ...
        // #region agent log - API integration marker: constraints definition READ
        fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            location: 'constraints.js:validateConstraintData:definition_api_call',
            message: 'API integration marker: constraints definition READ operation initiated',
            data: {
              timestamp: Date.now(),
              page: window.location.pathname,
              function: 'validateConstraintData',
              endpoint: `/api/constraints/definition/${constraint.constraint_name}`,
              method: 'GET',
              operation: 'READ',
 ...
 */
window.initializeConstraintsPage = async function() {
    window.Logger?.debug('🚀 [initializeConstraintsPage] Starting constraints page initialization');

    try {
        // Initialize constraints monitor
        if (window.initializeConstraints) {
            window.initializeConstraints();
        }

        // Load data to trigger API integration markers
        if (window.constraintsMonitor && typeof window.constraintsMonitor.loadData === 'function') {
            await window.constraintsMonitor.loadData();
        }

        // Mark as initialized
        if (window.Logger) {
            window.Logger.info('Constraints page initialized successfully', { page: 'constraints' });
        }

    } catch (error) {
        window.Logger?.error('Error initializing constraints page', error, { page: 'constraints' });
        if (typeof window.showErrorNotification === 'function') {
            window.showErrorNotification('שגיאה באתחול עמוד אילוצים', error.message);
        }
    }
};
