...
class FieldRendererService {
    ...
    static renderLinkedEntity(relatedType, relatedId, displayName = '', meta = null) {
        ...
        if (renderMode === 'linked_items-table') {
            const escapedLabel = this._escapeHtml(label);
            const escapedText = this._escapeHtml(text);
            const entityColorClass = `entity-${type}`;
            return `
            <a href="#" class="linked_items-table-link text-decoration-none d-flex align-items-center gap-2" role="link" tabindex="0" ${onclick} data-entity-type="${type}" data-entity-id="${safeId}">
                <span class="linked_items-table-icon ${entityColorClass}">
                    <img src="${iconPath}" alt="${escapedLabel}" class="linked_items-table-icon-image" />
                </span>
                <span class="linked_items-table-text d-flex flex-column align-items-start gap-1">
                    <span class="linked_items-table-label fw-semibold text-body">${escapedLabel}</span>
                    <span class="linked_items-table-name text-muted small">${escapedText}</span>
                </span>
            </a>`;
        }
