...
const tradePlansModalConfig = {
    ...
    fields: [
        ...
        // שורה שישית: Stop Loss + Take Profit
        {
            ...
            rowClass: 'row',
            colClass: 'col-md-6 col-sm-12',
            labelClass: 'label-numeric-negative'
        },
        {
            type: 'number',
            ...
            rowClass: 'row',
            colClass: 'col-md-6 col-sm-12',
            labelClass: 'label-numeric-positive'
        },
        {
            type: 'number',
        ...
        },
        {
            ...
            html: `
                <div class="conditions-control-wrapper d-flex flex-column gap-2" data-conditions-controls="trade_plan">
...
function initializeTradePlansModal() {
    if (window.ModalManagerV2 && typeof window.ModalManagerV2.createCRUDModal === 'function') {
        try {
            ...
            window.Logger?.debug?.('✅ Trade Plans modal created successfully', { page: 'trade_plans-config' });
            return true;
        } catch (error) {
            window.Logger?.error?.('❌ Error creating Trade Plans modal:', error, { page: 'trade_plans-config' });
            return false;
        }
    }
    return false;
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            setTimeout(() => {
                if (!initializeTradePlansModal()) {
                    window.Logger?.debug?.('⚠️ ModalManagerV2 not available after DOMContentLoaded - will retry', { page: 'trade_plans-config' });
                    // Final retry after additional delay
                    setTimeout(() => {
                        if (!initializeTradePlansModal()) {
                            window.Logger?.debug?.('⚠️ ModalManagerV2 not available for Trade Plans modal after retries', { page: 'trade_plans-config' });
                        }
                    }, 500);
                }
            }, 100);
