...
(function() {
  ...
  if (summaryStats) {
    ...
    if (toggleBtn) {
      issues.push('❌ Toggle button found in summaryStats (should be in portfolioSummaryContent first row)');
    }
  } else {
  ...
  if (portfolioSummary) {
    const firstRow = portfolioSummary.querySelector('.portfolio-summary__stats-row--first');
    const secondRow = portfolioSummary.querySelector('.portfolio-summary__stats-row--second');
    const toggleBtn = portfolioSummary.querySelector('.portfolio-summary__toggle-btn');
    
    if (firstRow && toggleBtn && !firstRow.contains(toggleBtn)) {
      issues.push('❌ Toggle button not in first row');
    } else if (firstRow && toggleBtn) {
      console.log('  ✅ Toggle button in first row');
    }
    
    if (secondRow) {
      const display = window.getComputedStyle(secondRow).display;
      if (display === 'none') {
        console.log('  ✅ Second row hidden by default');
      }
    }
  }
