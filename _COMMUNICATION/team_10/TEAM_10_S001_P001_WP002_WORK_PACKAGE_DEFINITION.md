...
| **Deliverables** | (1) Folder structure under `agents_os/` per LLD400 §2.4 (runtime/, validators/, tests/, documentation or docs-governance extension as needed). (2) Minimal validator interface/skeleton (e.g. single entry point or stub for 10↔90 validation hook). (3) README or equivalent under `agents_os/` describing runtime layout and how it ties to the 10↔90 flow. (4) Evidence for QA (GATE_4) and for GATE_5/GATE_6 (path compliance, domain isolation verification). |
