...
| ⚠️ **לא בוצע במלואו** | **§4.5 Code Evidence** — ארבעת הפריטים (AppRouter, ProtectedRoute, HomePage, Header icon) — רשומים "לטפל"; **§4.7** בדיקות מקדימות — לא מסומנות; **/profile** לא הוחלט; **משימה 7** — Acceptance תלוי בוולידציה ויזואלית. |
...
- **Batch 1 Header:** תפריט רמה 2 RTL, גובה כפתורי רמה 2, header-container padding — בקשה רשמית מ־30 ל־40; מימוש זמני ב־phoenix-header.css; נדרש אישור ובעלות Team 40.
- **כלל:** תיקונים ויזואליים נוספים שמתגלים במהלך הבדיקה — עדיין לא מרוכזים במסמך אחד; מומלץ לרכז רשימת "פערים ויזואליים שנתגלו" ולשייך לצוותים (30/40) עד סגירה.
...
| קטגוריה | פריטים |
|---------|--------|
| **משימות פתוחות (מטריצה)** | T10.1, T10.2, T10.4; T30.1 (Broker D21?), T30.6; T90.1, T90.2. |
| **שלבים / פריטים לא סגורים (תוכנית)** | §4.5 Code Evidence (4 פריטים); §4.7 בדיקות מקדימות; החלטת /profile; וידוא שלב 0 (0.4, 0.5, 0.6); משימה 7 — וולידציה סופית. |
| **תוכניות — חוסרים** | Visual Gaps Work Plan: Code Evidence + בדיקות מקדימות + /profile + כל סטייה ויזואלית שתצא משער ג'. |
| **תהליך ויזואלי** | תיקוני Header (Batch 1) ויתר פערים — לרוכז ולוודא סגירה עם 30/40. |
