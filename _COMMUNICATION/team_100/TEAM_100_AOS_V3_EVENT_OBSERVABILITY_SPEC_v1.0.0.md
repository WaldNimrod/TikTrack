---
id: TEAM_100_AOS_V3_EVENT_OBSERVABILITY_SPEC_v1.0.0
...
# AOS v3 — Event & Observability Spec (Stage 7) — v1.0.0
...
```json
{
  ...
  "failure_reason": "NO_MATCHING_RULE | NO_ACTIVE_ASSIGNMENT"
}
```
**Note:** `reason` base column = `failure_reason` value. Error code `ROUTING_UNRESOLVED` raised to caller.
...
Rules:
  ...
  - limit clamped to range [1, 200]; values outside raise INVALID_HISTORY_PARAMS
  ...
  - event_type filter validated against §1 registry; unknown → INVALID_HISTORY_PARAMS
...
| `ROUTING_UNRESOLVED` | UC-01, UC-08 | 500 | SYSTEM | No routing rule matched or no active assignment; auto-escalates to team_00 | No |
...
| `INSUFFICIENT_AUTHORITY` | UC-04 | 403 | APPLICATION | Role lacks blocking authority (no `gate_role_authorities` row); non-blocking path → UC-05 | No |
...
| `NOT_PRINCIPAL` | UC-06/07/08/12 | 403 | APPLICATION | Operation requires team_00 (D-03); requesting actor is not Principal | No |
...
| `VARIANT_IMMUTABLE` | Stage 5 §EC | 409 | APPLICATION | Attempt to change `runs.process_variant` after initiation | No |
...
**Total unique error codes: 33** (30 from UC Catalog + 1 from Routing + 0 new from Prompt Arch [already in UC or reused] + 3 from Stage 7). Note: `ROUTING_UNRESOLVED` appears in both UC-01 and Routing Spec — counted once.
...
- [x] Routing Spec error codes included
- [x] Prompt Arch error codes included
...
**log_entry | TEAM_100 | AOS_V3_EVENT_OBSERVABILITY_SPEC | v1.0.0 | SUBMITTED_FOR_REVIEW | 2026-03-26**
