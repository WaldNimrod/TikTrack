...
# AOS v3 — State Machine Spec (Stage 2)
...
| T06 | `IN_PROGRESS` | `approve()` **[HITL gate]** | `team_00` (Principal) | (G04) `current_gate.is_human_gate=1`; actor = `team_00` | `IN_PROGRESS` | (A02) קידום ל-phase/gate הבא; עדכון pipeline_state.json | `GATE_APPROVED` |
| T07 | `IN_PROGRESS` | `pause()` | `team_00` | (G05) `run.status = IN_PROGRESS` | `PAUSED` | (A06) כתיבת `paused_routing_snapshot_json` **באותה טרנזקציה**; `paused_at=now()`; עדכון pipeline_state.json | `RUN_PAUSED` |
| T08 | `PAUSED` | `resume()` | `team_00` | (G06) `paused_routing_snapshot_json IS NOT NULL` | `IN_PROGRESS` | (A07) שחזור `current_gate_id` + `current_phase_id` ממצב קפוא (ללא שינוי); `paused_at=NULL`; routing מ-snapshot אלא אם קיים `TEAM_ASSIGNMENT_CHANGED` event; עדכון pipeline_state.json | `RUN_RESUMED` |
...
| T11 | `CORRECTION` | `pass()` | `current_team` | (G02) — correction resolved, gate approved | `IN_PROGRESS` | (A02) קידום phase; `correction_cycle_count` נשמר (לא מאופס — audit trail) | `CORRECTION_RESOLVED` |
| T12 | ANY non-terminal | `principal_override(action, reason)` | `team_00` | (G09) actor = `team_00`; action ∈ {FORCE_PASS, FORCE_FAIL, FORCE_PAUSE, FORCE_RESUME, FORCE_CORRECTION} | לפי action | (A10) בצע action; רישום חובה ב-Event עם `reason` | `PRINCIPAL_OVERRIDE` |
...
| **G04** | `gates.is_human_gate=1 WHERE id=:current_gate_id` AND `:actor = 'team_00'` |
| **G05** | `runs.status = 'IN_PROGRESS' WHERE id=:run_id` |
| **G06** | `runs.paused_routing_snapshot_json IS NOT NULL WHERE id=:run_id` |
| **G07** | `runs.correction_cycle_count < (SELECT value FROM policies WHERE key='max_correction_cycles')` |
| **G08** | `runs.correction_cycle_count >= (SELECT value FROM policies WHERE key='max_correction_cycles')` |
| **G09** | `:actor = 'team_00'` (ללא תנאים נוספים — Principal authority) |
...
| **A07** | `UPDATE runs SET status='IN_PROGRESS', paused_at=NULL WHERE id=:run_id`; routing resolution: use `paused_routing_snapshot_json` אלא אם `EXISTS (SELECT 1 FROM events WHERE run_id=:run_id AND event_type='TEAM_ASSIGNMENT_CHANGED' AND created_at > :paused_at)` — במקרה זה: re-resolve via current Assignment |
...
| **A10** | בצע Action המתאים + `INSERT INTO events (event_type='PRINCIPAL_OVERRIDE', actor_id='team_00', payload={action, reason})` |
...
**כלל resume:** `paused_routing_snapshot_json.assignments[role_id]` → `team_id` — אלא אם `TEAM_ASSIGNMENT_CHANGED` event קיים עם `created_at > paused_at`. במקרה זה: re-resolve מ-current Assignment table.
...
5. **HITL gates = team_00 בלבד.** `approve()` transition חוקי אך ורק כאשר `actor = 'team_00'`.
...
9. **correction_cycle_count לא מאופס.** Counter נשמר לכל אורך חיי ה-Run — audit trail. לא מאופס בין correction cycles.
...
| EC-02 | `pause()` כאשר Run כבר PAUSED | G05 — `status=IN_PROGRESS` | Guard fails → error code `INVALID_STATE_TRANSITION`. ללא שינוי. |
| EC-03 | `resume()` כאשר `paused_routing_snapshot_json IS NULL` | G06 | Guard fails → error code `SNAPSHOT_MISSING`. דרוש Principal intervention (A10). |
...
| EC-11 | `principal_override(FORCE_PASS)` בזמן CORRECTION | G09 | T12: pass forced, event `PRINCIPAL_OVERRIDE` עם reason. `correction_cycle_count` נשמר. |
...
| `PRINCIPAL_OVERRIDE` | team_00 | T12 |
...
- `UC-PrincipalOverride` — T12

---

**log_entry | TEAM_100 | STATE_MACHINE_SPEC | v1.0.0 | SUBMITTED_FOR_REVIEW | 2026-03-26**
