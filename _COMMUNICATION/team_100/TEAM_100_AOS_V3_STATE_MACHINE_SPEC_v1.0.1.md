---
id: TEAM_100_AOS_V3_STATE_MACHINE_SPEC_v1.0.1
...
supersedes: TEAM_100_AOS_V3_STATE_MACHINE_SPEC_v1.0.0.md
revision_basis: TEAM_190_AOS_V3_STATE_MACHINE_SPEC_REVIEW_v1.0.0.md---

# AOS v3 — State Machine Spec (Stage 2) — v1.0.1
...
`:actor='team_00'` מייצג resolution לשורת DB ב-`teams` table:
```sql
SELECT id FROM teams WHERE team_code = 'team_00'
```
Pipeline engine חייב לאמת קיום שורה זו ב-boot. כל FK המייצג `actor_id='team_00'` (ב-events, assignments) חייב להצביע לשורה זו.
...
כל A10* חייב לסיים ב: `INSERT INTO events (event_type='PRINCIPAL_OVERRIDE', actor_id=teams.id WHERE team_code='team_00', payload={action, from_state, reason, timestamp})`.
