---
id: TEAM_100_AOS_V3_UI_SPEC_AMENDMENT_v1.0.2
...
# AOS v3 — UI Spec Amendment (Stage 8A) — v1.0.2
...
- `is_current_actor`: computed — matches `actor.team_id` from current active run
...
| `NOT_PRINCIPAL` | 403 | Caller is not team_00 and request includes `status` field |

**Authorization policy (AD-S8A-04):** Status transitions on ideas are restricted to `team_00` only. If a non-team_00 caller includes a `status` field in the request body, the **entire request is rejected** with `NOT_PRINCIPAL` (403). This is whole-request rejection, not partial application — the API does not silently strip unauthorized fields. Any team may update non-status fields (title, description, priority, decision_notes, target_program_id) without restriction.
...
| `NOT_PRINCIPAL` | 403 | Ideas | §4.18 | Caller is not team_00; request includes restricted field (status) |
...
- `NOT_PRINCIPAL` is a new authorization code consistent with the `INSUFFICIENT_AUTHORITY` / `NOT_HITL_GATE` family but specific to the team_00-only administrative pattern (AD-S8A-04).
...
| **AD-S8A-04** | PUT /api/ideas with unauthorized `status` field → whole-request rejection (NOT_PRINCIPAL 403). No partial application. | §4.18 | Deterministic contract — API never silently strips fields. Caller knows exactly whether update succeeded or was entirely rejected. |
...
- [x] §9: 3 new error codes — dedicated semantics, no drift from Stage 7 registry
