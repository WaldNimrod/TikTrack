---
id: TEAM_100_AOS_V3_USE_CASE_CATALOG_v1.0.4
...
supersedes: TEAM_100_AOS_V3_USE_CASE_CATALOG_v1.0.2.md
revision_basis: TEAM_190_AOS_V3_DDL_SPEC_REVIEW_v1.0.0.md---

# AOS v3 — Use Case Catalog (Stage 3) — v1.0.3

## Changelog v1.0.2 → v1.0.3 (Team 190 Stage 4 — F-01)

| ID | Severity | Fix Applied |
|---|---|---|
| F-01 | MAJOR | G07/G08 (UC-09, UC-10): `policy_key` + `policy_value_json` + חילוץ `max` (PostgreSQL) — יישור ל־DDL Stage 4 ול־State Machine v1.0.2. |
...
| `INSUFFICIENT_AUTHORITY` | `actor_team_id ≠ team_00` (Tier 1 — AUTHORITY_MODEL v1.0.0 §3) | 403 | HITL requires Principal authority |
| `INVALID_STATE` | `runs.status ≠ 'IN_PROGRESS'` | 409 | |
...
| `INSUFFICIENT_AUTHORITY` | G05 actor fails (Tier 1 — AUTHORITY_MODEL v1.0.0 §3) | 403 | |
...
| `INSUFFICIENT_AUTHORITY` | G06 check 2 fails (Tier 1 — AUTHORITY_MODEL v1.0.0 §3) | 403 | |
...
2. (G07) `runs.correction_cycle_count < (SELECT (policy_value_json::jsonb->>'max')::integer FROM policies WHERE policy_key = 'max_correction_cycles' AND scope_type = 'GLOBAL' ORDER BY priority DESC LIMIT 1)`
...
**Preconditions:**
1. `runs.status = 'CORRECTION'`
2. (G08) `runs.correction_cycle_count >= (SELECT (policy_value_json::jsonb->>'max')::integer FROM policies WHERE policy_key = 'max_correction_cycles' AND scope_type = 'GLOBAL' ORDER BY priority DESC LIMIT 1)`
...
| Code | Trigger | HTTP | Description |
|---|---|---|---|
| `INSUFFICIENT_AUTHORITY` | G09 actor fails (Tier 1 — AUTHORITY_MODEL v1.0.0 §3) | 403 | |
...
**log_entry | TEAM_100 | USE_CASE_CATALOG | v1.0.3 | POLICY_KEY_VALUE_JSON_ALIGNMENT | 2026-03-26**
**log_entry | TEAM_100 | USE_CASE_CATALOG | v1.0.4 | AUTHORITY_MODEL | NOT_PRINCIPAL_REPLACED_x4 | 2026-03-28**
