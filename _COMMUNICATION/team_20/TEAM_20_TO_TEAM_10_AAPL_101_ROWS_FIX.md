...
### 2.2 Yahoo — User-Agent + Retry (SPEC-PROV-YF-HIST)

| עובדה | הסבר |
|------|------|
| **EOD זמין 24/7** | מחיר היסטורי זמין מלא גם בשוק סגור/סוף שבוע — **אין דילוג** |
| **User-Agent** | `yf.Ticker(symbol)` — עם session + User-Agent (חובה) |
| **Retry** | 3 ניסיונות, 5 שניות ביניהם — תחזוקת שרתים בסופי שבוע |
| **תוצאה** | Yahoo תומך ב־`start`/`end` — יכול להחזיר 250+ ימים |
