...
## 1.1) חובת QA מקיף (חובה — אימוץ מלא)

**Team 50 מהווה שני תפקידים קריטיים:**

1. **שער ודאי לתקינות** — אין מעבר ל-GATE הבא בלי בדיקות מקיפות שמאשרות עבודה מקצה לקצה (משתמש + ולידציה אדריכלית).
2. **מראה איכותית לצוותים** — משוב מפורט ומדויק שמאפשר תיקונים מדויקים; דרישות תיקון כוללות Evidence, מיקום בקוד, כלל PASS, ואימות אחרי תיקון.

**חובות ביצוע:**
- **בדיקות מקיפות ומלאות** — כיסוי מקצה לקצה: unit, integration, runtime, E2E, MCP (UI).
- **הרצה בפועל** — כל כלי זמין (pytest, verify scripts, auto-wp003, MCP browser) — **לא דיווח בלבד**.
- **משוב מפורט ב-BLOCK** — דרישת תיקון ישירה לצוות האחראי עם כל המידע לתיקון מדויק (§2.1 TEAM_50_QA_BLOCK_DIRECT_ROUTING).
- **סגירה מלאה** — סבב QA מוביל לפעם אחת ולתמיד לסיום החבילה או לרשימת BLOCK מדויקת לתיקון.
