> ⚠️ **SUPERSEDED — 2026-07-23.** מסמך זה הוחלף ע"י **`TEAM_61_TO_TEAM_110_TIKTRACK_SCHEMA_PROVISIONING_CLARIFICATION_v1.1.0.md`**. בעקבות בדיקה מעמיקה נוספת (בעקבות הערת ה-Principal) התברר כי הממצאים כאן ייחסו בטעות ארטיפקט של כלי-dev (`create_all`) כחוב ארכיטקטוני. **הבעיות אינן קיימות בסביבה המקומית/סטייג'ינג/פרודקשן** — ראה את המסמך המעודכן. אין לפעול לפי מסמך זה.
