...
def main():
    ...
    # Cleanup backup files after sync
    print()
    print("=" * 60)
    print("Cleaning up backup files...")
    print("=" * 60)
    cleanup_script = Path(__file__).parent / "cleanup_production_backups.sh"
    if cleanup_script.exists():
        result = subprocess.run(['bash', str(cleanup_script)], capture_output=True, text=True)
        if result.returncode == 0:
            print("✅ Backup files cleaned up")
        else:
            print("⚠️  Backup cleanup had issues (non-critical)")
    else:
        # Manual cleanup
            ...
            for pattern in ['*.backup', '*.bak', '*backup*', '*old*']:
                for file_path in production_dir.rglob(pattern):
                    if file_path.is_file():
                        ...
                        removed_count += 1
            if removed_count > 0:
                print(f"✅ Cleaned up {removed_count} backup files")
