...
def run_step(dry_run: bool = False, skip_ui_tests: bool = False) -> dict:
    ...
    try:
        ...
        # Summary - UI tests are critical, others are warnings
        critical_passed = validation_results.get('ui_tests', True)
        if critical_passed:
            logger.info("  ✅ Critical validations passed")
        else:
            logger.warning("  ⚠️  Critical validations failed")
        
        # Report warnings for non-critical failures
        failed = [k for k, v in validation_results.items() if not v and k != 'ui_tests']
        if failed:
            logger.warning(f"  ⚠️  Some validations had issues: {', '.join(failed)}")
            reporter.add_warning(f"Some validations had issues: {', '.join(failed)}", "validate")
        
        # Success if critical tests pass (UI tests)
        return {
