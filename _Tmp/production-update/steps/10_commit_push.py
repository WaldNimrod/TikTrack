...
def run_step(dry_run: bool = False, commit_message: str = None) -> dict:
    ...
    try:
        ...
        for vfile in version_files:
            vpath = project_root / vfile
            if vpath.exists():
                logger.info(f"  📝 Staging version file: {vfile}")
                subprocess.run(
                    ['git', 'add', str(vpath)],
                    cwd=project_root,
                    capture_output=True
        ...
        if commit_message is None:
            commit_message = f"chore: production release 1.2.0.0 - {datetime.now().strftime('%Y-%m-%d')}"
        
        logger.info(f"  💾 Committing changes...")
        logger.info(f"    Message: {commit_message}")
        
        # Use SKIP_VERSION_CHECK since we already bumped version
        env = os.environ.copy()
