---
document_id: PIPELINE_DASHBOARD_ARCHITECTURE_v1.1.0
...
supersedes: PIPELINE_DASHBOARD_ARCHITECTURE_v1.0.0 (2026-03-16)
---

# AOS Pipeline Dashboard — Architecture Reference
## agents_os/ui/docs/PIPELINE_DASHBOARD_ARCHITECTURE_v1.1.0.md
...
| Constitution | `PIPELINE_CONSTITUTION.html` | Canonical process map and governance view: flow, ownership model, accountability chain, expected outputs |
| Live Monitor | `PIPELINE_MONITOR.html` | Read-only dual-domain runtime monitor: active programs, live state, gate matrix, events, alerts |
...
│   ├── PIPELINE_CONSTITUTION.html       ← Constitution page
│   ├── PIPELINE_MONITOR.html            ← Live monitor page
│   ├── css/
...
│   │   └── pipeline-monitor.css         ← Monitor page styles
│   ├── js/
...
│   │   ├── pipeline-monitor-core.js     ← Shared monitor/constitution read-only logic
│   │   ├── pipeline-monitor-live.js     ← Live monitor mode selector
│   │   ├── pipeline-monitor-constitution.js ← Constitution mode selector
│   │   └── pipeline-monitor.js          ← Deprecated compatibility shim
│   └── docs/
│       ├── PIPELINE_DASHBOARD_ARCHITECTURE_v1.0.0.md     ← Historical architecture snapshot
│       ├── PIPELINE_DASHBOARD_ARCHITECTURE_v1.1.0.md     ← This document
...
│       ├── PIPELINE_MONITOR_ARCHITECTURE_v1.0.0.md       ← Historical monitor architecture
│       └── PIPELINE_MONITOR_ARCHITECTURE_v1.1.0.md       ← Active split-page monitor/constitution architecture
├── scripts/
...
### Constitution page (`PIPELINE_CONSTITUTION.html`) + Live Monitor page (`PIPELINE_MONITOR.html`)

```
pipeline-config.js                 → DOMAIN_STATE_FILES, resolveCanonicalGate(), getExpectedTeamForPhase()
pipeline-state.js                  → fetchJSON(), fetchText(), domain file readers
pipeline-dom.js                    → gateStatus(), statusPillClass(), statusLabel()
pipeline-monitor-core.js           → shared read-only rendering and data orchestration
pipeline-monitor-constitution.js   → constitution mode selector (structure/governance containers)
pipeline-monitor-live.js           → live mode selector (runtime containers)
```
...
**log_entry | TEAM_00 | PIPELINE_DASHBOARD_ARCHITECTURE_v1.1.0 | UPDATED_MONITOR_SPLIT_AND_SCRIPT_SPLIT | 2026-03-20**
