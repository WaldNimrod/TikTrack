...
from sqlalchemy import String, Boolean, Integer, Text, ForeignKey, CheckConstraint, Index, TIMESTAMP
...
class User(Base):
    ...
    role: Mapped[UserRole] = mapped_column(
        user_role_enum, nullable=False, default=UserRole.USER, server_default="USER"
    )
...
class UserApiKey(Base):
    ...
    created_by: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("user_data.users.id"), nullable=False
    )
    ...
    # Metadata (renamed from 'metadata' - reserved name in SQLAlchemy)
    api_key_metadata: Mapped[dict] = mapped_column(
        "metadata", JSONB, default=dict, server_default="{}"
    )
