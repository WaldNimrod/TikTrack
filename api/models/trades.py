...
from sqlalchemy import (
    String, Integer, Text, ForeignKey, Numeric, CheckConstraint, TIMESTAMP
)
...
class Trade(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "parent_trade_id IS NULL OR parent_trade_id != id",
            name="trades_not_self_parent"
        ),
        {"schema": "user_data"},
    )
    ...
    parent_trade_id: Mapped[Optional[uuid.UUID]] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("user_data.trades.id", ondelete="SET NULL"),
        nullable=True
    )
