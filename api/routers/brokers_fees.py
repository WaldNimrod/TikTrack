...
async def get_brokers_fees(
    trading_account_id: Optional[str] = Query(
        None, description="Filter by trading account ULID (ADR-015)"
    ),
    broker: Optional[str] = Query(None, description="Filter by broker name (via account)"),
    commission_type: Optional[str] = Query(
        None, description="Filter by commission type (TIERED/FLAT)"
    ),
    search: Optional[str] = Query(
        None, description="Search in account name, broker, commission value"
    ),
    ...
    try:
        ...
        return BrokerFeeListResponse(data=broker_fees, total=len(broker_fees))
...
async def get_brokers_fees_summary(
    trading_account_id: Optional[str] = Query(
        default=None, description="Filter by trading account ULID (ADR-015)"
    ),
    broker: Optional[str] = Query(default=None, description="Filter by broker name (via account)"),
    commission_type: Optional[str] = Query(
        default=None, description="Filter by commission type (TIERED/FLAT)"
    ),
...
async def get_broker_fee(
    ...
    try:
        ...
        broker_fee = await service.get_broker_fee_by_id(
            user_id=current_user.id, broker_fee_id=id, db=db
        )
        return broker_fee
    except HTTPExceptionWithCode:
        raise
...
async def delete_broker_fee(
    ...
    try:
        service = get_brokers_fees_service()
        await service.delete_broker_fee(user_id=current_user.id, broker_fee_id=id, db=db)
        return None
    except HTTPExceptionWithCode:
        raise
    except Exception as e:
