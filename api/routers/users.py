    ...
    try:
        ...
        try:
            ...
            if current_user.role in (UserRole.ADMIN, UserRole.SUPERADMIN):
                ...
                logger.warning(
                    f"Admin/SuperAdmin user {current_user.id} hit integrity error: {error_msg}"
                )
                ...
                if "email" in error_msg.lower() or "phone" in error_msg.lower():
                    logger.error(
                        f"Admin user hit email/phone constraint. DB schema may need update. Error: {error_msg}"
                    )
                    raise HTTPExceptionWithCode(
                ...
                or "idx_users_email" in error_msg
                or "users_email_key" in error_msg
                or "idx_users_email_unique_non_admin" in error_msg
            ):
                raise HTTPExceptionWithCode(
                    status_code=status.HTTP_409_CONFLICT,
                ...
                "phone" in error_msg.lower()
                or "idx_users_phone_unique" in error_msg
                or "users_phone_number_key" in error_msg
                or "idx_users_phone_unique_non_admin" in error_msg
            ):
                raise HTTPExceptionWithCode(
                    status_code=status.HTTP_409_CONFLICT,
                    detail="Phone number is already in use by another user",
