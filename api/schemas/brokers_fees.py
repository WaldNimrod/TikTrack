...
class BrokerFeeResponse(BaseModel):
    ...
    trading_account_id: str = Field(
        ..., description="Trading account ULID (fees belong to account)"
    )
...
class BrokerFeeCreateRequest(BaseModel):
    ...
    trading_account_id: str = Field(
        ..., description="Trading account ULID (fees belong to this account)"
    )
    commission_type: str = Field(..., description="Commission type (TIERED or FLAT)")
    commission_value: Decimal = Field(..., description="Commission value (numeric)", ge=0)
...
class BrokerFeeUpdateRequest(BaseModel):
    ...
    commission_value: Optional[Decimal] = Field(
        None, description="Commission value (numeric)", ge=0
    )
    minimum: Optional[Decimal] = Field(
        None, description="Minimum commission per transaction (USD)", ge=0
    )
...
class BrokerFeeSummaryResponse(BaseModel):
    ...
    avg_commission_per_trade: Decimal = Field(
        default=Decimal("0"), description="Average commission per trade"
    )
    monthly_fixed_commissions: Decimal = Field(
        default=Decimal("0"), description="Total monthly fixed commissions"
    )
    yearly_fixed_commissions: Decimal = Field(
        default=Decimal("0"), description="Total yearly fixed commissions"
    )
