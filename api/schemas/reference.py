...
class BrokerReferenceItem(BaseModel):
    ...
    label: str = Field(
        ..., max_length=100, description="Alias for display_name (D16/D18 backward compat)"
    )
    is_supported: bool = Field(..., description="True if broker supports import; 'other' is false")
    default_fees: List[DefaultFeeItem] = Field(
        default_factory=list, description="Suggested fees for prefill"
    )
...
class ExchangeRatesResponse(BaseModel):
    ...
    total: int = Field(..., description="Total count")
    staleness: str = Field(default="ok", description="ok | warning (>15min) | na (>1 trading day)")
