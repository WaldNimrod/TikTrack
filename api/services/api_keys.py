...
class ApiKeyService:
    ...
    async def create_api_key(
        self, user_id: uuid.UUID, data: UserApiKeyCreate, db: AsyncSession
    ) -> UserApiKey:
        """
    ...
    async def list_api_keys(self, user_id: uuid.UUID, db: AsyncSession) -> List[UserApiKey]:
        """
        ...
        stmt = (
            select(UserApiKey)
            .where(and_(UserApiKey.user_id == user_id, UserApiKey.deleted_at.is_(None)))
    ...
    async def get_api_key(
        self, key_id: uuid.UUID, user_id: uuid.UUID, db: AsyncSession
    ) -> Optional[UserApiKey]:
        """
    ...
    async def update_api_key(
        self, key_id: uuid.UUID, user_id: uuid.UUID, data: Dict[str, Any], db: AsyncSession
    ) -> UserApiKey:
        """
        ...
        if "api_secret" in data:
            api_key.api_secret_encrypted = self.encryption_service.encrypt_api_key(
                data["api_secret"]
            )
    ...
    async def delete_api_key(self, key_id: uuid.UUID, user_id: uuid.UUID, db: AsyncSession) -> None:
        """
    ...
    async def verify_api_key(self, key_id: uuid.UUID, user_id: uuid.UUID, db: AsyncSession) -> bool:
        """
        ...
        try:
            ...
            if api_key.api_secret_encrypted:
                decrypted_secret = self.encryption_service.decrypt_api_key(
                    api_key.api_secret_encrypted
                )
