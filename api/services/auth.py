...
from ..schemas.identity import LoginResponse, RegisterResponse, RefreshResponse, UserResponse
...
class AuthService:
    ...
    async def validate_access_token(self, token: str, db: AsyncSession) -> Dict[str, Any]:
        """
    ...
    async def validate_refresh_token(self, token: str, db: AsyncSession) -> UserRefreshToken:
        """
    ...
    async def login(self, username_or_email: str, password: str, db: AsyncSession) -> LoginResponse:
        """
        ...
        try:
            ...
            logger.info(
                f"User found: {user.username}, email: {user.email}, is_active: {user.is_active}"
            )
            ...
            try:
                ...
                logger.error(
                    f"Password verification error: {type(e).__name__}: {str(e)}", exc_info=True
                )
                raise AuthenticationError("Invalid credentials")
            ...
            if not password_valid:
                ...
                try:
                    ...
                    logger.error(
                        f"Failed to update login attempts: {type(e).__name__}: {str(e)}",
                        exc_info=True,
                    )
            ...
            try:
                ...
                logger.error(
                    f"Failed to update user login status: {type(e).__name__}: {str(e)}",
                    exc_info=True,
                )
            ...
            try:
                ...
                refresh_token, refresh_expires_at, refresh_jti, token_hash = (
            ...
            try:
                ...
                logger.error(
                    f"Failed to store refresh token: {type(e).__name__}: {str(e)}", exc_info=True
                )
            ...
            try:
                ...
                logger.error(
                    f"Failed to create user response: {type(e).__name__}: {str(e)}", exc_info=True
                )
    ...
    async def register(
        ...
        try:
            ...
            try:
                ...
                stmt = select(User).where(combined_condition).where(User.deleted_at.is_(None))
                ...
                logger.error(
                    f"Database query error during user check: {type(e).__name__}: {str(e)}",
                    exc_info=True,
                )
            ...
            # Log unexpected errors and re-raise as AuthenticationError
            logger.error(
                f"Unexpected error in register: {type(e).__name__}: {str(e)}", exc_info=True
            )
            raise AuthenticationError("Registration processing failed")

    async def refresh_access_token(self, refresh_token: str, db: AsyncSession) -> RefreshResponse:
        """
        ...
        # Create new tokens
        access_token, expires_at, access_jti = self.create_access_token(user)
        new_refresh_token, new_refresh_expires_at, new_refresh_jti, new_token_hash = (
            self.create_refresh_token(user)
        )
    ...
    async def logout(
        self, access_token: str, refresh_token: Optional[str], db: AsyncSession
    ) -> None:
        """
        ...
        if refresh_token:
            ...
            stmt = select(UserRefreshToken).where(
                and_(
                    UserRefreshToken.token_hash == token_hash, UserRefreshToken.revoked_at.is_(None)
                )
            )
            result = await db.execute(stmt)
    ...
    async def revoke_all_user_tokens(self, user_id: uuid.UUID, db: AsyncSession) -> int:
        ...
        """
        stmt = select(UserRefreshToken).where(
            and_(UserRefreshToken.user_id == user_id, UserRefreshToken.revoked_at.is_(None))
        )
        result = await db.execute(stmt)
