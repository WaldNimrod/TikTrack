...
class CashFlowService:
    ...
    async def get_cash_flows(
        ...
        """
        # Base query
        conditions = [CashFlow.user_id == user_id, CashFlow.deleted_at.is_(None)]
        ...
        stmt = (
            select(CashFlow, TradingAccount.account_name)
            .outerjoin(TradingAccount, CashFlow.trading_account_id == TradingAccount.id)
    ...
    async def get_cash_flows_summary(
        ...
        stmt = (
            select(CashFlow.flow_type, func.sum(CashFlow.amount).label("total"))
            .where(and_(*conditions))
        ...
        return CashFlowSummaryResponse(
            total_deposits=total_deposits, total_withdrawals=total_withdrawals, net_flow=net_flow
        )

    async def get_cash_flow_by_id(
        self, user_id: uuid.UUID, cash_flow_id: str, db: AsyncSession
    ) -> CashFlowResponse:
        """
        ...
        if not row:
            raise HTTPExceptionWithCode(
                status_code=404, detail="Cash flow not found", error_code=ErrorCodes.USER_NOT_FOUND
            )
    ...
    async def create_cash_flow(
        ...
        # Get account name
        stmt = select(TradingAccount.account_name).where(TradingAccount.id == account_uuid)
        result = await db.execute(stmt)
    ...
    async def update_cash_flow(
        ...
        stmt = select(CashFlow).where(
            and_(
                CashFlow.id == flow_uuid, CashFlow.user_id == user_id, CashFlow.deleted_at.is_(None)
            )
        )
