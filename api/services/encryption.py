...
class EncryptionService:
    ...
    def __init__(self, encryption_key: Optional[str] = None):
        ...
        self._fernet = Fernet(
            self._primary_key.encode() if isinstance(self._primary_key, str) else self._primary_key
        )
        self._legacy_fernets = [
            Fernet(key.encode() if isinstance(key, str) else key) for key in self._legacy_keys
        ]
    ...
    def decrypt_api_key(self, encrypted_key: str) -> str:
        ...
        for i, legacy_fernet in enumerate(self._legacy_fernets, start=1):
            try:
                ...
                logger.info(
                    f"Decrypted using legacy key {i}. Consider re-encrypting with primary key."
                )
                return decrypted_bytes.decode("utf-8")
            except Exception:
                continue
