...
class PasswordResetService:
    ...
    async def request_reset(
        ...
        if not user:
            ...
            logger.warning(f"Password reset requested for non-existent {method.value}: {identifier}")
        ...
        if method == ResetMethod.EMAIL:
            ...
            token_expires_at = datetime.utcnow() + timedelta(hours=self.EMAIL_TOKEN_EXPIRY_HOURS)  # Fallback
