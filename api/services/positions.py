...
class PositionService:
    ...
    async def get_positions(
        self, user_id: uuid.UUID, db: AsyncSession, trading_account_id: Optional[str] = None
        ...
        agg_stmt = (
            ...
            .group_by(Trade.ticker_id, Trade.trading_account_id, Trade.direction)
        )
        ...
        # Get ticker symbols
        ticker_stmt = select(Ticker.id, Ticker.symbol).where(
            and_(Ticker.id.in_(ticker_ids), Ticker.deleted_at.is_(None))
        )
        ...
        latest_price_subq = (
            select(
                TickerPrice.ticker_id, func.max(TickerPrice.price_timestamp).label("max_timestamp")
            )
        ...
        # Get account values
        account_stmt = select(TradingAccount.id, TradingAccount.cash_balance).where(
            and_(
        ...
        for pos in aggregated_positions:
            ...
            price_data = price_map.get(
                pos.ticker_id, {"current_price": Decimal("0"), "previous_close": Decimal("0")}
            )
