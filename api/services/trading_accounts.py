...
class TradingAccountService:
    ...
    async def get_trading_accounts(
        ...
        conditions = [TradingAccount.user_id == user_id, TradingAccount.deleted_at.is_(None)]
        ...
        for account in accounts:
            stats = trade_stats.get(
                account.id,
                {"positions_count": 0, "total_pl": Decimal("0"), "holdings_value": Decimal("0")},
            )
    ...
    async def get_trading_accounts_summary(
        self, user_id: uuid.UUID, db: AsyncSession, status: Optional[bool] = None
    ) -> TradingAccountSummaryResponse:
        """
        ...
        stmt_totals = select(
            func.coalesce(func.sum(TradingAccount.cash_balance), Decimal("0")).label(
                "total_cash_balance"
            )
    ...
    async def get_trading_account_by_id(
        self, user_id: uuid.UUID, trading_account_id: str, db: AsyncSession
    ) -> TradingAccountResponse:
        """
        ...
        stats = {
            ...
            "total_pl": (
                trade_stats_row.total_pl or Decimal("0") if trade_stats_row else Decimal("0")
            ),
            "holdings_value": (
                trade_stats_row.holdings_value or Decimal("0") if trade_stats_row else Decimal("0")
            ),
        }
    ...
    async def update_trading_account(
        ...
        if external_account_id is not None:
            account.external_account_id = (
                external_account_id.strip() if external_account_id else None
            )
