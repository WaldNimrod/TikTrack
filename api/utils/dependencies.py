...
async def get_current_user(
    ...
    try:
        ...
        payload = await auth_service.validate_access_token(token=credentials.credentials, db=db)
        ...
        try:
            ...
            logger.error(
                f"ULID to UUID conversion failed: {type(e).__name__}: {str(e)}", exc_info=True
            )
            raise HTTPExceptionWithCode(
                status_code=status.HTTP_401_UNAUTHORIZED,
