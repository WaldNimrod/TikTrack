...
```javascript
{
  ...
  // Price/Percentage System (Generated Columns - October 2025)
  current_price: number,        // Entry price for calculations
  stop_price: number,           // Stop loss price (source of truth)
  target_price: number,         // Target price (source of truth)
  stop_percentage: number,      // Auto-calculated by SQLite (READ-ONLY)
  target_percentage: number,    // Auto-calculated by SQLite (READ-ONLY)
...
### 🔢 Generated Columns Implementation (October 2025)

**Problem Solved**: Eliminated data redundancy between prices and percentages.

**Solution**: SQLite GENERATED ALWAYS columns
- `stop_percentage` and `target_percentage` are **calculated automatically**
- Formula: `((current_price - stop_price) / current_price) * 100`
- Always consistent - no possibility of mismatch
- SQL queries work perfectly (WHERE, ORDER BY)
- Zero maintenance required

**Important Notes**:
- ✅ Percentage columns are **read-only**
- ✅ Update `stop_price` or `target_price` → percentages update automatically
- ✅ Update `current_price` → both percentages recalculate
- ✅ Use helper methods in model: `calculate_price_from_percentage()` and `calculate_percentage_from_price()`

**Example**:
```python
# Creating a trade plan with prices
plan = TradePlan(
    current_price=100.0,
    stop_price=95.0,      # 5% below entry
    target_price=110.0    # 10% above entry
)
db.add(plan)
db.commit()
...
# Creating with percentage (convert to price first)
stop_price = TradePlan.calculate_price_from_percentage(100, 5, is_stop=True)   # 95.0
target_price = TradePlan.calculate_price_from_percentage(100, 10, is_stop=False)  # 110.0
...
### ✅ Generated Columns Implementation (October 9, 2025)

**Major Update**: Implemented SQLite Generated Columns for price/percentage calculations.

**What Changed**:
- `stop_percentage` and `target_percentage` are now **calculated automatically by SQLite**
- **Zero data redundancy** - prices are source of truth
- **Always consistent** - percentages auto-update when prices change
- **Server-side conversion** - API accepts both prices AND percentages
- Frontend can send either format - server handles conversion

**API Usage Examples**:
```javascript
// Example 1: Create with percentages
POST /api/trade_plans/
{
  "current_price": 100.0,
  "stop_percentage": 5,      // Server converts to stop_price: 95
  "target_percentage": 10,   // Server converts to target_price: 110
  ...
}
// Response includes both: stop_price=95, stop_percentage=5

// Example 2: Create with prices
POST /api/trade_plans/
{
  "current_price": 100.0,
  "stop_price": 95.0,        // SQLite calculates stop_percentage: 5
  "target_price": 110.0,     // SQLite calculates target_percentage: 10
  ...
}
// Response includes both: stop_price=95, stop_percentage=5

// Example 3: Update with percentage
PUT /api/trade_plans/123
{
  "stop_percentage": 8       // Server converts to stop_price: 92
}
// Response: stop_price=92, stop_percentage=8 (auto-calculated)
```

**Benefits**:
- ✅ Frontend flexibility - send price OR percentage
- ✅ Backend handles conversion automatically
- ✅ Database always consistent
- ✅ SQL queries work: `WHERE stop_percentage > 5`
- ✅ Zero maintenance required

---

### ✅ Validation System Improvements (August 31, 2025)
...
- [Validation System](../../frontend/VALIDATION_SYSTEM.md)
