...
def get_trades():
    ...
    try:
        ...
        if account_id and status:
            ...
            trades = query.filter(Trade.trading_account_id == account_id, Trade.status == status).all()
            logger.info(f"Found {len(trades)} trades for account {account_id} with status {status}")
        elif account_id:
            trades = query.filter(Trade.trading_account_id == account_id).all()
        elif status:
            trades = query.filter(Trade.status == status).all()
        else:
            trades = query.all()
        ...
        for trade in trades:
            if hasattr(trade, 'ticker') and trade.ticker:
                ...
                if latest_quote:
                    trade.ticker.current_price = latest_quote.price
                    trade.ticker.change_percent = latest_quote.change_pct_day
                    trade.ticker.change_amount = latest_quote.change_amount_day
                    logger.debug(f"Added market data to ticker {trade.ticker.symbol}: price={latest_quote.price}")
        ...
        for trade in trades:
            ...
            # Add market data from ticker if available (now added to ticker object above)
            if hasattr(trade, 'ticker') and trade.ticker:
                if hasattr(trade.ticker, 'current_price'):
                    trade_dict['current_price'] = trade.ticker.current_price
                    trade_dict['daily_change'] = getattr(trade.ticker, 'change_percent', None)
                    trade_dict['change_amount'] = getattr(trade.ticker, 'change_amount', None)
                else:
