...
async function saveCredentials(username, password, rememberMeId = 'rememberMe') {
  const rememberMe = document.getElementById(rememberMeId);
  if (rememberMe && rememberMe.checked) {
    if (window.UnifiedCacheManager && window.UnifiedCacheManager.isInitialized()) {
      await window.UnifiedCacheManager.save('savedUsername', username, { layer: 'localStorage', ttl: null });
      await window.UnifiedCacheManager.save('savedPassword', password, { layer: 'localStorage', ttl: null });
      await window.UnifiedCacheManager.save('rememberCredentials', true, { layer: 'localStorage', ttl: null });
    } else {
      localStorage.setItem('savedUsername', username); // fallback
      localStorage.setItem('savedPassword', password); // fallback
      localStorage.setItem('rememberCredentials', 'true'); // fallback
    }
  } else {
    if (window.UnifiedCacheManager && window.UnifiedCacheManager.isInitialized()) {
      await window.UnifiedCacheManager.remove('savedUsername', { layer: 'localStorage' });
      await window.UnifiedCacheManager.remove('savedPassword', { layer: 'localStorage' });
      await window.UnifiedCacheManager.remove('rememberCredentials', { layer: 'localStorage' });
    } else {
      localStorage.removeItem('savedUsername'); // fallback
      localStorage.removeItem('savedPassword'); // fallback
      localStorage.removeItem('rememberCredentials'); // fallback
    }
  }
}

async function loadSavedCredentials(usernameId = 'username', passwordId = 'password', rememberMeId = 'rememberMe') {
  let rememberCredentials = false;
  if (window.UnifiedCacheManager && window.UnifiedCacheManager.isInitialized()) {
    rememberCredentials = await window.UnifiedCacheManager.get('rememberCredentials') === true;
  } else {
    rememberCredentials = localStorage.getItem('rememberCredentials') === 'true'; // fallback
  }
  ...
  if (rememberCredentials) {
    ...
    if (window.UnifiedCacheManager && window.UnifiedCacheManager.isInitialized()) {
      savedUsername = await window.UnifiedCacheManager.get('savedUsername');
      savedPassword = await window.UnifiedCacheManager.get('savedPassword');
    } else {
      savedUsername = localStorage.getItem('savedUsername'); // fallback
      savedPassword = localStorage.getItem('savedPassword'); // fallback
    }
