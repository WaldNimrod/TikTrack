...
def clear_all_cache():
    """Clear all cache entries - includes advanced cache AND preferences cache"""
    try:
        ...
        logger.info(f"Cache cleared by API request - Advanced cache + {prefs_cleared} preferences")
        return jsonify({
            'status': 'success',
            'message': 'All caches cleared successfully',
            'data': {
                'advanced_cache': 'cleared',
                'preferences_cache': f'{prefs_cleared} entries'
            }
        }), 200
    except Exception as e:
