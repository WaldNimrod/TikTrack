...
class ProfileManager {
    ...
    async updateDropdown() {
        ...
        if (!select) {
            console.warn('⚠️ profileSelect element not found');
            return;
        }
        ...
        console.log(`📋 Updated dropdown with ${this.profiles.length} profiles`);
    }
        ...
        inputs.forEach(input => {
            if (input.id && !input.disabled) {
                ...
                const skipIds = [
                    ...
                    'profileSelect', 'newProfileName',  // Profile management
    ...
    _setupEventListeners() {
        ...
        if (profileSelect) {
            profileSelect.addEventListener('change', async (e) => {
                ...
                console.log(`🔄 Profile changed to: ${selectedProfileName}`);
                
                // Find profile by name
                const profile = this.profiles.profiles.find(p => p.name === selectedProfileName);
                
                if (profile && profile.id !== this.profiles.activeProfile?.id) {
                    await this.profiles.switchProfile(profile.id);
                    // Reload and apply to form after switch
                    this._applyToForm(this.manager.currentPreferences);
                    await this.colors.loadColors();
                    await this.ui.updateCounters(this.manager.currentPreferences);
                }
            });
            console.log('✅ Profile select listener attached');
        }
    }
}
