 ...
 */
async function restoreDesignsSectionState() {
  // Restoring designs section state

  try {
    let collapsed = false;
    if (window.UnifiedCacheManager && window.UnifiedCacheManager.isInitialized()) {
      collapsed = await window.UnifiedCacheManager.get('designsSectionCollapsed') === true;
    } else {
      collapsed = localStorage.getItem('designsSectionCollapsed') === 'true'; // fallback
    }
...
async function savePageState(pageName, state) {
  try {
    ...
    if (window.UnifiedCacheManager && window.UnifiedCacheManager.isInitialized()) {
      await window.UnifiedCacheManager.save(key, stateToSave, {
        layer: 'localStorage',
        ttl: 24 * 60 * 60 * 1000, // 24 hours
        syncToBackend: false
      });
    } else {
      localStorage.setItem(key, JSON.stringify(stateToSave)); // fallback
    }
...
async function loadPageState(pageName) {
  try {
    ...
    if (window.UnifiedCacheManager && window.UnifiedCacheManager.isInitialized()) {
      savedState = await window.UnifiedCacheManager.get(key);
    } else {
      savedState = localStorage.getItem(key); // fallback
    }
