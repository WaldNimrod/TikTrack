...
/**
 * פונקציות עזר למערכת מטמון מאוחדת
 */

/**
 * פונקציה עזר לקבלת ערך ממערכת מטמון מאוחדת
 * @param {string} key - המפתח
 * @param {string} layer - השכבה ('localStorage', 'indexedDB', 'memory', 'backend')
 * @returns {Promise<any>} - הערך
 */
async function getFromUnifiedCache(key, layer = 'localStorage') {
  if (window.UnifiedCacheManager && window.UnifiedCacheManager.isInitialized()) {
    try {
      return await window.UnifiedCacheManager.get(key, layer);
    } catch (error) {
      console.warn(`⚠️ שגיאה בטעינת ${key} ממערכת מטמון מאוחדת:`, error);
      // Fallback ל-localStorage ישיר
      return localStorage.getItem(key);
    }
  } else {
    console.warn('⚠️ מערכת מטמון מאוחדת לא זמינה, שימוש ב-localStorage ישיר');
    return localStorage.getItem(key);
  }
}

/**
 * פונקציה עזר לשמירת ערך במערכת מטמון מאוחדת
 * @param {string} key - המפתח
 * @param {any} value - הערך
 * @param {string} layer - השכבה ('localStorage', 'indexedDB', 'memory', 'backend')
 * @returns {Promise<boolean>} - הצלחה
 */
async function setToUnifiedCache(key, value, layer = 'localStorage') {
  if (window.UnifiedCacheManager && window.UnifiedCacheManager.isInitialized()) {
    try {
      await window.UnifiedCacheManager.set(key, value, layer);
      return true;
    } catch (error) {
      console.warn(`⚠️ שגיאה בשמירת ${key} במערכת מטמון מאוחדת:`, error);
      // Fallback ל-localStorage ישיר
...
// ייצוא פונקציות חשובות בתחילת הקובץ
window.getFromUnifiedCache = getFromUnifiedCache;
window.setToUnifiedCache = setToUnifiedCache;

// פונקציה זמנית עד שהיא תוגדר
window.loadTradingAccountsDataForTradingAccountsPage = function() {
  console.log('⚠️ loadTradingAccountsDataForTradingAccountsPage נקראה לפני שהוגדרה');
  console.log('⏰ מנסה שוב בעוד 2 שניות...');
  setTimeout(() => {
    // נסה לטעון נתונים ישירות
    if (typeof loadTradingAccountsData === 'function') {
      console.log('📡 טוען נתונים ישירות...');
      loadTradingAccountsData().then(trading_accounts => {
        if (typeof updateTradingAccountsTable === 'function') {
      ...
      console.error('❌ loadTradingAccountsData לא זמינה');
    }
  }, 2000);
};
...
async function loadCurrenciesFromServer() {
  try {
    // שימוש בפונקציית עזר למערכת מטמון מאוחדת
    const token = await getFromUnifiedCache('authToken', 'localStorage');
...
//   if (tradingAccount.currency_symbol) {
//     // אם יש סמל מטבע מהשרת
//     switch (tradingAccount.currency_symbol) {
//     case 'USD': return '$';
//     case 'ILS': return '₪';
//     case 'EUR': return '€';
//     case 'GBP': return '£';
//     default: return tradingAccount.currency_symbol;
//     }
//   } else if (tradingAccount.currency_id && window.currenciesData.length > 0) {
//     // אם יש רק currency_id, נחפש את המטבע
//     const currency = window.currenciesData.find(c => c.id === tradingAccount.currency_id);
//     if (currency) {
...
//       }
//     }
//   } else if (tradingAccount.currency && tradingAccount.currency.symbol) {
//     // fallback למטבע הישן
//     switch (tradingAccount.currency.symbol) {
//     case 'USD': return '$';
//     case 'ILS': return '₪';
//     case 'EUR': return '€';
//     case 'GBP': return '£';
...
async function loadTradingAccountsFromServer() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      console.log('📊 כל החשבונות המסחר:', allTradingAccounts.length, 'חשבונות מסחר');
      ...
      console.log('📊 חשבונות מסחר פתוחים:', openTradingAccounts.length, 'חשבונות מסחר');
      ...
      return []; // החזרת מערך ריק במקרה של תגובה לא תקינה
    }
...
async function loadTradingAccountsData() {
  ...
  try {
    ...
    if (typeof window.apiCall === 'function') {
      ...
      console.log('📊 חשבונות מסחר מ-apiCall:', trading_accounts.length, 'חשבונות מסחר');
      // חשבונות שהתקבלו
    ...
    handleDataLoadError(error, 'טעינת נתוני חשבונות מסחר');
    throw error;
  }
}

// פונקציה לעדכון סטטיסטיקות חשבונות מסחר
/**
 * עדכון סטטיסטיקות בראש העמוד
 * @param {Array} trading_accounts - מערך חשבונות מסחר
 */
function updateTradingAccountsStatistics(trading_accounts) {
  ...
  try {
    ...
    const openAccountsEl = document.getElementById('openAccounts');
    ...
    if (openAccountsEl) openAccountsEl.textContent = openAccounts;
...
function updateTradingAccountsTable(trading_accounts) {
  ...
  if (!tbody) {
    ...
    if (table) {
      ...
      if (tbody) {
        console.log('✅ נמצא tbody אלטרנטיבי:', tbody);
      } else {
        console.error('❌ לא נמצא tbody בטבלה');
        // נסה ליצור tbody אם לא קיים
        tbody = document.createElement('tbody');
        table.appendChild(tbody);
        console.log('✅ נוצר tbody חדש');
      }
    } else {
      console.error('❌ לא נמצאה הטבלה #accountsTable');
      console.log('🔍 כל הטבלאות בעמוד:', document.querySelectorAll('table'));
      return;
    }
  }

  // בניית הטבלה מחדש לפי הכותרות בדיוק
  console.log('📊 עדכון טבלת חשבונות מסחר עם', trading_accounts.length, 'חשבונות מסחר');
  ...
  tbody.innerHTML = trading_accounts.map(tradingAccount => {
    // שימוש במערכת מיפוי טבלאות במקום מיפוי ידני
    const tableType = 'accounts'; // מתאים ל-data-table-type בטבלה
    
    // בדיקה אם מערכת מיפוי טבלאות זמינה
    let nameValue, currencyValue, statusValue, cashBalanceValue, totalValueValue, totalPlValue, notesValue;
    
    if (window.getColumnValue && window.tableMappings) {
      // console.log('✅ משתמש במערכת מיפוי טבלאות');
      // שימוש במערכת מיפוי טבלאות
      ...
      cashBalanceValue = window.getColumnValue(tradingAccount, 4, tableType) || tradingAccount.cash_balance || 0;
      totalValueValue = window.getColumnValue(tradingAccount, 5, tableType) || tradingAccount.total_value || 0;
      totalPlValue = window.getColumnValue(tradingAccount, 6, tableType) || tradingAccount.total_pl || 0;
      notesValue = window.getColumnValue(tradingAccount, 7, tableType) || tradingAccount.notes || '-';
    } else {
      console.warn('⚠️ מערכת מיפוי טבלאות לא זמינה, שימוש במיפוי ידני');
      // Fallback - מיפוי ידני
      nameValue = tradingAccount.name || '-';
      currencyValue = tradingAccount.currency || '-';
      statusValue = tradingAccount.status || '-';
      cashBalanceValue = tradingAccount.cash_balance || 0;
      totalValueValue = tradingAccount.total_value || 0;
      totalPlValue = tradingAccount.total_pl || 0;
      notesValue = tradingAccount.notes || '-';
    }
    
    // המרת סטטוס לעברית לפילטר
    const statusForFilter = statusValue === 'open' ? 'פתוח' :
      statusValue === 'closed' ? 'סגור' :
        statusValue === 'cancelled' ? 'מבוטל' : statusValue || '-';

    return `
    <tr data-trading-account-id="${tradingAccount.id}">
      <td class="ticker-cell" data-tradingAccount="${nameValue}">
      ...
      <td class="status-cell" data-status="${statusForFilter}">
        <span class="status-${statusValue}-badge" 
      ...
      <td>
        <span class="numeric-value-positive" 
          ...
          $${cashBalanceValue ? cashBalanceValue.toLocaleString() : '0'}
        </span>
      </td>
      <td>
        <span class="numeric-value-positive" 
              style="padding: 2px 6px; border-radius: 4px; font-size: 0.9em; font-weight: 500;">
          $${totalValueValue ? totalValueValue.toLocaleString() : '0'}
        </span>
      </td>
      <td>
        <span class="${totalPlValue >= 0 ? 'numeric-value-positive' : 'numeric-value-negative'}" 
              style="padding: 2px 6px; border-radius: 4px; font-size: 0.9em; font-weight: 500;">
          ${totalPlValue ? `$${totalPlValue.toLocaleString()}` : '$0'}
        </span>
      </td>
      <td>${notesValue}</td>
      <td class="col-actions actions-cell actions-3-btn">
        ${window.createLinkButton(`window.showLinkedItemsModal && window.showLinkedItemsModal(linkedItemsData, 'tradingAccount', ${tradingAccount.id})`)}
        ${window.createEditButton(`showEditTradingAccountModalById(${tradingAccount.id})`)}
        ${tradingAccount.status === 'cancelled' ?
          window.createButton('REACTIVATE', `restoreTradingAccount(${tradingAccount.id}, '${tradingAccount.name || 'Unknown'}')`) :
          window.createButton('CANCEL', `cancelTradingAccountWithLinkedItemsCheck(${tradingAccount.id}, '${tradingAccount.name || 'Unknown'}')`)
        }
        ${window.createDeleteButton(`deleteTradingAccountWithLinkedItemsCheck(${tradingAccount.id}, '${tradingAccount.name || 'Unknown'}')`)}
      </td>
    </tr>
  `;}).join('');
  ...
  if (countElement) {
    countElement.textContent = `${trading_accounts.length} חשבונות מסחר`;
  }

  // עדכון סטטיסטיקות
  updateTradingAccountsStatistics(trading_accounts);

  console.log('✅ טבלת חשבונות מסחר עודכנה בהצלחה עם', trading_accounts.length, 'חשבונות מסחר');
...
async function loadTradingAccounts() {
  try {
    ...
    handleDataLoadError(error, 'טעינת חשבונות מסחר');
    const tbody = document.querySelector('.content-section:nth-child(2) tbody');
    if (tbody) {
      const errorText = `שגיאה בטעינת חשבונות מסחר: ${error.message}`;
...
function updateTradingAccountFilterDisplayText() {
  ...
  if (selectedTradingAccounts.length === 0) {
    selectedText.textContent = 'כל החשבונות המסחר';
...
window.updateTradingAccountFilterMenuDirectly = function (trading_accounts) {
  ...
  // הוספת אופציית "כל החשבונות המסחר"
  ...
  allTradingAccountsItem.innerHTML = `
    <span class="option-text">כל החשבונות המסחר</span>
          ...
          <form id="tradingAccountForm">
              ...
              <div class="col-md-6">
                <div class="mb-3">
                  ...
                  <input type="text" class="form-control" id="accountName" name="name" required 
                         value="${account ? tradingAccount.name : ''}" placeholder="הכנס שם חשבון (לפחות 3 תווים)" minlength="3" maxlength="18">
              ...
              <div class="col-md-6">
                <div class="mb-3">
                  ...
                  <select class="form-select" id="accountStatus" name="status">
                    <option value="open" ${account && tradingAccount.status === 'open' ? 'selected' : ''}>פתוח</option>
                    <option value="closed" ${account && tradingAccount.status === 'closed' ? 'selected' : ''}>סגור</option>
                  </select>
                </div>
              </div>
              <div class="col-md-6">
                <div class="mb-3">
                  ...
                  <input type="number" class="form-control" id="accountCashBalance" name="cash_balance" 
                         value="${account ? tradingAccount.cash_balance || 0 : 0}" placeholder="0" step="0.01" min="0">
            ...
            <div class="mb-3">
              ...
              <textarea class="form-control" id="accountNotes" name="notes" rows="3" 
                        placeholder="הכנס הערות על החשבון">${account ? tradingAccount.notes || '' : ''}</textarea>
            </div>
          </form>
        </div>
        <div class="modal-footer">
          <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">ביטול</button>
          <button type="button" class="btn btn-primary" onclick="saveTradingAccount('${mode}', ${account ? tradingAccount.id : 'null'})">
            ${buttonText}
          </button>
        </div>
      </div>
...
async function restoreTradingAccountsSectionState() {
  // שימוש בפונקציית עזר למערכת מטמון מאוחדת
  const savedState = await getFromUnifiedCache('trading_accountsSectionCollapsed', 'localStorage');
...
/**
 * פונקציות ניהול סקשנים - שימוש במערכת הכללית
 */
function toggleSection(sectionId) {
  console.log(`🔧 toggleSection נקרא עבור סקשן: ${sectionId}`);
  
  // בדיקה אם מערכת ניהול סקשנים זמינה
  if (typeof window.toggleSection === 'function' && window.toggleSection !== toggleSection) {
    console.log('✅ משתמש במערכת ניהול סקשנים הכללית');
    return window.toggleSection(sectionId);
  }
  
  // Fallback - מימוש מקומי פשוט
  console.log('⚠️ מערכת ניהול סקשנים לא זמינה, שימוש במימוש מקומי');
  const section = document.querySelector(`#${sectionId}, .${sectionId}-section`);
  if (section) {
    ...
    if (sectionBody && toggleBtn) {
      ...
      // שמירת מצב באמצעות פונקציית עזר
      setToUnifiedCache(`${sectionId}SectionCollapsed`, !isHidden, 'localStorage');
      
      console.log(`✅ סקשן ${sectionId} ${isHidden ? 'נפתח' : 'נסגר'}`);
    }
  } else {
    console.warn(`⚠️ לא נמצא סקשן עם מזהה: ${sectionId}`);
  }
}
