/**
 * בדיקה סופית של התיקון
 * Final fix test
 */

console.clear();
console.log('%c🎯 ===== בדיקה סופית של התיקון =====', 'font-size: 20px; font-weight: bold; color: #e74c3c; background: linear-gradient(90deg, #e74c3c, #c0392b); color: white; padding: 15px; border-radius: 10px;');
...
if (!copyButton) {
  ...
  // בדיקת CSS Variables
  console.log('\n%c🎨 בדיקת CSS Variables:', 'font-size: 16px; font-weight: bold; color: #2c3e50;');
  const rootStyle = getComputedStyle(document.documentElement);
  console.log(`%c--primary-color: ${rootStyle.getPropertyValue('--primary-color')}`, 'color: #3498db;');
  console.log(`%c--apple-gray-light: ${rootStyle.getPropertyValue('--apple-gray-light')}`, 'color: #95a5a6;');
}
