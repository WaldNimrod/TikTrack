...
async function loadModalData() {
  try {
    ...
    try {
      ...
      console.warn('⚠️ שגיאה בטעינת חשבונות:', error);
    }
    ...
    try {
      ...
      if (tradesResponse.ok) {
        ...
        trades = tradesData.data || tradesData;
      }
    } catch (error) {
      console.warn('⚠️ שגיאה בטעינת טריידים:', error);
    }
    ...
    try {
      ...
      if (tradePlansResponse.ok) {
        ...
        tradePlans = tradePlansData.data || tradePlansData;
      }
    } catch (error) {
      console.warn('⚠️ שגיאה בטעינת תכנונים:', error);
    }
    ...
    try {
      ...
      if (tickersResponse.ok) {
        ...
        tickers = tickersData.data || tickersData;
      }
    } catch (error) {
      console.warn('⚠️ שגיאה בטעינת טיקרים:', error);
    }

    console.log(`✅ נטענו ${accounts.length} חשבונות, ${trades.length} טריידים, ${tradePlans.length} תכנונים, ${tickers.length} טיקרים`);
    console.log('🔍 נתונים שנטענו:', { accounts, trades, tradePlans, tickers });

    // שמירת הנתונים במשתנים גלובליים
    window.modalAccounts = Array.isArray(accounts) ? accounts : [];
    window.modalTrades = Array.isArray(trades) ? trades : [];
    window.modalTradePlans = Array.isArray(tradePlans) ? tradePlans : [];
    window.modalTickers = Array.isArray(tickers) ? tickers : [];

    // טעינת אפשרויות לטיקר כברירת מחדל
    setTimeout(() => {
      onNoteRelationTypeChange();
    }, 100);
    ...
    // הגדרת מערכים ריקים במקרה של שגיאה
    window.modalAccounts = [];
    window.modalTrades = [];
    window.modalTradePlans = [];
    window.modalTickers = [];
  }
}
