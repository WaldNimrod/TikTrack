#!/usr/bin/env node
/**
 * CRUD E2E Repeat Validation - Team D (Gate 1 Re-run)
 * ================================================
 *
 * Re-run Full CRUD E2E after Team fixes - expecting 15/15 success
 */
...
class CRUDRepeatValidation {
    ...
    async validateSystemHealth() {
        ...
        fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'crud_e2e_repeat_validation.js:70',message:'Starting system health validation',data:{runId:this.results.runId},timestamp:Date.now(),sessionId:'debug-session',runId:this.results.runId,hypothesisId:'HEALTH'})}).catch(()=>{});
        ...
        if (response.ok) {
            ...
            fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'crud_e2e_repeat_validation.js:80',message:'System health validation passed',data:{apiStatus:response.status,runId:this.results.runId},timestamp:Date.now(),sessionId:'debug-session',runId:this.results.runId,hypothesisId:'HEALTH'})}).catch(()=>{});
            return response.data.data.access_token;
        } else {
            console.log('❌ System health FAIL - API not responding');
            fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'crud_e2e_repeat_validation.js:84',message:'System health validation failed',data:{apiStatus:response.status,runId:this.results.runId},timestamp:Date.now(),sessionId:'debug-session',runId:this.results.runId,hypothesisId:'HEALTH'})}).catch(()=>{});
            return null;
        }
    }
    ...
    async runCRUDE2EForEntity(entityType, token) {
        ...
        fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'crud_e2e_repeat_validation.js:109',message:'Starting CRUD test for entity',data:{entityType,apiEndpoint,runId:this.results.runId},timestamp:Date.now(),sessionId:'debug-session',runId:this.results.runId,hypothesisId:'CRUD_TEST'})}).catch(()=>{});
        ...
        try {
            ...
            if (crudResults.overallSuccess) {
                ...
                fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'crud_e2e_repeat_validation.js:197',message:'CRUD test passed',data:{entityType,executionTimeMs:crudResults.executionTimeMs,runId:this.results.runId},timestamp:Date.now(),sessionId:'debug-session',runId:this.results.runId,hypothesisId:'CRUD_TEST'})}).catch(()=>{});
            } else {
                console.log(`  ❌ ${entityType} CRUD: FAILED (${crudResults.executionTimeMs}ms)`);
                fetch('http://127.0.0.1:7243/ingest/6e906bd0-148a-41fc-aa3b-e13c2ed1de41',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'crud_e2e_repeat_validation.js:201',message:'CRUD test failed',data:{entityType,executionTimeMs:crudResults.executionTimeMs,runId:this.results.runId},timestamp:Date.now(),sessionId:'debug-session',runId:this.results.runId,hypothesisId:'CRUD_TEST'})}).catch(()=>{});
            }
    ...
    getCreateDataForEntity(entityType) {
        const baseData = {
            trade: {
                ...
                entry_price: 100.0,
                notes: `Gate 1 Repeat Test Trade ${Date.now()}`
            },
            trade_plan: {
                ticker_id: 1,
                trading_account_id: 1,
                ...
                entry_price: 100.0,
                notes: `Gate 1 Repeat Test Trade Plan ${Date.now()}`
            },
            alert: {
                related_type_id: 1,
                related_id: 1,
            ...
            ticker: {
                ...
                name: `Gate 1 Repeat Test Ticker ${Date.now()}`,
                exchange: 'NASDAQ'
            },
            trading_account: {
                name: `Gate 1 Repeat Account ${Date.now()}`,
                account_type: 'stock'
            },
            note: {
                title: `Gate 1 Repeat Note ${Date.now()}`,
                content: `Gate 1 Repeat Test Content ${Date.now()}`,
                related_type_id: 1,
                related_id: 1
            },
            tag: {
                name: `Gate 1 Repeat Tag ${Date.now()}`,
                color_hex: '#ff0000'
            },
            watch_list: {
                name: `Gate 1 Repeat Watch List ${Date.now()}`,
                color_hex: '#00ff00'
            },
            execution: {
                trade_id: 1,
            ...
            cash_flow: {
                ...
                notes: `Gate 1 Repeat Test Cash Flow ${Date.now()}`
            },
            user_profile: {
                first_name: `Gate1Repeat${Date.now()}`,
                last_name: 'Test'
            },
            user_management: {
                username: `gate1repeat${Date.now()}`,
                email: `gate1repeat${Date.now()}@test.com`,
                first_name: `Gate1Repeat${Date.now()}`,
                last_name: 'User'
            },
            trading_journal: {
                notes: `Gate 1 Repeat Test Journal Entry ${Date.now()}`
            },
            tag_management: {
                name: `Gate 1 Repeat Tag Mgmt ${Date.now()}`,
                color_hex: '#0000ff'
            },
            data_import: {
                name: `Gate 1 Repeat Import ${Date.now()}`,
                import_type: 'csv'
            },
            preferences: {
                theme: 'light',
    ...
    getUpdateDataForEntity(entityType, entityId) {
        const updateData = {
            trade: {
                notes: `Gate 1 Repeat Updated Trade ${Date.now()}`
            },
            trade_plan: {
                notes: `Gate 1 Repeat Updated Trade Plan ${Date.now()}`
            },
            alert: {
                condition_number: 200
            },
            ticker: {
                name: `Gate 1 Repeat Updated Ticker ${Date.now()}`
            },
            trading_account: {
                name: `Gate 1 Repeat Updated Account ${Date.now()}`
            },
            note: {
                content: `Gate 1 Repeat Updated Content ${Date.now()}`
            },
            tag: {
                name: `Gate 1 Repeat Updated Tag ${Date.now()}`
            },
            watch_list: {
                name: `Gate 1 Repeat Updated Watch List ${Date.now()}`
            },
            execution: {
                price: 200.0
            },
            ...
            user_profile: {
                first_name: `UpdatedRepeat${Date.now()}`
            },
            user_management: {
                first_name: `UpdatedRepeat${Date.now()}`
            },
            trading_journal: {
                notes: `Gate 1 Repeat Updated Journal ${Date.now()}`
            },
            tag_management: {
                name: `Gate 1 Repeat Updated Tag Mgmt ${Date.now()}`
            },
            data_import: {
                name: `Gate 1 Repeat Updated Import ${Date.now()}`
            },
            preferences: {
                theme: 'dark'
            }
    ...
    async runRepeatValidation() {
        console.log('🔄 CRUD E2E Repeat Validation - Gate 1');
        console.log('=====================================');
        ...
        console.log(`🎯 Testing ${crudEntities.length} entities with full CRUD operations (expecting 15/15 PASS):`);
        console.log('================================================================================');
    ...
    generateReport() {
        console.log('\n' + '='.repeat(80));
        console.log('🔄 CRUD E2E REPEAT VALIDATION REPORT - GATE 1');
        ...
        console.log('\n🏁 CRUD E2E Repeat Validation Complete');
    ...
    exportResults() {
        const reportPath = path.join(__dirname, 'crud_e2e_repeat_report.json');
        fs.writeFileSync(reportPath, JSON.stringify(this.results, null, 2));
        console.log(`\n💾 Detailed report saved to: ${reportPath}`);

        // Export summary for documentation
        const summaryPath = path.join(__dirname, 'crud_e2e_repeat_summary.json');
        const summary = {
            runId: this.results.runId,
...
if (require.main === module) {
    const validator = new CRUDRepeatValidation();
...
module.exports = CRUDRepeatValidation;
