...
- [ ] **סדר טעינת CSS:** וידוא שסדר הטעינה נכון (לפי `CSS_LOADING_ORDER.md`)

#### 1.2 בדיקת טעינת CSS (חובה לפני יישום)
- [ ] **כלי בדיקה:** הרצת `npm run check:css` על הבלופרינט
- [ ] **וידוא טעינה:** בדיקה שכל קבצי ה-CSS נטענים
- [ ] **וידוא סדר:** בדיקה שסדר הטעינה נכון
- [ ] **וידוא אין כפילויות:** בדיקה שאין כפילויות בטעינה
- [ ] **תיעוד בעיות:** תיעוד כל בעיות טעינת CSS שנמצאו

#### 1.3 בדיקת מבנה DOM (חובה לפני יישום)
- [ ] **השוואת מבנה:** השוואת מבנה DOM לבלופרינט הקודם (אם קיים)
- [ ] **וידוא תבנית V3:** בדיקה שהמבנה תואם לתבנית V3
...
#### 2.1 בדיקת טעינת CSS (חובה לפני יישום)
- [ ] **כלי בדיקה:** הרצת `npm run check:css` על הבלופרינט
- [ ] **וידוא טעינה:** בדיקה שכל קבצי ה-CSS נטענים
...
#### 2.2 בדיקת מבנה DOM (חובה לפני יישום)
- [ ] **כלי בדיקה:** הרצת `blueprint-comparison.js` על הבלופרינט
- [ ] **וידוא מבנה:** בדיקה שמבנה DOM תואם לתבנית V3
- [ ] **וידוא מחלקות:** בדיקה שכל המחלקות קיימות במערכת
- [ ] **תיעוד בעיות:** תיעוד כל בעיות מבנה DOM שנמצאו

#### 2.3 הרצת כלי הבדיקה המלא
...
- [ ] **תבנית:** Template V3 (כמו `HomePage.jsx`):
  ...
  // CSS Loading Order (CRITICAL - DO NOT CHANGE):
  // 1. Pico CSS - Already loaded in index.html
  // 2. phoenix-base.css - Already loaded in main.jsx
  // 3. phoenix-components.css - Already loaded in main.jsx
  // 4. phoenix-header.css - Already loaded in main.jsx
  ...
  ```
- [ ] **וידוא טעינת CSS:** בדיקה שכל קבצי ה-CSS הנדרשים נטענים (לפי `CSS_LOADING_ORDER.md`)
...
- [ ] **בדיקת טעינת CSS:** הרצת `npm run check:css` על העמוד בפועל
- [ ] **בדיקת מבנה DOM:** הרצת `blueprint-comparison.js` על העמוד בפועל
...
- [ ] **קובץ:** `documentation/00-MANAGEMENT/00_MASTER_INDEX.md` (אינדקס מאוחד; D15_SYSTEM_INDEX — DEPRECATED)
...
### 1. כלי בדיקת טעינת CSS
- **קובץ:** `ui/check-css-loading.js`
- **שימוש:** `npm run check:css`
- **תפקיד:** בודק שכל קבצי ה-CSS נטענים בסדר הנכון
- **תוצאות:** דוח מפורט על בעיות טעינת CSS

### 2. כלי בדיקה אוטומטי (מבנה DOM וסגנונות)
- **קובץ:** `ui/blueprint-comparison.js`
- **שימוש:** העתקת הקובץ לקונסולת הדפדפן והרצתו
- **תפקיד:** בודק מבנה DOM וסגנונות CSS
...
### לפני מסירה לצוות 30:
- [ ] הבלופרינט מקשר לקבצי ה-CSS הקיימים בסדר הנכון
- [ ] הבלופרינט משתמש במחלקות הקיימות במערכת בלבד
- [ ] הבלופרינט מכיל תוכן דמה מלא
- [ ] הבלופרינט תואם לתבנית V3
- [ ] **בדיקת טעינת CSS:** הרצת `npm run check:css` (אם רלוונטי)
- [ ] **בדיקת מבנה DOM:** הרצת `blueprint-comparison.js` (אם רלוונטי)
- [ ] **תיעוד בעיות:** תיעוד כל בעיות טעינת CSS ומבנה DOM שנמצאו
