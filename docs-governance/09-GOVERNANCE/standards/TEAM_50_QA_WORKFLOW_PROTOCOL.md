...
### ⚠️ חובה: יצירת בדיקה חדשה — עדכון אינדקס מיידי

**עם יצירת כל בדיקה חדשה (קובץ .test.js, .e2e.test.js, סקריפט run-*-qa-api.sh וכו'):**

1. **[ ] עדכון TEAM_50_QA_TEST_INDEX.md** — הוספת הבדיקה לאינדקס:
   - סעיף קטגוריה רלוונטי (או יצירת סעיף חדש)
   - קישור לקובץ הבדיקה
   - תיאור קצר (מה בודק, הרצה, דרישות)
2. **[ ] עדכון טבלת "קישורים לבדיקות"** באינדקס
3. **[ ] עדכון package.json** — הוספת npm script (אם E2E/Node)

**מקור אינדקס:** `documentation/09-GOVERNANCE/standards/TEAM_50_QA_TEST_INDEX.md`
...
### **תחזוקת סדר קבצי הבדיקות + אינדקס בדיקות (מחייב):**
- ✅ **עם יצירת כל בדיקה חדשה** — עדכון מיידי של `TEAM_50_QA_TEST_INDEX.md` (סעיף קטגוריה + קישור + תיאור קצר)
- ✅ אינדקס חייב להיות עדכני ומסונכרן עם Specs חדשים
- ✅ כל בדיקה חדשה חייבת להיכנס לאינדקס — אין יצירת בדיקה ללא תיעוד באינדקס
- ✅ תחזוקה שוטפת של אינדקס הבדיקות (לאחר Phase/Module completion)
