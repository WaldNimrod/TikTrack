...
- הסרת `packageScripts` fallback מ-`standardize-pages.js`
- שימוש ב-`PACKAGE_MANIFEST` בלבד
- אחידות קונפיגורציה בכל העמודים

#### ✅ **אחידות מודלים**

- יצירת `ModalHelperService` חדש עם `showModalSafe` גלובלי
- הסרת הגדרות כפולות מ-8 עמודים
- שירות מרכזי לכל פעולות מודלים

#### ✅ **אבטחת API**

- הוספת `@require_authentication()` למסלולי `/pending-plan/*`
- הגנה על נתונים רגישים מפני גישה אנונימית

#### ✅ **סדר טעינה משופר**

- החלפת `async` ב-`defer` לסקריפטים קריטיים
- מניעת מצבי מרוץ בטעינת scripts

### השפעה

- **יציבות משופרת**: 0 עמודים עם async קריטי
- **אבטחה מחוזקת**: כל מסלולי API רגישים מוגנים
- **אחידות קוד**: ModalHelperService מאוחד בכל העמודים
- **תחזוקה קלה**: קונפיגורציה נקייה ומרכזית
