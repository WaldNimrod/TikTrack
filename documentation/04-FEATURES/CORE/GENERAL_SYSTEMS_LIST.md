...
6. **PageStateManager** - מנהל מצב מרכזי לכל עמודי המערכת (פילטרים, סידור, סקשנים, פילטרים פנימיים) ✅ **חדש! ינואר 2025**
...
### 🎯 **עמודים מסטנדרטיים**
עמודים שהושלמה בהם סטנדרטיזציה מלאה למערכות כלליות:
- ✅ **trade_plans.html** - תכנון (רפקטורינג מלא ינואר 2025)
  - **מבנה מודולרי:** 4 מודולים (data, business, ui, main)
  - **גודל:** 276-501 שורות למודול (סה"כ 1,466 שורות)
  - **ביצועים:** טעינה < 500ms, רינדור < 200ms
- ✅ **trades.html** - מעקב (רפקטורינג מלא ינואר 2025)
  - **מבנה מודולרי:** 4 מודולים (data, business, ui, main)
  - **גודל:** 316-535 שורות למודול (סה"כ 1,635 שורות)
  - **ביצועים:** טעינה < 500ms, רינדור < 200ms
...
### **PageStateManager** 🟢 **חבילת בסיס** ✅ **חדש! ינואר 2025**
- **קובץ:** `page-state-manager.js`
- **תיאור:** מנהל מצב מרכזי לכל עמודי המערכת - שמירה ושחזור של מצב מלא (פילטרים, סידור, סקשנים, פילטרים פנימיים)
- **דוקומנטציה:** [PAGE_STATE_MANAGEMENT_SYSTEM.md](../../02-ARCHITECTURE/FRONTEND/PAGE_STATE_MANAGEMENT_SYSTEM.md)
- **תכונות עיקריות:**
  - שמירה/טעינה של מצב מלא לעמוד: filters, sort, sections, entityFilters
  - שימוש רק ב-UnifiedCacheManager לאחסון
  - מיגרציה אוטומטית של נתונים קיימים מ-localStorage
  - אינטגרציה מלאה עם כל מערכות המצב במערכת
- **API עיקרי:**
  - `savePageState(pageName, state)` - שמירת מצב מלא
  - `loadPageState(pageName)` - טעינת מצב מלא
  - `saveFilters(pageName, filters)` - שמירת פילטרים בלבד
  - `saveSort(pageName, sort)` - שמירת סידור בלבד
  - `saveSections(pageName, sections)` - שמירת סקשנים בלבד
  - `saveEntityFilters(pageName, entityFilters)` - שמירת פילטרים פנימיים בלבד
  - `migrateLegacyData(pageName)` - מיגרציה של נתונים קיימים
- **מערכות משולבות:**
  - `header-system.js` - מערכת פילטרים ראשית
  - `ui-utils.js` - מערכת ניהול סקשנים
  - `entity-details-renderer.js` - מערכת פילטרים פנימיים
  - `tables.js` - מערכת סידור טבלאות
  - `unified-table-system.js` - מערכת טבלאות מאוחדת
- **שימוש:**
  - כל 8 עמודי המשתמש המרכזיים משתמשים ב-PageStateManager
  - שמירה אוטומטית בעת שינויים (פילטרים, סידור, סקשנים)
  - שחזור אוטומטי בעת טעינת עמוד דרך `restorePageState()`
...
### **עדכון אחרון:**
- **תאריך:** 27 ינואר 2025
- **שינוי:** עדכון מלא למערכת כללית חדשה עם 8 מודולים + הוספת PageStateManager למערכת ניהול מצב עמודים
- **סטטוס:** מעודכן ומתועד - מערכת חדשה פעילה + PageStateManager מושלם
...
#### **מטריקס JS** (`/page-scripts-matrix`) - הועבר לארכיון (נובמבר 2025)
> נשמר לתיעוד היסטורי בלבד.
- **🟢 חבילת בסיס:** מלא (כל המערכות) _(סטטוס: ארכיון)_
- **🔵 חבילת CRUD:** חלקי (טבלאות + אחסון)
- **🔍 חבילת פילטרים:** מלא (כותרת + מיפוי + זיהוי)
- **📈 חבילת גרפים ותצוגה:** חלקי (עמודים בלבד)
- **🔔 חבילת התראות מתקדמות:** לא נדרש
...
#### **מפת JS** (`/js-map`) - הועבר לארכיון (נובמבר 2025)
> הכלי הישן הוסר מהמערכת הפעילה; המיפוי זמין כיום דרך דוחות איכות הקוד ומערכות הניטור המאוחדות.
- **🟢 חבילת בסיס:** מלא (כל המערכות) _(סטטוס: ארכיון)_
- **🔵 חבילת CRUD:** חלקי (טבלאות + אחסון) _(סטטוס: ארכיון)_
- **🔍 חבילת פילטרים:** מלא (כותרת + מיפוי + זיהוי) _(סטטוס: ארכיון)_
- **📈 חבילת גרפים ותצוגה:** חלקי (עמודים בלבד) _(סטטוס: ארכיון)_
- **🔔 חבילת התראות מתקדמות:** לא נדרש
- **🎨 חבילת ממשק משתמש מתקדם:** מלא (כותרת + תפריט + צבעים) _(סטטוס: ארכיון)_
- **⚙️ חבילת העדפות והגדרות:** לא נדרש
- **📁 חבילת קבצים ומיפוי:** מלא (מיפוי קבצים) _(סטטוס: ארכיון)_
- **🗓️ חבילת תאריכים וזמן:** לא נדרש
- **🌐 חבילת נתונים חיצוניים:** לא נדרש
