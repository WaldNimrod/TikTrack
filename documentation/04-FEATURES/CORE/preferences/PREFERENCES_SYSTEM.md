...
### סטטיסטיקות מערכת נוכחיות - **עדכון 12/01/2025:**
- **110 העדפות** פעילות במערכת (עדכון מדויק לפי DB)
- **13 קבוצות** העדפות מאורגנות
- **2 פרופילים** משתמש (ברירת מחדל + נימרוד) - זהים לחלוטין
- **110 העדפות** שמורות למשתמש בכל פרופיל
- **מטבע משני יורו** מוגדר בפרופיל נימרוד
- **58 צבעים** (entity, status, type, priority, value, chart colors)
- **22 הגדרות התראות** (הקבוצה הגדולה ביותר)
...
- `POST /api/preferences/profiles` - יצירת פרופיל חדש (עם ברירות מחדל)
...
**יצירת פרופיל חדש:**
- פרופיל חדש נוצר עם כל ברירות המחדל של המערכת (110 העדפות)
- לא מעתיק מהפרופיל הפעיל - התחלה נקייה
- **מופעל אוטומטית** - הופך לפרופיל הפעיל מיד אחרי היצירה
...
**עדכון אחרון:** 12 באוקטובר 2025 - Profile Management CRUD הושלם
**גרסה:** 4.1
**שינויים:** הוספת יצירה ומחיקה של פרופילים (Backend + Frontend + API)
