...
| 12 | טקסט שמירה | "שמירה" (לא "שמור") — תמיד | D16 SSOT §2.3 |
| 12a | **כפתורים עברית — ללא גנדר** | אי־שימוש בצורות זכר יחיד (שמור, בטל, בחר). שימוש: שמירה, לבטל, לבחור, צרוף קובץ, הוספה, סגירה. ראה §2.4 | נוהל פנימי |
...
## 2.4 כלל ברזל — טקסט כפתורים בעברית (ללא גנדר)

חובה להשתמש בטקסטים הבאים — **תמיד**:
...
**דוגמאות:** `saveButtonText: 'שמירה'`, `cancelButtonText: 'לבטל'`, placeholder: `"— לבחור טיקר —"`.  
**טרמינולוגיה:** אין "חשבון" — רק חשבון מסחר (trading account).
