...
- **user_preferences_v3**: 12 רשומות _(מכיל עמודת `updated_at` חדשה לשמירת גרסת ההעדפות ובקרה על עדכונים)_
...
4. **טבלאות שנמחקו** - tickers_new נמחקה ✅ ; `user_preferences_v3` נשארת פעילה ומשמשת כמקור הנתונים לגרסת ההעדפות.

---

**עדכון 14 נובמבר 2025:**  
- לטבלת `user_preferences_v3` נוספה עמודת `updated_at` (מוזנת אוטומטית) כדי לאפשר ל־`/api/preferences/version` לזהות שינויי העדפות ולהשוות גרסאות מול ה־Frontend.  
- כל פעולת עדכון/הכנסה מעדכנת כעת את `updated_at`, וערכי עבר הוזנו רטרואקטיבית מתוך `created_at` בכל בסיסי הנתונים הפעילים.
