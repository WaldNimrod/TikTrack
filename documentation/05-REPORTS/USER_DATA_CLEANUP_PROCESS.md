...
- **משתמשים:** **נשמרים במלואו** - כל המשתמשים במערכת נשמרים (מערכת multi-user פעילה)
- **נתוני משתמש נמחקים:** 
  - **הערות** (`notes`)
  - **ביצועים** (`executions`)
  - **טריידים** (`trades`)
  ...
  - **ניתוחי AI** (`ai_analysis_requests`) - כל ניתוחי ה-AI נמחקים
  - **שיוך טיקרים** (`user_tickers`) - כל השיוכים נמחקים
- **מפתחות API:** **נשמרים במלואו** - `user_llm_providers` לא נמחק (מכיל מפתחות API מוצפנים של משתמשים)
...
- **פרופילים והעדפות:** נשמר רק הפרופיל הפעיל של כל משתמש, נמחקים כל השאר. הערכי העדפות נמחקים (המשתמש יכול להגדיר מחדש)
...
### קטגוריה 7: ניתוחי AI ושיוך טיקרים

#### 7.1 `ai_analysis_requests` - ניתוחי AI
**פעולה:** מחיקת כל הרשומות
...
**הערות חשובות:**
- כל ניתוחי ה-AI נמחקים - אלה נתוני משתמש
- **לא למחוק** `user_llm_providers` - טבלה זו מכילה מפתחות API מוצפנים של משתמשים ונשמרת במלואה
- תבניות AI (`ai_analysis_templates`) נשמרות - אלה נתוני מערכת

#### 7.2 `user_tickers` - שיוך טיקרים למשתמשים
**פעולה:** מחיקת כל הרשומות
...
**הערות חשובות:**
- כל השיוכים בין טיקרים למשתמשים נמחקים
- הטיקרים עצמם נשמרים (רק SPY נשאר)
- בעת יצירת נתוני דוגמה, יש ליצור מחדש את השיוכים דרך `user_tickers`
...
| 14 | `user_llm_providers` | מפתחות API של משתמשים | מפתחות API מוצפנים - נשמר |
| 15 | `ai_analysis_templates` | תבניות ניתוח AI | תבניות מערכת - נשמר |

### טבלאות עם ניקוי חלקי

| # | שם טבלה | פעולה | הערות |
|---|---------|-------|-------|
| 1 | `users` | **שמירה מלאה** | כל המשתמשים נשמרים (מערכת multi-user פעילה) |
| 2 | `tickers` | שמירת SPY בלבד | רק טיקר SPY נשמר |
| 3 | `preference_profiles` | שמירת פרופיל פעיל | רק הפרופיל הפעיל של כל משתמש |
...
| 7 | `user_llm_providers` | **שמירה מלאה** | מפתחות API מוצפנים - נשמרים במלואם |
...
### שלב 6.5: ניתוחי AI ושיוך טיקרים
1. ✅ מחיקת `ai_analysis_requests` - כל ניתוחי ה-AI
2. ✅ מחיקת `user_tickers` - כל שיוכי הטיקרים למשתמשים
3. ✅ **לא למחוק** `user_llm_providers` - מפתחות API נשמרים

**הערות:**
- כל ניתוחי ה-AI נמחקים (נתוני משתמש)
- כל השיוכים בין טיקרים למשתמשים נמחקים
- מפתחות API מוצפנים נשמרים ב-`user_llm_providers`
- תבניות AI (`ai_analysis_templates`) נשמרות (נתוני מערכת)
...
**הערות:**
- ⚠️ **כל המשתמשים נשמרים במלואם** - מערכת multi-user פעילה
- נשמר רק הפרופיל הפעיל (`is_active=TRUE`) של כל משתמש במערכת
...
5. ✅ בדיקת שנותר רק הפרופיל הפעיל של כל משתמש
6. ✅ בדיקת שתגיות מובנות של ייבוא נשמרו
7. ✅ בדיקת שתנאים בסיסיים נשמרו (trading_methods, method_parameters)
8. ✅ בדיקת שכל ניתוחי ה-AI נמחקו (`ai_analysis_requests` ריק)
9. ✅ בדיקת שכל שיוכי הטיקרים נמחקו (`user_tickers` ריק)
10. ✅ בדיקת שמפתחות API נשמרו (`user_llm_providers` לא נמחק)
11. ✅ **הרצת השרת מול בסיס הנתונים החדש** לבדיקה ותיקונים בהתאם לצורך
...
- **שיוך טיקרים** - כל הטיקרים משויכים למשתמש דרך `user_tickers`
- **2 חשבונות מסחר:**
...
- **ניתוחי AI** - מספר ניתוחי AI לדוגמה, משויכים למשתמש
...
| 14 | `preference_profiles` | מחיקה חלקית | רק הפרופיל הפעיל של כל משתמש נשמר |
| 15 | `tag_categories` | מחיקה חלקית | רק קטגוריות "ייבוא נתונים" נשמרות |
| 16 | `tags` | מחיקה חלקית | רק תגים מובנים של ייבוא נשמרים |
| 17 | `ai_analysis_requests` | מחיקה מלאה | כל ניתוחי ה-AI נמחקים |
| 18 | `user_tickers` | מחיקה מלאה | כל שיוכי הטיקרים למשתמשים נמחקים |
