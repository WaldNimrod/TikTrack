...
## ✅ עדכון 8 בנובמבר 2025 – מודול פרטי תזרים מזומנים
- `EntityDetailsService` מעשיר כעת את תגובת `/api/entity-details/cash_flow/<id>` בשדות `account_name`, `currency_symbol`, `trade_symbol`, `trade_status`, `trade_side` וערכי fallback ל-`type`/`date`, כך שה-UI מקבל את כל המידע הדרוש להצגה מלאה.
- `entity-details-renderer.js` מציג סכום, עמלה, תאריך תזרים, טרייד מקושר, סוג תזרים, מקור ומזהה חיצוני בפורמט מרוכז עם עיצוב RTL מלא, ללא שדה סטטוס (לתזרים אין סטטוס). תיאור התזרים מוצג במקטע מלא ברוחב הקרטיס.
- פילטר הפריטים המקושרים (Linked Items) במודל הפרטים של תזרים בוטל – מוצגת טבלה מינימלית עם הפריט היחיד הרלוונטי, בהתאם לדרישת האפיון.
- `entity-details-api.js` מבצע נורמליזציה עקבית לשדות `type`, `date`, `account_name`, `currency_symbol`, `source` ו-`external_id`, כולל fallback להעדפות המטבע הגלובליות בעת הצורך.
...
### עדכון 16 בנובמבר 2025 – רינדור פרטי המרה ושיפורי תצוגה
- תאריכים: השרת שולח ומקבל DateEnvelope; כל נקודות ה-API של תזרימים/המרות עודכנו לתמיכה בעטיפה זו (וכן ISO/`YYYY-MM-DD`). ב-UI, `formatDate` מציג `display`/`local`/`utc`/`epochMs` כראוי.
- פרטי המרה במודול פרטים: מוצגים "מקור" (from) ו"מבוקש" (to) עם שם/סמל מטבע מתוך `exchange_pair_summary`, יחד עם שער המרה ועמלה.
- כפתור "פתח רשומה" בוטל עבור `cash_flow` (אין דף פרטים עצמאי).
- בטבלאות: הוסר באדג' ההמרה (כולל אייקון); הכיוון/פרטים זמינים במודול הפרטים.
...
- ולידציה: אם `trade_id` מוגדר, הטרייד חייב להיות שייך לאותו `trading_account_id` (CUSTOM constraint `CASH_FLOW_TRADE_TICKER_MATCH`).
- Frontend: מודל בחירת טרייד מסתנן אוטומטית לפי `trading_account_id` של התזרים.
- שדה `linkedTrade` במודל תזרים מזומנים מאפשר בחירה וקישור לטרייד, עם תצוגה של פרטי הטרייד המקושר.
- הקישור הוא אופציונלי ואינו מחליף שדות קיימים (כמו `ticker_id`, `source`, `external_id`).
