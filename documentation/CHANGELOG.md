...
### Added
- **Tickers Modal** – נוספה אפשרות בחירה ל-`bond` (אג"ח) בטופס ההוספה/עריכה, בהתאם להרחבת הקטלוג המאוחד של סוגי טיקרים.

### Changed
- **TickerService** – ולידציית סוג טיקר מתבססת כעת על רשימת ENUM דינמית מבסיס הנתונים (כולל אג"ח), עם מנגנון מטמון וטיפול בנפילות, במקום רשימה קשיחה בצד השרת.
