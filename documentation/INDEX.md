...
### מערכת כללית חדשה ✅ **14 מערכות מאוחדות! הושלמה בינואר 2025**
- **מערכת כללית חדשה**: 8 מודולים מאוחדים + 6 מערכות שירות (100%)
  - 📋 **רשימת מערכות כלליות**: [GENERAL_SYSTEMS_LIST.md](frontend/GENERAL_SYSTEMS_LIST.md)
  - 🏗️ **ארכיטקטורת JavaScript**: [JAVASCRIPT_ARCHITECTURE.md](02-ARCHITECTURE/FRONTEND/JAVASCRIPT_ARCHITECTURE.md)
  - 🚀 **מערכת אתחול מאוחדת**: [UNIFIED_INITIALIZATION_SYSTEM.md](frontend/UNIFIED_INITIALIZATION_SYSTEM.md)
  - 🔧 **ארכיטקטורת שירותים**: [SERVICES_ARCHITECTURE.md](frontend/SERVICES_ARCHITECTURE.md) ✨ **חדש!**
  - 📋 **אפיון המערכת החדשה**: [new_general_systems_architecture_specification.md](../new_general_systems_architecture_specification.md)
...
- **6 מערכות שירות חדשות**: DataCollection, FieldRenderer, SelectPopulator, CRUDHandler, DefaultValues, Statistics
- **הפחתת ~6,940 שורות קוד**: סטנדרטיזציה מלאה של כל עמודי CRUD
- **90% חיסכון בזיכרון**: טעינה מותנית של מודולים
- **70% שיפור בזמן טעינה**: טעינה מהירה יותר
- **60% הפחתה בזמן פיתוח**: עמוד חדש מהיר יותר
- **תמיכה מלאה**: תאימות לאחור עם המערכת הישנה
- **דוקומנטציה מעודכנת**: כל הקבצים עודכנו בהתאם למערכת החדשה

#### 📚 **דוקומנטציה מפורטת למערכת החדשה**
- 🚀 **מערכת כללית חדשה**: 8 מודולים מאוחדים עם טעינה דינמית
...
- 📢 **מערכת הודעות והתראות**: [NOTIFICATION_SYSTEM.md](frontend/NOTIFICATION_SYSTEM.md) - דוקומנטציה מפורטת של מערכת ההודעות
- 📚 **מדריך יישום הודעות**: [NOTIFICATION_IMPLEMENTATION_GUIDE.md](frontend/NOTIFICATION_IMPLEMENTATION_GUIDE.md) - מדריך מעשי ליישום המערכת בעמודי האתר
...
- 🔄 **מערכת רענון מרכזיות**: [CENTRAL_REFRESH_SYSTEM.md](frontend/CENTRAL_REFRESH_SYSTEM.md)
...
- **העדפות**: מערכת הגדרות מתקדמת עם פרופילים מרובים, יבוא/יצוא ויסטוריה
...
### נתונים חיצוניים ✅ **מדיניות רענון מערכתית הוטמעה (אוקטובר 2025)**
...
**עדכון מרכזי:** מערכת הגדרות מערכתית (DB) למדיניות רענון נתונים חיצוניים הוטמעה. TTL דינמי לפי סטטוס טיקר ושעות מסחר (ניו־יורק) באמצעות `PolicyProvider`. נוספה תמיכה ב־`GET /api/tickers/active` עם TTL דינמי. הדשבורד מאפשר עריכה של ערכי TTL והפעלת Scheduler מערכתית.
...
**קבצים מרכזיים:**
- `Backend/services/system_settings_service.py` - שכבת ניהול הגדרות מערכתית
- `Backend/services/external_data/policy_provider.py` - ספק TTL דינמי + שעות מסחר
- `Backend/routes/api/system_settings.py` - API לניהול הגדרות מערכתיות
- `Backend/routes/api/tickers.py` - `GET /api/tickers/active` עם TTL דינמי
- `Backend/app.py` - הפעלת `DataRefreshScheduler` לפי הגדרה מערכתית

**השלמות עיקריות:**
1. מקור אמת יחיד למדיניות רענון (DB + Service)
2. כיבוד TTL בצד שרת וב־Frontend (SWR/UCM)
3. ניקוי מטמון תלוי־תלויות בעת עדכון הגדרות
4. דשבורד: עריכה + ניטור
...
**גרסה**: 2.0.6  
**עדכון אחרון**: 28 בספטמבר 2025  
**סטטוס**: יציב ופעיל עם External Data System ומערכת העדפות מלאה ומוכנה להפעלה
