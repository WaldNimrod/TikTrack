...
## 2) SSOT Rule (2026-03-21 update)

CURRENT_OPERATIONAL_STATE is auto-generated by `pipeline_run.sh` on every pass/fail/approve.
Teams MUST NOT manually edit this block.
The `ssot_check` tool validates consistency: `python -m agents_os_v2.tools.ssot_check`.
Manual WSM updates are only permitted by Team 170 for:
1. Baseline sync when operating in meta-process mode (no pipeline_run.sh)
2. Correcting stale state after architecture-level resets

---

## 3) Mandatory sync fields
...
Registry mirroring is the responsibility of the pipeline/automation or designated owner (Team 170 / Team 10 as per process). WSM CURRENT_OPERATIONAL_STATE itself is written by `pipeline_run.sh`; see §2 SSOT Rule.
...
## 4) Hierarchy and ID invariants (locked)
...
## 5) Allowed state: NO_ACTIVE_WORK_PACKAGE
...
## 7) Date discipline and anti-backward-time rule
