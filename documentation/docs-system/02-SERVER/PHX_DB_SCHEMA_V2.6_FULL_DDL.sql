...
--   KB-003: Renamed legacy fees table to trading_account_fees (per migration)
...
CREATE TABLE market_data.tickers (
    ...
    -- Constraints
    -- (no partial unique constraints inline; see uix_tickers_symbol_exchange_active)
);

CREATE UNIQUE INDEX uix_tickers_symbol_exchange_active
...
-- ----------------------------------------------------------------------------
-- Table: market_data.exchange_rates
-- ----------------------------------------------------------------------------
CREATE TABLE market_data.exchange_rates (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX idx_exchange_rates_last_sync
    ON market_data.exchange_rates(last_sync_time);

COMMENT ON TABLE market_data.exchange_rates IS 'Exchange rates for currency conversion (D21 CURRENCY_CONVERSION).';
COMMENT ON COLUMN market_data.exchange_rates.conversion_rate IS 'NUMERIC(20,8) canonical production column.';
...
-- ----------------------------------------------------------------------------
-- Table: user_data.user_refresh_tokens (added in V2.6 from production/ORM)
-- ----------------------------------------------------------------------------
...
CREATE UNIQUE INDEX idx_user_refresh_tokens_jti
...
CREATE INDEX idx_user_refresh_tokens_revoked_at
    ON user_data.user_refresh_tokens(revoked_at)
    WHERE revoked_at IS NULL;

COMMENT ON TABLE user_data.user_refresh_tokens IS 'JWT refresh tokens with rotation support';

-- ----------------------------------------------------------------------------
-- Table: user_data.revoked_tokens (added in V2.6 from production/ORM)
-- ----------------------------------------------------------------------------
...
COMMENT ON TABLE user_data.revoked_tokens IS 'JWT access token blacklist for revoked tokens';
...
CREATE TABLE user_data.trading_accounts (
    ...
    -- (no partial unique constraints inline; see uix_trading_accounts_unique_name_active)
);

CREATE UNIQUE INDEX uix_trading_accounts_unique_name_active
...
CREATE TABLE user_data.strategies (
    ...
    -- Constraints
    -- (no partial unique constraints inline; see uix_strategies_unique_name_per_user_active)
...
CREATE UNIQUE INDEX uix_strategies_unique_name_per_user_active
    ON user_data.strategies(user_id, name)
    WHERE deleted_at IS NULL;
...
-- ----------------------------------------------------------------------------
-- Table: user_data.trading_account_fees (ADR-015: Fees per Trading Account)
...
-- ----------------------------------------------------------------------------
CREATE TABLE user_data.trading_account_fees (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id UUID NOT NULL REFERENCES user_data.users(id) ON DELETE CASCADE,
    trading_account_id UUID NOT NULL REFERENCES user_data.trading_accounts(id) ON DELETE CASCADE,
    ...
    -- Constraints
    CONSTRAINT trading_account_fees_minimum_check CHECK (minimum >= 0),
    CONSTRAINT trading_account_fees_commission_value_check CHECK (commission_value >= 0)
);

CREATE INDEX idx_trading_account_fees_user_id 
    ON user_data.trading_account_fees(user_id);

CREATE INDEX idx_trading_account_fees_trading_account_id 
    ON user_data.trading_account_fees(trading_account_id);
    
CREATE INDEX idx_trading_account_fees_commission_type 
    ON user_data.trading_account_fees(commission_type);
    
CREATE INDEX idx_trading_account_fees_deleted_at 
    ON user_data.trading_account_fees(deleted_at) 
    WHERE deleted_at IS NULL;
    
CREATE INDEX idx_trading_account_fees_user_deleted 
    ON user_data.trading_account_fees(user_id, deleted_at) 
    WHERE deleted_at IS NULL;

CREATE INDEX idx_trading_account_fees_account_deleted 
    ON user_data.trading_account_fees(trading_account_id, deleted_at) 
    WHERE deleted_at IS NULL;

COMMENT ON TABLE user_data.trading_account_fees IS 'Fees per Trading Account (D18) - ADR-015. One-to-many: trading_account → trading_account_fees. Broker is on account only.';
COMMENT ON COLUMN user_data.trading_account_fees.trading_account_id IS 'FK to trading_accounts. Fees belong to account; broker derived from account.';
COMMENT ON COLUMN user_data.trading_account_fees.commission_value IS 'Commission value NUMERIC(20,6) - SSOT: _COMMUNICATION/team_10/TEAM_10_COMMISSION_VALUE_NUMERIC_DECISIONS.md';
COMMENT ON COLUMN user_data.trading_account_fees.minimum IS 'Minimum commission per transaction in USD (NUMERIC(20,6) for precision - per SSOT)';
...
CREATE TABLE user_data.user_api_keys (
    ...
    -- Constraints
    -- (no partial unique constraints inline; see uix_user_api_keys_unique_user_provider_active)
...
CREATE UNIQUE INDEX uix_user_api_keys_unique_user_provider_active
    ON user_data.user_api_keys(user_id, provider, provider_label)
    WHERE deleted_at IS NULL;
