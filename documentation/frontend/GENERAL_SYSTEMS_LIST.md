...
| **Business Logic API Wrappers** | `trading-ui/scripts/services/*-data.js` | [BUSINESS_LOGIC_LAYER.md](../02-ARCHITECTURE/BACKEND/BUSINESS_LOGIC_LAYER.md)<br>[BUSINESS_LOGIC_DEVELOPER_GUIDE.md](../03-DEVELOPMENT/GUIDES/BUSINESS_LOGIC_DEVELOPER_GUIDE.md) | Wrappers ל-Business Logic API בכל Data Service: **Trade** (6 wrappers: calculateStopPrice, calculateTargetPrice, calculatePercentageFromPrice, calculateInvestment, calculatePL, validateTrade), **Execution** (3 wrappers: calculateExecutionValues, calculateAveragePrice, validateExecution), **Alert** (2 wrappers: validateAlert, validateConditionValue), **Statistics** (4 wrappers: calculateStatistics, calculateSum, calculateAverage, countRecords), **Cash Flow** (2 wrappers: calculateCashFlowBalance, validateCashFlow), **Note** (2 wrappers: validateNote, validateNoteRelation), **Trading Account** (1 wrapper: validateTradingAccount), **Trade Plan** (1 wrapper: validateTradePlan), **Ticker** (2 wrappers: validateTicker, validateTickerSymbol). **סה"כ 23 wrappers** - כל wrapper משתמש ב-CacheTTLGuard למטמון אוטומטי |
