...
### איקונים שחסרים ונמצאים ב-`icon-mappings.js`:

| שם איקון חסר | אלטרנטיבה נוכחית | הערות |
|--------------|------------------|--------|
| `info-circle` | `note` | לא קיים ב-Tabler - משתמשים ב-`note` |
| `bookmark` | `clipboard-list` | לא קיים ב-Tabler - משתמשים ב-`clipboard-list` |
| `alert-circle` | `alert-triangle` | לא קיים ב-Tabler - משתמשים ב-`alert-triangle` |

**הערה:** איקונים אלה לא קיימים ב-Tabler Icons, ולכן המערכת משתמשת באלטרנטיבות. אין צורך להוריד אותם.
