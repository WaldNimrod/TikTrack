...
- [שלב 23: עדכון רשימת המשימות](#שלב-23-עדכון-רשימת-המשימות)
- [שלב 24: בדיקות נוספות לאימות מבנה תקין](#שלב-24-בדיקות-נוספות-לאימות-מבנה-תקין)
- [שלב 25: שילוב כפתורי פעולות בעמודת פעולות](#שלב-25-שילוב-כפתורי-פעולות-בעמודת-פעולות)
...
### צעד 24: בדיקות נוספות לאימות מבנה תקין
...
**טבלת התראות (רק ביטול ומחיקה):**
```javascript
loadTableActionButtons('alertsTable', 'alert', {
    showDetails: false,
    showLinked: false,
    showEdit: false,
    showCancel: true,
...
- **דינמית** - עובדת עם מספר שורות משתנה
- **גמישה** - הגדרות מותאמות אישית לכל טבלה
- **אוטומטית** - מגדירה פונקציות לפי סוג הישות
- **עקבית** - אותו עיצוב בכל הטבלאות

#### 25.5 דרישות HTML

**עמודת פעולות בטבלה:**
```html
<td class="actions-cell" data-entity-id="1" data-status="פתוח">
    <!-- כפתורים יטענו אוטומטית -->
</td>
```

**מזהה טבלה:**
```html
<table id="tickersTable" data-table-type="tickers">
    <!-- תוכן הטבלה -->
</table>
```

#### 25.6 יישום בעמוד ספציפי

**הוספה לקובץ הסקריפט של העמוד:**
```javascript
// טעינת כפתורי פעולות אחרי טעינת הטבלה
function loadActionButtons() {
    if (typeof window.loadTableActionButtons === 'function') {
        ...
        window.loadTableActionButtons('tableId', 'entityType', config);
    }
}

// קריאה אחרי טעינת העמוד
document.addEventListener('DOMContentLoaded', function() {
    // ... שאר הקוד ...
    loadActionButtons();
});
...
```bash
# 1. בדוק שהכפתורים מופיעים בכל השורות
# פתח את העמוד ובדוק שכל שורה בטבלה יש לה כפתורי פעולות

# 2. בדוק שהכפתורים עובדים
# לחץ על כל כפתור ובדוק שהוא מפעיל את הפונקציה הנכונה

# 3. בדוק שהעיצוב נכון
# וודא שכל הכפתורים נראים עקבית

# 4. בדוק שהגדרות מותאמות אישית עובדות
# אם הגדרת showDetails: false, וודא שכפתור הפרטים לא מופיע
```
...
|------|--------|--------|--------------|-------|
| **בדיקת כותרת** | `/test-header-only` | ✅ הושלם | 2025-01-15 | 3 סקשנים: מצב מערכות, טבלת ביצועים, מידע דיבאג - **כל 21 הסעיפים הושלמו** |
| **טבלאות עזר** | `/db_extradata` | ✅ הושלם | 2025-01-15 | 4 סקשנים: כרטיסיות התראות, מטבעות, סוגי קישור, כפתורי טריגרים - **כל 21 הסעיפים הושלמו** |
| **תבנית בסיס** | `/designs` | ✅ הושלם | 2025-01-15 | תבנית בסיס מעודכנת עם כל הסקריפטים והכפתורים - **כל 21 הסעיפים הושלמו** |
