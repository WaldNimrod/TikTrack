...
1. **שלב 1-2**: העלאת קובץ ובחירת חשבון מסחר
...
- `trading_account_id`: קישור ישיר לחשבון המסחר (עדכון 2025-10-27)
...
המערכת בודקת מול **כל הרשומות הקיימות במערכת** (לא רק לפי חשבון מסחר):
...
2. **בחירת חשבון מסחר**: רשימת חשבונות מסחר
...
- בדיקה מול כל הרשומות במערכת (לא רק לפי חשבון מסחר)
