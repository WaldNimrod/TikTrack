...
4. **שלב 3**: פתרון בעיות (מפורט, משתנה לפי Task)
   ...
   - הצגת בעיות בממשק אינטראקטיבי מפורט (ספציפי לסוג התהליך):
    ...
    - **כפילויות בקובץ**: כל רשומה ב-`within_file_duplicates` מקבלת `record_index` ייחודי; ה-UI מציג שורה ראשית + שורות match מקוננות (מ-`within_file_duplicate_match`) עם ויזואליזציה של הזוגות. **רק השורה הראשית מציגה כפתורי "קבל"/"דחה"** - שורות ה-match מציגות רק מידע. כפתורי הפעולה מופעלים דרך Button System (`data-onclick="acceptDuplicate"`/`"rejectDuplicate"`) כך שכל פעולה מעדכנת את אותה רשומה ב-skip list. **עמודת תאריך ושעה מדויקים** נוספה לטבלאות כפילויות (נובמבר 2025).
     - **רשומות קיימות**: כרטיסים עם כפתורי "קבל"/"דחה". **רשומות שנדחו (rejected) לא מוצגות** בטבלאות (נובמבר 2025).
