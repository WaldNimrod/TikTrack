...
class ApplicationInitializer {
    ...
    async initializeApplication(options = {}) {
        ...
        if (this.initializationStatus.isInitialized) {
            console.log('✅ Application already initialized, but checking for new page config...');
            // Store new page configuration even if already initialized
            this.pageConfig = options;
            console.log('🔧 Stored new page config:', this.pageConfig);
            
            // Re-run Page Systems stage with new config
            if (this.pageConfig.customInitializers && Array.isArray(this.pageConfig.customInitializers)) {
                console.log('🔄 Re-running Page Systems stage with new config...');
                await this.executeStage('page');
            }
...
window.appInitializer.registerStageFunction('core', async (pageConfig) => {
    ...
    if (window.UnifiedCacheManager) {
        try {
            ...
            console.warn('⚠️ UnifiedCacheManager initialization failed:', error);
        }
    }
    ...
    if (window.CacheSyncManager) {
        try {
            ...
            console.warn('⚠️ CacheSyncManager initialization failed:', error);
        }
    }
    ...
    if (window.CachePolicyManager) {
        try {
            ...
            console.warn('⚠️ CachePolicyManager initialization failed:', error);
        }
    }
    ...
    if (window.MemoryOptimizer) {
        try {
            ...
            console.warn('⚠️ MemoryOptimizer initialization failed:', error);
        }
    }
    
    
    console.log('✅ Core Systems initialized (IndexedDB ready)');
...
window.appInitializer.registerStageFunction('page', async (pageConfig) => {
    ...
    if (pageConfig.requiresTables && typeof window.setupSortableHeaders === 'function') {
        ...
        if (pageConfig.customInitializers && Array.isArray(pageConfig.customInitializers)) {
            console.log(`🔧 Executing ${pageConfig.customInitializers.length} custom initializers for ${pageConfig.name}...`);
            for (const customInit of pageConfig.customInitializers) {
                if (typeof customInit === 'function') {
                    try {
                        await customInit(pageConfig);
                    } catch (error) {
                        console.error('❌ Error in custom initializer:', error);
                    }
                }
            }
        } else {
            console.log(`⚠️ No custom initializers found for ${pageConfig.name || 'unknown page'}`);
            console.log('🔧 Page config details:', {
                name: pageConfig?.name,
                customInitializers: pageConfig?.customInitializers,
                hasCustomInitializers: !!pageConfig?.customInitializers,
                isArray: Array.isArray(pageConfig?.customInitializers)
            });
        }
...
window.appInitializer.registerStageFunction('final', async (pageConfig) => {
    ...
    // Show initialization complete notification (handled by unified system)
