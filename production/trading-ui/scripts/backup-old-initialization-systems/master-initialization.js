/**
 * Master Initialization System - TikTrack
 * ======================================
 *
 * מערכת האתחול המרכזית הסופית שמחליפה את כל ה-DOMContentLoaded listeners
 ...
 * 3. Smart Detection - זיהוי חכם של מערכות
...
class MasterInitialization {
  ...
  /**
   * Master initialization - Single Point of Entry
   */
  async initialize() {
    if (this.initialized) {
   ...
   */
  logSuccess() {
    console.log('🎉 Master Initialization Success!', {
      page: this.pageInfo.name,
      type: this.pageInfo.type,
      systems: this.availableSystems.size,
...
window.MasterInitialization = MasterInitialization;
window.masterInit = new MasterInitialization();

// ===== GLOBAL EXPORT =====

window.initializeMaster = async function () {
  return await window.masterInit.initialize();
};

window.getMasterInitStatus = function () {
  return window.masterInit.getStatus();
};
...
document.addEventListener('DOMContentLoaded', async () => {
  console.log('🎯 DOM Content Loaded - Starting Master Initialization');

  try {
    // Small delay to ensure all scripts are loaded
    setTimeout(async () => {
      await window.initializeMaster();
    }, 100);
  } catch (error) {
    console.error('❌ Master auto-initialization failed:', error);
  }
});
