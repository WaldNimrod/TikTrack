...
class CachePolicyManager {
  constructor() {
    ...
    this.defaultPolicies = {
      ...
      'preference-data': {
        ...
        optimize: false,
        syncToBackend: true,
        description: 'נתוני העדפות מעובדים (פר משתמש/פרופיל)',
      },
      'profile-data': {
        layer: 'memory',
        maxSize: 256 * 1024,
        ...
        optimize: false,
        syncToBackend: true,
        description: 'רשימות פרופילים והקשר פעיל',
      },
      'preference-groups': {
        layer: 'memory',
        maxSize: 128 * 1024,
        ...
        optimize: false,
        syncToBackend: true,
        description: 'מטא-דטה של קבוצות העדפות',
      },
      'preference-types': {
        layer: 'memory',
        maxSize: 256 * 1024,
        ...
        optimize: false,
        syncToBackend: true,
        description: 'טבלת סוגי העדפות מלאה',
      },
      'filter-state': {
        layer: 'localStorage',
  ...
  setupValidators() {
    ...
    this.validators.set('memory', data =>
      this.calculateDataSize(data) <= 100 * 1024, // 100KB
    );
    ...
    this.validators.set('indexedDB', data =>
      this.calculateDataSize(data) <= 100 * 1024 * 1024, // 100MB
    );

    // Backend Cache Validator
    this.validators.set('backend', data =>
      this.calculateDataSize(data) <= 50 * 1024 * 1024, // 50MB
    );
  }
  ...
  hasInvalidContent(data) {
    try {
      ...
      const hasControlChars = [...jsonString].some(char => {
...
if (window.UnifiedInitializationSystem) {
  window.UnifiedInitializationSystem.addCoreSystem('CachePolicyManager', () => window.CachePolicyManager.initialize());
}
