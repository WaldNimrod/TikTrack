...
${scriptsHTML}
  ...
  generateScriptTags(packages) {
    ...
    if (window.PACKAGE_MANIFEST) {
      ...
      // Add header comment
      html +=
        '    <!-- ===== SCRIPT LOADING ORDER - GENERATED BY PageTemplateGenerator ===== -->\n';
      html +=
        '    <!-- ⚠️ DO NOT MODIFY MANUALLY - Use PageTemplateGenerator.generateScriptTagsForPage() -->\n';
      html +=
        '    <!-- 📋 Loading Order: BASE → SERVICES → MODULES → UI-ADVANCED → CRUD → PREFERENCES → INIT-SYSTEM -->\n\n';
      ...
      html +=
        '    <!-- 🔧 For maintenance: Use PageTemplateGenerator.generateScriptTagsForPage("' +
        (packages[0] || 'pageName') +
        '") -->\n';
    } else {
  ...
  generateIcon(pageName) {
    const icons = {
      ...
      'code-quality-dashboard': 'bug.svg',
