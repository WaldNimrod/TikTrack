...
if (typeof window.UnifiedAppInitializer === 'undefined') {
  class UnifiedAppInitializer {
    ...
    async detectAndAnalyze() {
      unifiedInitDebugLog('🔍 Stage 1: Detecting and analyzing...');
      const stageStart = Date.now();

      // Detect page information
      this.pageInfo = this.detectPageInfo();
      unifiedInitDebugLog('📄 Detected page info:', this.pageInfo);

      // Detect available systems
      this.availableSystems = this.detectAvailableSystems();
      unifiedInitDebugLog('🔧 Detected available systems:', Array.from(this.availableSystems));
    ...
    prepareConfiguration() {
      unifiedInitDebugLog('⚙️ Stage 2: Preparing configuration...');
      const stageStart = Date.now();

      // Load page-specific configuration from page-initialization-configs.js
      let pageConfig = null;
      const pageName = this.pageInfo?.name || 'unknown';
      unifiedInitDebugLog('🔍 Looking for config:', pageName);
      unifiedInitDebugLog(
        '🔍 PAGE_CONFIGS exists:',
        typeof window.PAGE_CONFIGS !== 'undefined',
        'Keys:',
        window.PAGE_CONFIGS ? Object.keys(window.PAGE_CONFIGS).slice(0, 5) : 'none'
      );
    ...
    requiresCharts(pageName) {
      return (
        pageName === 'index' || document.querySelectorAll('canvas, .chart-container').length > 0
      );
    }
    ...
    async initializeCacheSystem() {
      ...
      if (typeof window.CacheSyncManager !== 'undefined') {
        try {
          ...
          window.Logger.error('❌ CacheSyncManager initialization failed:', error, {
            page: 'unified-app-initializer',
          });
        }
      } else {
      ...
      if (typeof window.CachePolicyManager !== 'undefined') {
        try {
          ...
          window.Logger.error('❌ CachePolicyManager initialization failed:', error, {
            page: 'unified-app-initializer',
          });
        }
      } else {
      ...
      if (typeof window.MemoryOptimizer !== 'undefined') {
        try {
          ...
          window.Logger.error('❌ MemoryOptimizer initialization failed:', error, {
            page: 'unified-app-initializer',
          });
        }
      } else {
    ...
    reportCacheSystemStatus() {
      ...
      window.cacheSystemReady = systems.every(
        system => system.instance && system.instance.initialized
      );
    ...
    handleError(error) {
      ...
      for (const handler of this.errorHandlers) {
        try {
          ...
          window.Logger.error('❌ Error handler failed:', handlerError, {
            page: 'unified-app-initializer',
          });
        }
      }
    ...
    showCriticalError(validationResult) {
      ...
      window.Logger.info(
        'The monitoring system has detected changes in the page loading structure.',
        { page: 'unified-app-initializer' }
      );
      window.Logger.info('', { page: 'unified-app-initializer' });
      ...
      window.Logger.info('   - Add new scripts to appropriate package in package-manifest.js', {
        page: 'unified-app-initializer',
      });
      window.Logger.info('   - Define global checks for each script', {
        page: 'unified-app-initializer',
      });
      window.Logger.info('', { page: 'unified-app-initializer' });
      ...
      window.Logger.info('   - Add required globals to the page configuration', {
        page: 'unified-app-initializer',
      });
      window.Logger.info('', { page: 'unified-app-initializer' });
      window.Logger.info('3. UPDATE ACTUAL PAGE:', { page: 'unified-app-initializer' });
      window.Logger.info('   - Add the new script tags to the HTML page', {
        page: 'unified-app-initializer',
      });
      ...
      window.Logger.info('   - Run the monitoring system to verify everything is correct', {
        page: 'unified-app-initializer',
      });
      ...
      if (validationResult.missing.length > 0) {
        ...
        validationResult.missing.forEach(m => {
          ...
          window.Logger.warn(`     מתועד בחבילה: ${m.package}`, {
            page: 'unified-app-initializer',
          });
          window.Logger.warn(`     מצפה ל-Global: ${m.global}`, {
            page: 'unified-app-initializer',
          });
          window.Logger.warn(`     תיאור: ${m.description}`, { page: 'unified-app-initializer' });
          window.Logger.warn(
            `     🤔 בחר: (1, { page: "unified-app-initializer" }) הוסף לעמוד או (2) הסר מתיעוד`
          );
        });
      }
    ...
    showDetailedErrorModal(validationResult) {
      ...
      const tryShowDetailed = () => {
        if (typeof window.showCriticalErrorModal === 'function') {
          window.Logger.info('✅ Using showCriticalErrorModal for detailed error information', {
            page: 'unified-app-initializer',
          });
          const errorInfo = {
      ...
      setTimeout(() => {
        ...
        window.Logger.info('⚠️ showCriticalErrorModal not available, using fallback', {
          page: 'unified-app-initializer',
        });
        if (typeof window.showNotification === 'function') {
    ...
    buildDetailedErrorMessage(validationResult) {
      ...
      message += '🔍 מה קרה?\n';
      message +=
        'מערכת הניטור משווה בין מה שמתועד (package-manifest.js + page-initialization-configs.js)\n';
      ...
      message +=
        '• מערכת משופרת: documentation/frontend/init-system/ENHANCED_INITIALIZATION_SYSTEM.md\n\n';
    ...
    showRealError(errorType, errorData) {
      ...
      switch (errorType) {
        case 'DUPLICATE_SCRIPT':
          window.Logger.error(`🔴 כפילות בטעינה: ${errorData.script}`, {
            page: 'unified-app-initializer',
          });
          window.Logger.error(`נטען ${errorData.count} פעמים`, { page: 'unified-app-initializer' });
          window.Logger.error('זו בעיית ביצועים חמורה - הסר את הכפילות מה-HTML', {
            page: 'unified-app-initializer',
          });
          break;

        case 'LOAD_ORDER':
          window.Logger.error(`🔴 סדר טעינה שגוי: ${errorData.script}`, {
            page: 'unified-app-initializer',
          });
          window.Logger.error(`נטען לפני: ${errorData.dependency}`, {
            page: 'unified-app-initializer',
          });
          window.Logger.error('זו שגיאה קריטית - תקן את סדר הטעינה ב-HTML', {
            page: 'unified-app-initializer',
          });
          break;

        case 'SCRIPT_FAILED':
          window.Logger.error(`🔴 סקריפט נכשל בטעינה: ${errorData.script}`, {
            page: 'unified-app-initializer',
          });
          window.Logger.error(`שגיאה: ${errorData.error}`, { page: 'unified-app-initializer' });
          window.Logger.error('בדוק את הנתיב, תחביר, או קונסול לפרטים', {
            page: 'unified-app-initializer',
          });
          break;
      }
...
document.addEventListener('DOMContentLoaded', async () => {
  unifiedInitDebugLog('🎯 DOM Content Loaded - Starting Unified App Initialization');
  window.Logger.info('🎯 DOM Content Loaded - Starting Unified App Initialization', {
    page: 'unified-app-initializer',
  });
  unifiedInitDebugLog('🔍 Current URL:', window.location.href);
  unifiedInitDebugLog('🔍 Current pathname:', window.location.pathname);
  window.Logger.info('🔍 Current URL:', window.location.href, { page: 'unified-app-initializer' });
  window.Logger.info('🔍 Current pathname:', window.location.pathname, {
    page: 'unified-app-initializer',
  });
  ...
  try {
    ...
    setTimeout(async () => {
      unifiedInitDebugLog('🚀 About to call initializeUnifiedApp...');
      window.Logger.info('🚀 About to call initializeUnifiedApp...', {
        page: 'unified-app-initializer',
      });
      await window.initializeUnifiedApp();
      window.Logger.info('✅ initializeUnifiedApp completed', {
        page: 'unified-app-initializer',
        keepInfo: true,
      });
...
async function handlePageSpecificFunctions() {
  try {
    window.Logger.info('🔧 Handling page-specific functions...', {
      page: 'unified-app-initializer',
    });
    ...
    window.Logger.info('✅ Page-specific functions handled successfully', {
      page: 'unified-app-initializer',
      keepInfo: true,
    });
  } catch (error) {
    window.Logger.error('❌ Failed to handle page-specific functions:', error, {
      page: 'unified-app-initializer',
    });
  }
}
...
window.addEventListener('error', event => {
  ...
  if (
    ...
    window.Logger.info('🔧 This error is handled by individual functions - ignoring', {
      page: 'unified-app-initializer',
    });
  ...
  async initializeCoreSystems() {
    ...
    for (const [name, initFunction] of this.coreSystems) {
      try {
        ...
        window.Logger.error(`❌ Failed to initialize ${name}:`, error, {
          page: 'unified-app-initializer',
        });
      }
    }
