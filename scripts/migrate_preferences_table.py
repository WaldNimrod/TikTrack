...
BACKUP_PREFIX = "tiktrack_backup_preferences_migration"

def create_backup():
    """יצירת גיבוי לפני המיגרציה"""
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    backup_path = DATABASE_PATH.parent / f"{BACKUP_PREFIX}_{timestamp}.db"
...
def migrate_preferences_table():
    ...
    try:
        ...
        migration_report = {
            ...
            "source_table": "user_preferences_v3",
            "target_table": "user_preferences",
            "records_copied": copied_records,
            "backup_path": backup_path,
            "status": "success"
        }
        
        with open(f"./migration_report_preferences_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json", 'w', encoding='utf-8') as f:
...
def verify_migration():
    ...
    try:
        ...
        # בדיקת דוגמה
        cursor.execute("""
            SELECT up.user_id, up.preference_id, pt.preference_name, up.saved_value
        ...
        print("📋 דוגמאות מהטבלה החדשה:")
        for sample in samples:
            print(f"  User {sample[0]}: {sample[2]} = {sample[3]}")
