...
async function runPartialMonitoring() {
    console.log('🚀 Starting PARTIAL page init/loading monitoring for first 10 pages...');
    ...
    // Save results
    const outputPath = path.join(__dirname, '..', 'documentation', '05-REPORTS', 'artifacts', '2026_01_04', 'partial_10_pages_monitoring_results.json');
    fs.writeFileSync(outputPath, JSON.stringify(allResults, null, 2));
    
    console.log('\n📊 PARTIAL SUMMARY (10 pages):');
    console.log(`✅ OK: ${allResults.summary.ok}/${TEST_PAGES.length}`);
    console.log(`❌ CRITICAL: ${allResults.summary.critical}/${TEST_PAGES.length}`);
    console.log(`🚫 NO SCRIPTS: ${allResults.summary.no_scripts}/${TEST_PAGES.length}`);
    console.log(`🌐 NAVIGATION ERROR: ${allResults.summary.navigation_error}/${TEST_PAGES.length}`);
    console.log(`\n💾 Results saved to: ${outputPath}`);
}

// Run the monitoring
runPartialMonitoring().catch(console.error);
