// Focused alerts CRUD testing with Logger+Network evidence
...
const entities = [
    {
        ...
        endpoint: '/api/alerts/',
        payload: {
            related_type_id: 4, // Assuming 4 = ticker
            related_id: 1,
            condition_attribute: 'price',
            condition_operator: 'more_than',
            condition_number: 100,
            status: 'open' // Trying 'open' as valid status value
        }
    }
];
...
async function testEntityCRUD(entity, token) {
    ...
    try {
        ...
        const updatePayload = {
            ...
            status: 'closed' // Using 'closed' which is a valid status from alert service
        };
...
// Main execution
async function main() {
    console.log('🚀 Starting Focused Alerts QA Testing');
    console.log('=====================================');
    console.log('Credentials: admin/admin123');
    console.log('Entity: alerts');
    ...
    const alerts = results[0]; // Only testing alerts
    const operations = ['create', 'read', 'update', 'delete'];
    let passCount = 0;

    operations.forEach(op => {
        const result = alerts[op];
        const status = result.pass ? '✅ PASS' : '❌ FAIL';
        console.log(`${alerts.entity} ${op.toUpperCase()}: ${status}`);
    ...
    if (passCount === 4) {
        console.log('✅ ALERTS QA: FULL PASS - Ready for production');
    } else {
        console.log('❌ ALERTS QA: PARTIAL/FAILED - Issues remain');
    }

    // Store results globally for debugging
    if (typeof window !== 'undefined') {
        window.alertsQAResults = results;
    }
