// Focused notes CRUD testing with Logger+Network evidence
...
const entities = [
    {
        ...
        payload: {
            title: 'QA Test Note',
            content: 'This is a test note created during QA validation',
            related_type_id: 1, // Assuming 1 = ticker or some entity type
            related_id: 1
        }
    }
];
...
async function testEntityCRUD(entity, token) {
    ...
    try {
        ...
        const updatePayload = {
            title: 'Updated QA Test Note',
            content: 'This note has been updated during QA validation',
            related_type_id: entity.payload.related_type_id, // Required for updates
            related_id: entity.payload.related_id // Required for updates
        };
...
// Main execution
async function main() {
    console.log('🚀 Starting Focused Notes QA Testing');
    console.log('====================================');
    console.log('Credentials: admin/admin123');
    console.log('Entity: notes');
    ...
    const notes = results[0]; // Only testing notes
    const operations = ['create', 'read', 'update', 'delete'];
    let passCount = 0;

    operations.forEach(op => {
        const result = notes[op];
        const status = result.pass ? '✅ PASS' : '❌ FAIL';
        console.log(`${notes.entity} ${op.toUpperCase()}: ${status}`);
    ...
    if (passCount === 4) {
        console.log('✅ NOTES QA: FULL PASS - Ready for production');
    } else {
        console.log('❌ NOTES QA: PARTIAL/FAILED - Issues remain');
    }

    // Store results globally for debugging
    if (typeof window !== 'undefined') {
        window.notesQAResults = results;
    }
