...
async function updateAddTickerInfo() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      // צבע השינוי
      if (dailyChange > 0) {
        changeDisplay.style.color = '#28a745'; // ירוק
        changeDisplay.style.fontWeight = 'bold';
      } else if (dailyChange < 0) {
        changeDisplay.style.color = '#dc3545'; // אדום
        changeDisplay.style.fontWeight = 'bold';
      } else {
        changeDisplay.style.color = '#6c757d'; // אפור
      }
...
window.enableAddFormFields = enableAddFormFields;
window.disableAddFormFields = disableAddFormFields;
...
/**
 * חישוב מחירי עצירה ויעד ברירת מחדל
 */
...
function updateSharesFromAmount() {
  ...
  if (amountInput && sharesInput && priceDisplay) {
    ...
    if (amountInput.value && priceDisplay.textContent !== '-' && priceDisplay.textContent !== '') {
      ...
      if (price > 0) {
        ...
        if (typeof window.convertAmountToShares === 'function') {
          ...
          console.warn('פונקציית המרת סכום למניות לא נמצאה');
          // fallback
    ...
    console.warn('אלמנטים נדרשים לא נמצאו לעדכון מניות');
  }
}
...
function updateAmountFromShares() {
  ...
  if (sharesInput && amountInput && priceDisplay) {
    if (sharesInput.value && priceDisplay.textContent !== '-') {
      ...
      if (price > 0) {
        ...
        if (typeof window.convertSharesToAmount === 'function') {
          ...
          console.warn('פונקציית המרת מניות לסכום לא נמצאה');
          // fallback
    ...
    console.warn('אלמנטים נדרשים לא נמצאו לעדכון סכום');
  }
}
