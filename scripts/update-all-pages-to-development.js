#!/usr/bin/env node
/**
 * Update All Pages to Development Mode (No Bundles)
 * 
 * This script updates all HTML pages to use individual scripts in development mode.
 */
...
function updatePage(pageName) {
    ...
    try {
        ...
        const backupPath = `${htmlPath}.backup-dev-${Date.now()}`;
        fs.copyFileSync(htmlPath, backupPath);
        
        // Generate new script loading code in development mode
        const newScripts = execSync(
            `node "${GENERATE_SCRIPT}" ${pageName} --mode=development --no-bundles`,
...
function main() {
    ...
    console.log('🛠️  Updating All Pages to Development Mode (No Bundles)');
    console.log('='.repeat(80));
    console.log();
    ...
    console.log('='.repeat(80));
    console.log();
    console.log('📝 All pages are now in DEVELOPMENT mode (no bundles)');
    console.log('📝 Individual scripts will be loaded for easier debugging');
