...
class ExternalDataTester {
    constructor() {
        ...
        this.tickerNamesCache = {
            // Known ticker names as fallback
            'AAPL': 'Apple Inc.',
            'GOOGL': 'Alphabet Inc.',
    ...
    initializeEventListeners() {
        ...
        // Load database tickers
        document.getElementById('load-db-tickers').addEventListener('click', () => {
            this.loadDatabaseTickers();
        });
     ...
     */
    async loadDatabaseTickers() {
        this.showLoading('טוען טיקרים מבסיס הנתונים...');
        this.log('info', 'טוען טיקרים מבסיס הנתונים');

        try {
            // Real API call to get tickers from database
            ...
            // Update cache with ticker names
            tickers.forEach(ticker => {
                this.tickerNamesCache[ticker.symbol] = ticker.name || 'ללא שם';
            });
            
            // Update select dropdown
            const select = document.getElementById('db-tickers-select');
            ...
            this.log('success', `${tickers.length} טיקרים נטענו מבסיס הנתונים`);
            
            // Update test symbols input with first few tickers
            if (tickers.length > 0) {
                const symbols = tickers.slice(0, 5).map(t => t.symbol).join(',');
                document.getElementById('test-symbols').value = symbols;
            }
            
        } catch (error) {
            this.log('error', `שגיאה בטעינת טיקרים: ${error.message}`);
            this.showError('שגיאה בטעינת טיקרים מבסיס הנתונים');
        } finally {
            this.hideLoading();
        }
    }
    ...
    async testDatabaseConnection() {
        this.showLoading('בודק חיבור לבסיס הנתונים...');
        this.log('info', 'בודק חיבור לבסיס הנתונים');
        ...
        try {
            ...
            for (const endpoint of endpoints) {
                try {
                    ...
                    if (response.ok) {
                        ...
                        results[endpoint].count = Array.isArray(data) ? data.length : 1;
                    }
                } catch (error) {
                    results[endpoint] = {
                        status: 'error',
            ...
            this.displayDatabaseStatus(results);
            this.log('success', 'בדיקת חיבור לבסיס הנתונים הושלמה');
            
        } catch (error) {
            this.log('error', `שגיאה בבדיקת חיבור לבסיס הנתונים: ${error.message}`);
            this.showError('שגיאה בבדיקת חיבור לבסיס הנתונים');
        } finally {
            this.hideLoading();
        }
    }
            ...
            <div class="quote-item fade-in">
                <div class="quote-header">
                    <div class="quote-symbol">
                        ...
                        <br>
                        <small class="text-muted">${this.getTickerName(quote.symbol)}</small>
                    </div>
     ...
     */
    displayDatabaseStatus(results) {
        const card = document.getElementById('system-status-card');
        const content = document.getElementById('system-status-content');
        
        let statusHtml = `
            <div class="fade-in">
                <h6>סטטוס חיבור לבסיס הנתונים:</h6>
                <div class="row">
        `;
        ...
        Object.entries(results).forEach(([endpoint, result]) => {
            ...
            const statusText = result.ok ? 'מחובר' : 'שגיאה';
            
            statusHtml += `
                <div class="col-md-4 mb-3">
                    <div class="card bg-light">
                        <div class="card-body text-center">
                            <h6 class="card-title">${endpointName}</h6>
                            <p class="card-text">
                                <span class="badge bg-${statusClass}">${statusText}</span>
                            </p>
                            ${result.ok ? `
                                <small class="text-muted">
                                    סטטוס: ${result.status} | רשומות: ${result.count}
                                </small>
                            ` : `
                                <small class="text-danger">
                                    שגיאה: ${result.error || 'לא ידוע'}
                                </small>
                            `}
                        </div>
                    </div>
    ...
    getTickerName(symbol) {
        return this.tickerNamesCache[symbol] || 'שם לא ידוע';
    }
