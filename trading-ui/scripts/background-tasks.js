...
const utils = {
  ...
  showNotification: (message, type = 'info') => {
    ...
    if (typeof window.showNotification === 'function') {
      ...
      // Fallback למקרה שמערכת התראות לא זמינה - console removed for no-console compliance
      // Message would be: ${type.toUpperCase()}: ${message}
    }
  },
...
const apiService = {
  ...
  async request(url, options = {}) {
    try {
      ...
      // API Request URL logged - console removed for no-console compliance
      ...
      // API request failed - console removed for no-console compliance
...
const uiManager = {
  /**
     ...
     */
  updateStatus(status) {
    // Updating UI status - console removed for no-console compliance
  ...
  updateSchedulerButtons(isRunning) {
    // Updating scheduler buttons - console removed for no-console compliance
    ...
    if (startBtn && stopBtn) {
      if (isRunning) {
        ...
        // Scheduler running - showing stop button
      } else {
        ...
        // Scheduler stopped - showing start button
      }
    }
  },
...
const eventHandlers = {
  /**
     ...
     */
  init() {
    // EventHandlers init - fixing button connections v20250903-2050
    ...
    // Button check - console removed for no-console compliance
    // Object would contain: startBtn: !!startBtn, stopBtn: !!stopBtn, refreshBtn: !!refreshBtn
