...
function restoreDbExtradataSectionsState() {
  try {
    ...
    // Warning:('⚠️ Error restoring sections state:', error);
  }
}
...
function editCurrencyRecord(id) {
  ...
  if (id === 1) {
    if (typeof window.showWarningNotification === 'function') {
      ...
      // Warning:('לא ניתן לערוך רשומת בסיס מוגנת');
    }
    return;
  }
...
function deleteCurrencyRecord(id) {
  ...
  if (id === 1) {
    if (typeof window.showWarningNotification === 'function') {
      ...
      // Warning:('לא ניתן למחוק רשומת בסיס מוגנת');
    }
    return;
  }
...
function initializeRealTimeValidation() {
  ...
  if (window.setupFieldValidation) {
    ...
    // Warning:('⚠️ Validation system not available');
  }
}
