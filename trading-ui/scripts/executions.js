...
/**
 * פתיחת מודול ייבוא נתוני משתמש
 * פותח את מודול הייבוא בתוך העמוד
 */
function openImportUserDataModal() {
    try {
        window.Logger.info('📥 פותח מודול ייבוא נתוני משתמש', { page: "executions" });
        
        // הצגת המודול
        const modal = document.getElementById('import-user-data-modal');
        if (modal) {
            modal.style.display = 'block';
            
            // אתחול המודול - קריאה לפונקציה מהקובץ הנפרד
            if (typeof window.initializeImportModal === 'function') {
                window.initializeImportModal();
            } else {
                window.Logger.error('פונקציית initializeImportModal לא זמינה', { page: "executions" });
            }
        } else {
            window.Logger.error('מודול ייבוא לא נמצא', { page: "executions" });
        }
        
    } catch (error) {
        window.Logger.error('שגיאה בפתיחת מודול הייבוא:', error, { page: "executions" });
        if (typeof window.showErrorNotification === 'function') {
            window.showErrorNotification('שגיאה בפתיחת מודול הייבוא', error.message);
        } else if (typeof window.showNotification === 'function') {
            window.showNotification('שגיאה בפתיחת מודול הייבוא', 'error');
        } else {
            alert('שגיאה בפתיחת מודול הייבוא: ' + error.message);
        }
    }
}

/**
 * סגירת מודול ייבוא נתוני משתמש
 */
function closeImportUserDataModal() {
    try {
        ...
        if (modal) {
            ...
            // איפוס המודול - קריאה לפונקציה מהקובץ הנפרד
            if (typeof window.resetImportModal === 'function') {
                window.resetImportModal();
            }
        }
    } catch (error) {
        window.Logger.error('שגיאה בסגירת מודול הייבוא:', error, { page: "executions" });
    }
}
