...
async function loadTickersWithActiveTrades() {
    console.log('🔄 Loading tickers with active trades...');
    ...
    try {
        ...
        // סינון טיקרים שיש להם טריידים פעילים
        ...
        console.error('❌ Error loading tickers with active trades:', error);
    }
}

/**
 * טעינת טריידים פעילים לטיקר שנבחר
 * @param {string} mode - 'add' או 'edit'
 */
async function loadActiveTradesForTicker(mode = 'add') {
    console.log('🔄 Loading active trades for ticker, mode:', mode);
    ...
    try {
        ...
        // סינון טריידים פעילים לטיקר שנבחר
        const activeTrades = trades.filter(trade =>
            trade.ticker_id == tickerId &&
            trade.status === 'active'
        );

        console.log('✅ Found', activeTrades.length, 'active trades for ticker', tickerId);
        ...
        if (tradeSelect) {
            ...
            activeTrades.forEach(trade => {
                ...
                option.textContent = `טרייד ${trade.id} - ${trade.side} ${trade.investment_type}`;
        ...
        console.error('❌ Error loading active trades for ticker:', error);
    }
}
