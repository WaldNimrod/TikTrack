...
// ===== PROBLEM RESOLUTION FUNCTIONS - DETAILED VIEW =====

/**
 * Display detailed problem resolution with individual cards for each issue
...
function displayProblemResolutionDetailed() {
    ...
    // Show the problem resolution step
    document.getElementById('step-problems').style.display = 'block';
}

/**
 * Display missing tickers as detailed cards
 * 
 * @function displayMissingTickersDetailed
 * @param {Array} missingTickers - Array of missing ticker symbols
 * @returns {void}
 */
function displayMissingTickersDetailed(missingTickers) {
    const section = document.getElementById('missingTickersSection');
    const container = document.getElementById('missingTickersList');
    
    // Create cards container
    const cardsContainer = document.createElement('div');
    cardsContainer.className = 'problem-cards-container';
    
    missingTickers.forEach(ticker => {
        const card = renderMissingTickerCard(ticker);
...
/**
 * Render a missing ticker card
 * 
 * @function renderMissingTickerCard
 * @param {string} ticker - Ticker symbol
 * @returns {HTMLElement} Card element
 */
function renderMissingTickerCard(ticker) {
    const card = document.createElement('div');
    card.className = 'problem-card missing-ticker-card';
    card.innerHTML = `
        <div class="problem-card-header">
            <i class="bi bi-question-circle"></i>
            <span>טיקר חסר: ${ticker}</span>
        </div>
        <div class="problem-card-body">
            <div class="missing-ticker-info">
                <i class="bi bi-info-circle"></i>
                הטיקר ${ticker} לא קיים במערכת ויושמט מהייבוא
            </div>
            <div class="problem-card-actions">
                <button data-button-type="PRIMARY" data-text="הוסף טיקר" 
                        onclick="openAddTickerModal('${ticker}')"></button>
            </div>
        </div>
    `;
    return card;
...
/**
 * Display duplicates as detailed cards
 * 
 * @function displayDuplicatesDetailed
 * @param {Object} previewData - Preview data containing duplicate information
 * @returns {void}
 */
function displayDuplicatesDetailed(previewData) {
    // Within-file duplicates
    const withinFileDuplicates = previewData.records_to_skip.filter(r => r.reason === 'within_file_duplicate');
    if (withinFileDuplicates.length > 0) {
        displayDuplicateCards('duplicatesSection', 'duplicatesList', withinFileDuplicates, 'within_file');
    }
    
    // Existing records
    const existingRecords = previewData.records_to_skip.filter(r => r.reason === 'existing_record');
    if (existingRecords.length > 0) {
        displayDuplicateCards('existingRecordsSection', 'existingRecordsList', existingRecords, 'existing_record');
    }
}

/**
 * Display duplicate cards for a specific type
 * 
 * @function displayDuplicateCards
 * @param {string} sectionId - Section ID to show
 * @param {string} containerId - Container ID to populate
 * @param {Array} duplicates - Array of duplicate records
 * @param {string} type - Type of duplicates ('within_file' or 'existing_record')
 * @returns {void}
 */
function displayDuplicateCards(sectionId, containerId, duplicates, type) {
    ...
    duplicates.forEach((duplicate, index) => {
        const card = renderDuplicateCard(duplicate, type, index);
        cardsContainer.appendChild(card);
    });
 ...
 * @param {Object} duplicate - Duplicate record data
 * @param {string} type - Type of duplicate ('within_file' or 'existing_record')
 * @param {number} index - Index of the duplicate
 * @returns {HTMLElement} Card element
 */
function renderDuplicateCard(duplicate, type, index) {
    const card = document.createElement('div');
    const cardClass = type === 'within_file' ? 'within-file-duplicate' : 'existing-record';
    const icon = type === 'within_file' ? 'bi-files' : 'bi-exclamation-triangle';
    const title = type === 'within_file' ? 'כפילות בקובץ' : 'רשומה קיימת';
    
    card.className = `problem-card ${cardClass}`;
    card.innerHTML = `
        <div class="problem-card-header">
            <i class="bi ${icon}"></i>
            <span>${title}</span>
        ...
        <div class="problem-card-body">
            <div class="problem-card-details">
                <div class="problem-card-detail">
                    <span class="problem-card-detail-label">סמל:</span>
                    <span class="problem-card-detail-value">${duplicate.symbol}</span>
                </div>
                <div class="problem-card-detail">
                    <span class="problem-card-detail-label">פעולה:</span>
                    <span class="problem-card-detail-value">${duplicate.action === 'buy' ? 'קניה' : 'מכירה'}</span>
                </div>
                <div class="problem-card-detail">
                    <span class="problem-card-detail-label">כמות:</span>
                    <span class="problem-card-detail-value">${duplicate.quantity}</span>
                </div>
                <div class="problem-card-detail">
                    <span class="problem-card-detail-label">מחיר:</span>
                    <span class="problem-card-detail-value">$${duplicate.price}</span>
                </div>
                <div class="problem-card-detail">
                    <span class="problem-card-detail-label">תאריך:</span>
                    <span class="problem-card-detail-value">${formatDate(duplicate.date)}</span>
                </div>
                <div class="problem-card-detail">
                    <span class="problem-card-detail-label">עמלה:</span>
                    <span class="problem-card-detail-value">$${duplicate.fee || '0'}</span>
                </div>
            </div>
            
            ${duplicate.confidence_score ? `
            <div class="problem-card-confidence">
                <span class="confidence-text">ביטחון:</span>
                <div class="confidence-bar ${getConfidenceClass(duplicate.confidence_score)}">
                    <div class="confidence-fill" style="width: ${duplicate.confidence_score}%"></div>
                </div>
                <span class="confidence-text">${duplicate.confidence_score}%</span>
            </div>
            ` : ''}
            ...
            <div class="duplicate-matches">
                <i class="bi bi-info-circle"></i>
                נמצאו ${duplicate.match_count || 1} רשומות דומות בקובץ
            </div>
            ` : `
            <div class="existing-record-info">
                <i class="bi bi-info-circle"></i>
                רשומה דומה קיימת כבר במערכת
            </div>
            `}
            
            <div class="problem-card-actions">
                <button data-button-type="SUCCESS" data-text="קבל" 
                        onclick="acceptDuplicate(${index}, '${type}')"></button>
                <button data-button-type="DANGER" data-text="דחה" 
                        onclick="rejectDuplicate(${index}, '${type}')"></button>
            </div>
        </div>
    `;
    return card;
...
/**
 * Accept a duplicate record for import
 * 
 ...
 * @returns {void}
 */
async function acceptDuplicate(index, type) {
    try {
        const response = await fetch(`/api/user-data-import/session/${ImportState.sessionId}/accept-duplicate`, {
            method: 'POST',
        ...
        if (response.ok) {
            showNotification('הרשומה אושרה לייבוא', 'success');
            // Refresh preview data
            await refreshPreviewData();
            // Update display
            displayProblemResolutionDetailed();
        } else {
            showNotification('שגיאה באישור הרשומה', 'error');
        }
        } catch (error) {
 ...
 * @returns {void}
 */
async function rejectDuplicate(index, type) {
    try {
        const response = await fetch(`/api/user-data-import/session/${ImportState.sessionId}/reject-duplicate`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ 
                record_index: index,
        ...
        if (response.ok) {
            showNotification('הרשומה נדחתה', 'info');
            // Refresh preview data
            await refreshPreviewData();
            // Update display
            displayProblemResolutionDetailed();
        } else {
            showNotification('שגיאה בדחיית הרשומה', 'error');
        }
    } catch (error) {
...
/**
 * Open add ticker modal with pre-filled symbol
 * 
 * @function openAddTickerModal
 * @param {string} symbol - Ticker symbol to pre-fill
 * @returns {void}
 */
function openAddTickerModal(symbol) {
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="form-group">
                        ...
                        <input type="text" id="tickerSymbol" class="form-control" placeholder="לדוגמה: AAPL">
                    </div>
                    <div class="form-group">
                        <label for="tickerName">שם החברה:</label>
                        <input type="text" id="tickerName" class="form-control" placeholder="לדוגמה: Apple Inc.">
                    </div>
                    <div class="form-group">
                        <label for="tickerExchange">בורסה:</label>
                        <select id="tickerExchange" class="form-select">
                ...
                <div class="modal-footer">
                    <button data-button-type="SECONDARY" data-text="ביטול" onclick="closeAddTickerModal()"></button>
                    <button data-button-type="PRIMARY" data-text="שמור טיקר" onclick="saveTickerFromModal()"></button>
                </div>
            </div>
        `;
    ...
    // Pre-fill symbol
    document.getElementById('tickerSymbol').value = symbol;
 ...
 * 
 * @function saveTickerFromModal
 * @returns {void}
 */
async function saveTickerFromModal() {
    ...
    const exchange = document.getElementById('tickerExchange').value;
    
    if (!symbol) {
        showNotification('יש להזין סמל טיקר', 'error');
        return;
    }
    
    try {
        // Use existing saveTicker function from ticker service
        ...
        if (result.success) {
            ...
            // Refresh preview data to include the new ticker
            await refreshPreviewData();
            
            // Update problem resolution display
            displayProblemResolutionDetailed();
            
            // Call callback if exists
            if (typeof onTickerAdded === 'function') {
                onTickerAdded(symbol);
            }
        } else {
            showNotification(result.error || 'שגיאה בשמירת הטיקר', 'error');
        }
        } catch (error) {
        console.error('Error saving ticker:', error);
        showNotification('שגיאה בשמירת הטיקר', 'error');
    }
}

/**
 * Callback after ticker is added
 * 
 * @function onTickerAdded
 * @param {string} symbol - Added ticker symbol
 * @returns {void}
 */
function onTickerAdded(symbol) {
    console.log(`Ticker ${symbol} added successfully`);
    // Update any relevant UI elements
    updateMissingTickersCount();
}
...
/**
 * Refresh preview data from server
 * 
...
/**
 * Update missing tickers count in analysis cards
 * 
 * @function updateMissingTickersCount
 * @returns {void}
 */
function updateMissingTickersCount() {
    if (ImportState.analysisResults && ImportState.analysisResults.missing_tickers) {
        const count = ImportState.analysisResults.missing_tickers.length;
        const element = document.getElementById('missingTickers');
        if (element) {
