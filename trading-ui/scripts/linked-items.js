...
function viewLinkedItems(itemId, itemType = null) {
  ...
  loadLinkedItemsData(itemType, itemId).then(data => {
    if (data) {
      showLinkedItemsModal(data, itemType, itemId, 'view');
    }
  });
}
  ...
  let content = `
    <div class="linked-items-container">
      <style>
        ...
        .modal-header {
          background: linear-gradient(135deg, ${mode === 'warningBlock' ? '#dc3545, #c82333' : '#28a745, #20c997'});
          color: white;
        ...
        .modal-header .btn-close-custom {
          ...
          color: ${mode === 'warningBlock' ? '#dc3545' : '#28a745'};
          border: 2px solid ${mode === 'warningBlock' ? '#dc3545' : '#28a745'};
          border-radius: 6px;
          padding: 6px 12px;
          font-size: 14px;
          font-weight: bold;
        ...
        .modal-header .btn-close-custom:hover {
          background-color: ${mode === 'warningBlock' ? '#dc3545' : '#28a745'};
          color: white;
        }
      ...
      <div class="alert alert-info">
        ...
        ${mode === 'warningBlock' ? '<br><strong style="color: #dc3545;">⚠️ לא ניתן לבצע פעולה זו עד לטיפול בפריטים המקושרים</strong>' : ''}
      </div>
