...
function initializeApplication() {
  try {
    ...
    // // console.error('❌ Application initialization error:', error); // Disabled for linting
...
function checkDependencies() {
  ...
  if (missingModules.length > 0) {
    // // console.warn('⚠️ Missing modules:', missingModules); // Disabled for linting
...
function initializeCoreSystems() {
  ...
  if (window.NotificationsCenter && !window.notificationsCenter && !window.location.pathname.includes('notifications-center')) {
    try {
      ...
      // // console.warn('⚠️ שגיאה באתחול מרכז התראות:', error); // Disabled for linting
    }
  }
...
function initializeCurrentPage() {
  ...
  setTimeout(() => {
    if (typeof window.restoreAllSectionStates === 'function') {
      ...
      // // console.warn('⚠️ restoreAllSectionStates function not found'); // Disabled for linting
    }
  }, 100);
}
...
function showSystemError(message) {
  if (typeof window.showErrorNotification === 'function') {
    ...
    // // console.error('❌ שגיאת מערכת:', message); // Disabled for linting
  }
}
...
function restoreAllSectionStates() {
  ...
  try {
    ...
    if (topSection && topIcon) {
      ...
      // // console.warn('⚠️ Could not restore top section state - elements not found'); // Disabled for linting
    }
...
function initializeDevelopmentShortcuts() {
  ...
  document.addEventListener('keydown', function(e) {
    if ((e.ctrlKey || e.metaKey) && e.shiftKey &&
            e.key === 'C') {
      e.preventDefault();
      // // console.log('🔧 קיצור מקלדת: ניקוי Cache'); // Disabled for linting
...
window.updateSortIcons = function (tableType, activeColumnIndex, direction) {
  try {
    ...
    if (!table) {
      // // console.warn(`⚠️ Table with type "${tableType}" not found`); // Disabled for linting
      return;
    }
    ...
    // // console.warn('⚠️ Error updating sort icons:', error); // Disabled for linting
  }
};
