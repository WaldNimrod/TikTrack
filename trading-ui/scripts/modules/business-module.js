...
function updateTradesTable(trades) {
  ...
  const tableHTML = trades.map(trade => {
    ...
    <tr>
      ...
      <td data-account="${trade.account_id || trade.account_name || ''}"><strong><a href="#" onclick="viewAccountDetails('${trade.account_id}')" class="account-link">${trade.account_name || trade.account_id || 'חשבון מסחר לא ידוע'}</a></strong></td>
      <td>${trade.notes || ''}</td>
...
async function checkLinkedItemsBeforeDelete(tradeId) {
  try {
    ...
    // parent entities הם פריטים שהטרייד מקושר אליהם (חשבון מסחר, טיקר, תוכנית) - לא רלוונטי למחיקה
...
async function checkLinkedItemsBeforeCancel(tradeId) {
  try {
    ...
    // בדיקה רק אם יש פריטים שמקושרים אל הטרייד (child entities)
    // parent entities הם פריטים שהטרייד מקושר אליהם (חשבון מסחר, טיקר, תוכנית) - לא רלוונטי לביטול
    if (childEntities.length > 0) {
      // יש פריטים מקושרים - הצגת חלון מקושרים
      if (typeof window.showLinkedItemsModal === 'function') {
        window.showLinkedItemsModal(linkedItemsData, 'trade', tradeId);
