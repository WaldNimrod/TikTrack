...
class PageScriptsMatrixSystem {
    ...
    async loadScanResults() {
        try {
            ...
            if (response.ok) {
                ...
                console.warn('⚠️ Failed to load scan results:', response.status);
            }
        } catch (error) {
    ...
    async loadRealSystemStats() {
        try {
            ...
            const filesData = filesResponse.ok ? await filesResponse.json() : { data: { js: [] } };
            const files = filesData.data.js || [];
...
function generateDetailedLog() {
    ...
    log.push(`עמודים במערכת: ${totalPages}`);
    log.push(`קבצי JavaScript: ${totalScripts}`);
    log.push(`עדכון אחרון: ${lastUpdate}`);
    
    // בדיקת נתונים מהמערכת
    if (window.pageScriptsMatrixSystem) {
        const system = window.pageScriptsMatrixSystem;
        log.push(`נתוני סריקה: ${system.scanData ? 'נטענו' : 'לא נטענו'}`);
        log.push(`סטטיסטיקות מערכת: ${system.systemStats ? 'נטענו' : 'לא נטענו'}`);
        if (system.scanData) {
            const pagesCount = Object.keys(system.scanData.matrix || {}).length;
            log.push(`עמודים שנסרקו: ${pagesCount}`);
        }
    }
    ...
    // אלמנטים פעילים/גלויים
    log.push('--- אלמנטים פעילים/גלויים ---');
    const jsFilesContent = document.getElementById('jsFilesContent');
    ...
    const containers = [
        { name: 'ניהול קבצי JavaScript', element: jsFilesContent },
        { name: 'ניתוח תלויות', element: dependenciesContent },
        { name: 'ניהול אחסון נתונים', element: storageContent },
        { name: 'בדיקת ארכיטקטורה', element: architectureContent },
        { name: 'סטטוס אינטגרציה', element: integrationContent },
        { name: 'מיפוי עמודים לקבצים', element: pageMappingContent }
    ];
    
    containers.forEach(container => {
        const visible = container.element && container.element.offsetParent !== null ? 'נראה' : 'לא נראה';
        const hasContent = container.element && container.element.innerHTML.trim() !== '' ? 'יש תוכן' : 'אין תוכן';
        log.push(`${container.name}: ${visible} - ${hasContent}`);
    });

    // היסטוריה/נתונים בטבלאות
    log.push('--- טבלאות ונתונים ---');
    const pageRows = document.querySelectorAll('.page-row');
    if (pageRows.length > 0) {
        log.push(`מספר שורות בטבלה: ${pageRows.length}`);
        pageRows.forEach((row, index) => {
            ...
            log.push(`  ${index + 1}. ${pageName} | קבצי JS: ${jsFiles} | סטטוס: ${status}`);
        });
    } else {
        log.push('אין טבלאות נתונים זמינות');
    }
    ...
    buttons.forEach(btn => {
        const element = document.querySelector(`[onclick*="${btn.id}"]`);
        const visible = element && element.offsetParent !== null ? 'נראה' : 'לא נראה';
        const disabled = element ? (element.disabled ? 'מבוטל' : 'פעיל') : 'לא קיים';
        log.push(`${btn.text}: ${visible} - ${disabled}`);
    });

    // סטטוס חיבור/מערכת
    log.push('--- סטטוס חיבור/מערכת ---');
    if (window.pageScriptsMatrix) {
        log.push('מערכת Page Scripts Matrix: פעילה');
        log.push(`נתוני סריקה: ${window.pageScriptsMatrix.scanData ? 'נטענו' : 'לא נטענו'}`);
        log.push(`סטטיסטיקות מערכת: ${window.pageScriptsMatrix.systemStats ? 'נטענו' : 'לא נטענו'}`);
    } else {
        log.push('מערכת Page Scripts Matrix: לא פעילה');
    }
    ...
    if (performance.timing) {
        ...
        log.push('זמן טעינת עמוד: לא זמין');
    }
