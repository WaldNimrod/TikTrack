...
class SystemManagement {
  ...
  static initializeDashboard() {
    ...
    // Set page title (only for system-management page)
    if (window.location.pathname === '/system-management.html' || window.location.pathname.includes('system-management')) {
      document.title = 'מנהל מערכת - TikTrack';
    } else {
      // Don't override other pages' titles
      console.log('🔧 Not overriding page title for:', window.location.pathname, 'Current title:', document.title);
    }
  }
  ...
  async updateAlerts() {
    try {
      // Get alerts from unified cache system
      let globalHistory = [];
      if (window.UnifiedCacheManager && window.UnifiedCacheManager.isInitialized()) {
        try {
          globalHistory = await window.UnifiedCacheManager.get('globalNotificationHistory') || [];
        } catch (error) {
          globalHistory = JSON.parse(localStorage.getItem('globalNotificationHistory') || '[]'); // fallback
        }
      } else {
        globalHistory = JSON.parse(localStorage.getItem('globalNotificationHistory') || '[]'); // fallback
      }
...
function generateDetailedLog() {
    ...
    log.push('=== לוג מפורט - ניהול מערכת ===');
    ...
    // 2. סטטיסטיקות מערכת
    log.push('');
    log.push('--- סטטיסטיקות מערכת ---');
    const systemStats = [
        'overallHealthStatus', 'systemScore', 'responseTime', 'uptime',
        'serverHealthStats', 'databaseHealthStats', 'cacheHealthStats', 'networkHealthStats',
        'cpuUsage', 'memoryUsage', 'diskUsage', 'networkStatus',
        'activeProvidersCount', 'lastDataUpdate', 'dataAccuracy', 'cacheStatus',
        'errorCount', 'warningCount', 'infoCount', 'totalAlerts'
    ];
    
    systemStats.forEach(statId => {
    ...
    // 4. תוצאות בדיקות מערכת
    log.push('');
    log.push('--- תוצאות בדיקות מערכת ---');
    const checkResults = document.querySelectorAll('.check-result, .system-check-item, .health-check-item');
    log.push(`מספר בדיקות: ${checkResults.length}`);
    checkResults.forEach((result, index) => {
        const status = result.querySelector('.status, .badge, .status-value')?.textContent.trim() || 'לא ידוע';
        const description = result.querySelector('.description, .text, .check-name')?.textContent.trim() || 'ללא תיאור';
        log.push(`  ${index + 1}. ${status}: ${description}`);
    });

    // 5. רשימת ספקים
    log.push('');
    log.push('--- רשימת ספקים ---');
    const providerList = document.getElementById('providerList');
    if (providerList) {
        const providers = providerList.querySelectorAll('.provider-item, .provider-card');
        log.push(`מספר ספקים: ${providers.length}`);
        providers.forEach((provider, index) => {
            const name = provider.querySelector('.provider-name, .card-title')?.textContent.trim() || 'ללא שם';
            const status = provider.querySelector('.provider-status, .badge')?.textContent.trim() || 'לא ידוע';
            log.push(`  ${index + 1}. ${name}: ${status}`);
        });
    }
...
/**
 * Setup Cursor Tasks event listeners
 * הגדרת מאזינים ל-Cursor Tasks
 */
SystemManagement.setupCursorTasksListeners = function() {
  // Quick Actions
  const quickStartSystemBtn = document.getElementById('quickStartSystemBtn');
  if (quickStartSystemBtn) {
    quickStartSystemBtn.addEventListener('click', () => SystemManagement.executeQuickStart());
  }

  const quickRestartSystemBtn = document.getElementById('quickRestartSystemBtn');
  if (quickRestartSystemBtn) {
    quickRestartSystemBtn.addEventListener('click', () => SystemManagement.executeQuickRestart());
  }

  const quickRestartSystemBtn2 = document.getElementById('quickRestartSystemBtn2');
  if (quickRestartSystemBtn2) {
    quickRestartSystemBtn2.addEventListener('click', () => SystemManagement.executeQuickRestart());
  }

  const quickStopSystemBtn = document.getElementById('quickStopSystemBtn');
  if (quickStopSystemBtn) {
    quickStopSystemBtn.addEventListener('click', () => SystemManagement.executeQuickStop());
  }

  // Cache Mode Buttons
  const devModeSystemBtn = document.getElementById('devModeSystemBtn');
  if (devModeSystemBtn) {
    devModeSystemBtn.addEventListener('click', () => SystemManagement.executeCacheMode('development'));
  }

  const noCacheSystemBtn = document.getElementById('noCacheSystemBtn');
  if (noCacheSystemBtn) {
    noCacheSystemBtn.addEventListener('click', () => SystemManagement.executeCacheMode('no-cache'));
  }

  const productionModeSystemBtn = document.getElementById('productionModeSystemBtn');
  if (productionModeSystemBtn) {
    productionModeSystemBtn.addEventListener('click', () => SystemManagement.executeCacheMode('production'));
  }

  // Dashboard Buttons
  const openServerMonitorSystemBtn = document.getElementById('openServerMonitorSystemBtn');
  if (openServerMonitorSystemBtn) {
    openServerMonitorSystemBtn.addEventListener('click', () => SystemManagement.openServerMonitor());
  }

  const openAllDashboardsSystemBtn = document.getElementById('openAllDashboardsSystemBtn');
  if (openAllDashboardsSystemBtn) {
    openAllDashboardsSystemBtn.addEventListener('click', () => SystemManagement.openAllDashboards());
  }

  const openExternalDataBtn = document.getElementById('openExternalDataBtn');
  if (openExternalDataBtn) {
    openExternalDataBtn.addEventListener('click', () => SystemManagement.openExternalData());
  }

  const openCrudTestingBtn = document.getElementById('openCrudTestingBtn');
  if (openCrudTestingBtn) {
    openCrudTestingBtn.addEventListener('click', () => SystemManagement.openCrudTesting());
  }
 ...
 * ביצוע התחלה מהירה
 */
SystemManagement.executeQuickStart = async function() {
  try {
    SystemManagement.showNotification('🚀 מבצע התחלה מהירה...', 'info');
    ...
    if (response.ok) {
      SystemManagement.showNotification('✅ שרת הותח בהצלחה', 'success');
      setTimeout(() => SystemManagement.refreshSystemData(), 2000);
    } else {
      throw new Error(`HTTP ${response.status}`);
    }
  } catch (error) {
    SystemManagement.showNotification(`❌ שגיאה בהתחלת שרת: ${error.message}`, 'error');
  }
};
 ...
 * ביצוע איתחול מהיר
 */
SystemManagement.executeQuickRestart = async function() {
  try {
    SystemManagement.showNotification('🔄 מבצע איתחול מהיר...', 'info');
    ...
    if (response.ok) {
      SystemManagement.showNotification('✅ שרת אותחל בהצלחה', 'success');
      setTimeout(() => SystemManagement.refreshSystemData(), 3000);
    } else {
      throw new Error(`HTTP ${response.status}`);
    }
  } catch (error) {
    SystemManagement.showNotification(`❌ שגיאה באיתחול שרת: ${error.message}`, 'error');
  }
};
 ...
 * ביצוע עצירה מהירה
 */
SystemManagement.executeQuickStop = async function() {
  try {
    SystemManagement.showNotification('🛑 מבצע עצירה מהירה...', 'info');
    ...
    if (response.ok) {
      SystemManagement.showNotification('✅ שרת נעצר בהצלחה', 'success');
      setTimeout(() => SystemManagement.refreshSystemData(), 1000);
    } else {
      throw new Error(`HTTP ${response.status}`);
    }
  } catch (error) {
    SystemManagement.showNotification(`❌ שגיאה בעצירת שרת: ${error.message}`, 'error');
  }
};
 ...
 */
SystemManagement.executeCacheMode = async function(mode) {
  try {
    SystemManagement.showNotification(`🎛️ משנה מצב מטמון ל-${mode}...`, 'info');
    ...
    if (response.ok) {
      SystemManagement.showNotification(`✅ מצב מטמון שונה ל-${mode}`, 'success');
      SystemManagement.updateSystemStatus(mode);
      setTimeout(() => SystemManagement.refreshSystemData(), 3000);
    } else {
      throw new Error(`HTTP ${response.status}`);
    }
  } catch (error) {
    SystemManagement.showNotification(`❌ שגיאה בשינוי מצב מטמון: ${error.message}`, 'error');
  }
};
 ...
 * הצגת מודל בחירת מצב
 */
SystemManagement.showModeSelector = function() {
  const modes = [
  ...
  const modalHtml = `
    <div class="modal fade" id="modeSelectorSystemModal" tabindex="-1">
  ...
  const existingModal = document.getElementById('modeSelectorSystemModal');
  if (existingModal) {
  ...
  // Show modal
  const modal = new bootstrap.Modal(document.getElementById('modeSelectorSystemModal'));
  modal.show();
};

/**
 * Update system status display
 * עדכון תצוגת סטטוס מערכת
 */
SystemManagement.updateSystemStatus = function(mode) {
  const systemCacheMode = document.getElementById('systemCacheMode');
  if (systemCacheMode) {
 ...
 * קבלת צבע למצב
 */
SystemManagement.getModeColor = function(mode) {
  const colors = {
    'development': 'primary',
    'no-cache': 'danger',
    'production': 'success',
