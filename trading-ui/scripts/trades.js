...
function getInvestmentTypeColor(investmentType) {
  ...
  const typeColors = {
    ...
    'investment': '#28a745', // ירוק - השקעות ארוכות טווח
    'passive': '#6f42c1',    // סגול - השקעות פאסיביות
  };
...
async function cancelTradeRecord(tradeId) {
  try {
    ...
    try {
      ...
      console.warn('לא ניתן לטעון פרטי טרייד:', error);
    }
    ...
    if (trade) {
      ...
      if (editForm) {
        ...
        if (trade.trade_plan_id) {
          ...
          const tradePlanSelect = document.getElementById('editTradeTradePlanId');
          if (tradePlanSelect) {
            const selectedOption = tradePlanSelect.querySelector(`option[value="${trade.trade_plan_id}"]`);
            if (selectedOption) {
        ...
        if (tradePlanSelect) {
          if (trade.trade_plan_id) {
            ...
            if (selectedOption) {
              ...
              tradePlanSelect.setAttribute('data-cleared', 'true');
...
// window.updateTickersForClosedTradesFilter = window.updateTickersForClosedTradesFilter; // פונקציה גלובלית לעדכון טיקרים - מושבתת עקב self-assignment
