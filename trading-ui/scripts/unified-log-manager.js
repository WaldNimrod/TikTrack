...
class UnifiedLogManager {
    ...
    initializeLogTypes() {
        ...
        this.logTypes.set('serverBackgroundTasksLogs', {
            name: 'לוג שרת - משימות ברקע',
            icon: 'fa-tasks',
            color: '#6f42c1',
            description: 'לוגי משימות ברקע',
            fields: ['task', 'status', 'timestamp', 'duration', 'result'],
            defaultFilters: ['task', 'status', 'timeRange'],
            sortBy: 'timestamp',
            sortOrder: 'desc'
        });
        ...
        try {
            ...
            switch (logType) {
                ...
                case 'externalDataLog':
                    ...
                    break;
                case 'serverAppLogs':
                    data = await this.getServerAppLogs();
                    break;
                case 'serverErrorLogs':
                    data = await this.getServerErrorLogs();
                    break;
                case 'serverPerformanceLogs':
                    data = await this.getServerPerformanceLogs();
                    break;
                case 'serverDatabaseLogs':
                    data = await this.getServerDatabaseLogs();
                    break;
                case 'serverCacheLogs':
                    data = await this.getServerCacheLogs();
                    break;
                case 'serverBackgroundTasksLogs':
                    data = await this.getServerBackgroundTasksLogs();
                    break;
                case 'cacheLog':
        ...
        try {
            if (window.UnifiedCacheManager && window.UnifiedCacheManager.initialized) {
                ...
                if (cachedData && Array.isArray(cachedData) && cachedData.length > 0) {
                    console.log(`📊 Loaded ${cachedData.length} real external data log entries from cache`);
                    return cachedData;
                }
            }
            
            // Initialize with empty log if no data exists
            ...
            if (window.UnifiedCacheManager && window.UnifiedCacheManager.initialized) {
                ...
                console.log('💾 Initialized external data log in cache');
            }
    ...
    parseLogLine(line, logType) {
        try {
            ...
            const loggerMatch = remainingLine.match(/^(\w+) - (\w+) -/);
            const source = loggerMatch ? loggerMatch[1] : 'unknown';
