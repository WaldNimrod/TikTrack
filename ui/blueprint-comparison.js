...
(function () {
  ...
  console.log(
    '%c🔍 Blueprint Comparison Script - Enhanced Version',
  ...
  function compareCSSValue(actual, expected, property) {
    ...
    if (
      actualPx !== null &&
      expectedPx !== null &&
      Math.abs(actualPx - expectedPx) < 0.1
    ) {
      return true;
    }
  ...
  function checkDropdownSpacing() {
    console.log(
      '\n%c📋 Checking Dropdown Menu Spacing',
    ...
    if (dropdownMenus.length > 0) {
      ...
      const topMatch =
        compareCSSValue(paddingTop, expectedPadding, 'padding-top') ||
        compareCSSValue(paddingTop, expectedPaddingPx, 'padding-top');
      const bottomMatch =
        compareCSSValue(paddingBottom, expectedPadding, 'padding-bottom') ||
    ...
    if (dropdownItems.length > 0) {
      ...
      const rightMatch = compareCSSValue(
        paddingRight,
        expectedRight,
        'padding-right',
      );
      const bottomMatch = compareCSSValue(
        paddingBottom,
        expectedBottom,
        'padding-bottom',
      );
      const leftMatch = compareCSSValue(
        paddingLeft,
        expectedLeft,
        'padding-left',
      );
    ...
    if (separators.length > 0) {
      ...
      const marginTopMatch = compareCSSValue(
        marginTop,
        expectedMargin,
        'margin-top',
      );
      const marginBottomMatch = compareCSSValue(
        marginBottom,
        expectedMargin,
        'margin-bottom',
      );
      const shadowMatch =
        boxShadow.includes('0.05') || boxShadow.includes('rgba(0, 0, 0, 0.05)');
  ...
  function checkFilterUserPosition() {
    console.log(
      '\n%c👤 Checking Filter User Section Position',
      'color: #ff9500; font-weight: bold;',
    );
  ...
  function checkInvestmentTypeFilter() {
    console.log(
      '\n%c💰 Checking Investment Type Filter Options',
      'color: #ff9500; font-weight: bold;',
    );

    const typeFilterItems = document.querySelectorAll(
      '#unified-header .type-filter-item',
    );
    const expectedOptions = ['הכול'];
    const actualOptions = Array.from(typeFilterItems)
      .map((item) => {
    ...
    if (actualOptions.length === 1 && actualOptions[0] === 'הכול') {
      ...
      console.log(
        '❌ Investment type filter options:',
        actualOptions,
        'Expected:',
        expectedOptions,
      );
  ...
  function checkActiveAlertsDesign() {
    console.log(
      '\n%c🚨 Checking Active Alerts Structure & Design',
      'color: #ff9500; font-weight: bold;',
    );
    ...
    if (display === 'grid') {
      results.summary.passed++;
      console.log('✅ Active alerts list is grid:', {
        display,
        gridTemplateColumns,
        gap,
      });
    ...
    if (alertCards.length > 0) {
      ...
      if (hasHeader) {
        ...
        if (linkedEntity && detailsBtn) {
          results.summary.passed++;
          console.log(
            '✅ Alert card header structure: linked entity + details button',
          );
        } else {
          results.summary.failed++;
          results.errors.push({
      ...
      if (hasBody) {
        ...
        const conditionRow = hasBody.querySelector(
          '.active-alerts__row--condition',
        );
        const messageRow = hasBody.querySelector(
          '.active-alerts__row--message',
        );
        ...
        if (rows.length >= 2 && conditionRow && messageRow) {
          results.summary.passed++;
          console.log(
            `✅ Alert card body: ${rows.length} rows (condition + message)`,
          );
        } else {
          results.summary.failed++;
          results.errors.push({
      ...
      if (hasFooter) {
        ...
        const markReadBtn = hasFooter.querySelector(
          '.active-alerts__mark_read',
        );
      ...
      const hasEntityClass =
        cardClasses.includes('active-alerts__card--trade') ||
  ...
  function checkInfoSummaryLayout() {
    console.log(
      '\n%c📊 Checking Info Summary Structure & Design',
      'color: #ff9500; font-weight: bold;',
    );
    ...
    const firstRow = document.querySelector(
      '.info-summary__row.info-summary__row--first, .info-summary__row:first-child',
    );
    if (!firstRow) {
    ...
    const justifyMatch =
      justifyContent === expectedJustify || justifyContent === 'start';
    ...
    const secondRow = document.querySelector(
      '.info-summary__row.info-summary__row--second, .info-summary__row:nth-child(2)',
    );
    if (secondRow) {
      ...
      const isHidden =
        secondRowDisplay === 'none' || secondRow.style.display === 'none';
      ...
      if (isHidden) {
        ...
        results.warnings.push(
          'Second row should be hidden by default but is visible',
        );
        console.log('⚠️ Second row is visible (should be hidden by default)');
      }
    } else {
      results.warnings.push(
        'Second row not found - may be dynamically created',
      );
    }
  ...
  function checkWidgetPlaceholders() {
    console.log(
      '\n%c🎨 Checking Widget Placeholders Structure & Design',
      'color: #ff9500; font-weight: bold;',
    );
    ...
    widgets.forEach((widget, index) => {
      ...
      if (hasHeader) {
        const titleRow = hasHeader.querySelector(
        ...
        const badges = hasHeader.querySelector(
          '.widget-placeholder__header-badges',
        );
        const searchForm = hasHeader.querySelector(
          '.widget-placeholder__search-form',
        );
        const refreshBtn = hasHeader.querySelector(
        ...
        const titleIcon = title
          ? title.querySelector('.widget-placeholder__title-icon, img')
          : null;
        results.summary.total++;
        if (titleIcon) {
          results.summary.passed++;
        ...
        if (tabs) {
          const tabItems = tabs.querySelectorAll(
      ...
      if (hasBody) {
        const tabContent = hasBody.querySelectorAll(
        ...
        const tagCloud = hasBody.querySelector(
          '.widget-placeholder__tag-cloud',
        );
        const chartGrid = hasBody.querySelector(
  ...
  function checkPortfolioFilters() {
    console.log(
      '\n%c💼 Checking Portfolio Section Header Filters',
      'color: #ff9500; font-weight: bold;',
    );
    ...
    const headerActions = portfolioSection.querySelector(
      '.index-section__header-actions, .dashboard-section__header-actions',
    );
    if (!headerActions) {
    ...
    const filtersContainer = headerActions.querySelector(
    ...
    if (filtersDisplay === 'flex') {
      results.summary.passed++;
      console.log('✅ Portfolio filters container uses flex layout:', {
    ...
    const accountFilter = filtersContainer.querySelector(
    ...
    const sideFilters = filtersContainer.querySelector(
      '.portfolio-side-filters',
    );
    if (sideFilters) {
      const sideFilterBtns = sideFilters.querySelectorAll(
      ...
      const hasLongBtn = Array.from(sideFilterBtns).some((btn) =>
        btn.classList.contains('portfolio-side-filter-long'),
      );
      const hasShortBtn = Array.from(sideFilterBtns).some((btn) =>
        btn.classList.contains('portfolio-side-filter-short'),
      );
      ...
      if (hasLongBtn && hasShortBtn) {
        results.summary.passed++;
        console.log(
          '✅ Side filter buttons have correct classes (long, short)',
        );
      } else {
        results.summary.failed++;
        results.errors.push({
    ...
    const checkboxesContainer = filtersContainer.querySelector(
      '.portfolio-checkboxes-container',
    );
    if (checkboxesContainer) {
      const checkboxLabels = checkboxesContainer.querySelectorAll(
      ...
      if (checkboxLabels.length >= 2) {
        ...
        results.warnings.push(
          'Portfolio checkboxes container has fewer than 2 labels',
        );
      }
    } else {
  ...
  function checkCSSLoading() {
    console.log(
      '\n%c📋 Checking CSS Loading (Basic)',
      'color: #ff9500; font-weight: bold;',
    );

    const loadedFiles = Array.from(document.styleSheets).map(
      (sheet) => sheet.href || 'inline',
    );
    const hasPhoenixBase = loadedFiles.some(
      (href) => href && href.includes('phoenix-base.css'),
    );
    const hasPhoenixComponents = loadedFiles.some(
      (href) => href && href.includes('phoenix-components.css'),
    );
    const hasPhoenixHeader = loadedFiles.some(
      (href) => href && href.includes('phoenix-header.css'),
    );
    const hasDashboardStyles = loadedFiles.some(
      (href) => href && href.includes('D15_DASHBOARD_STYLES.css'),
    );
    ...
    if (hasVariables) {
      ...
      results.errors.push({
        ...
        actual: 'Not found',
        expected:
          'CSS Variables should be available (phoenix-base.css may not be loaded)',
      });
      console.log(
        '❌ CSS Variables not available - phoenix-base.css may not be loaded',
      );
    }
  }
  ...
  function checkDOMStructure() {
    ...
    if (ttSections.length > 0) {
      ...
      results.errors.push({
        ...
        actual: '0 found',
        expected:
          'Should have at least one tt-section (Template V3 requirement)',
      });
