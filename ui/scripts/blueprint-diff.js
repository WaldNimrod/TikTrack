...
function extractElementStructure(content, isJSX = false) {
  ...
  while ((match = tagRegex.exec(content)) !== null) {
    ...
    const classes = classMatch
      ? classMatch[1].split(/\s+/).filter((c) => c.trim())
      : [];
...
function generateReport(
  ...
  differences.forEach((diff) => {
    report.summary.byType[diff.type] =
      (report.summary.byType[diff.type] || 0) + 1;
  });
...
function printReport(report) {
  ...
  if (report.differences.length > 0) {
    ...
    report.differences.forEach((diff, idx) => {
      ...
      if (diff.blueprint) {
        console.log(
          `   Blueprint: <${diff.blueprint.tag}> classes=[${diff.blueprint.classes.join(', ')}]`,
        );
      }
      if (diff.react) {
        console.log(
          `   React: <${diff.react.tag}> classes=[${diff.react.classes.join(', ')}]`,
        );
      }
...
function main() {
  ...
  if (args.length < 2) {
    ...
    console.error('\nExample:');
    console.error(
      '  node scripts/blueprint-diff.js _COMMUNICATION/team_01/team_01_staging/D15_LOGIN.html ui/src/cubes/identity/components/auth/LoginForm.jsx',
    );
    process.exit(1);
  }
