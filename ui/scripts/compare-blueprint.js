...
function extractJSXStructure(jsxContent) {
  ...
  while ((match = tagRegex.exec(jsxContent)) !== null) {
    ...
    if (!isClosing && (tag.includes('div') || tag.includes('main') || tag.includes('section') || tag.includes('header') || tag.includes('footer') || tag.includes('form') || tag.includes('tt-'))) {
      structure.push({
        tag: tag.toLowerCase(),
        full: match[0],
        index: match.index
...
function compareBlueprint(blueprintPath, reactPath) {
  ...
  const mainMatch = blueprintContent.match(/<main[^>]*>([\s\S]*?)<\/main>/i) || 
  ...
  const reportPath = path.join(path.dirname(reactPath), 'blueprint-comparison-report.json');
  const report = {
...
if (require.main === module) {
  ...
  if (args.length < 2) {
    console.error('Usage: node scripts/compare-blueprint.js <blueprint-path> <react-file-path>');
    process.exit(1);
  }
...
module.exports = { compareBlueprint, extractClasses, extractStructure, compareStructures, compareClasses };
