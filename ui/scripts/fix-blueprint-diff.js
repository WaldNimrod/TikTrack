...
function main() {
  ...
  if (args.length < 1) {
    console.error('Usage: node scripts/fix-blueprint-diff.js <diff-report.json>');
    process.exit(1);
  }
  ...
  suggestions.forEach((suggestion, idx) => {
    ...
    if (suggestion.element) {
      console.log(`   Element: <${suggestion.element.tag}> classes=[${suggestion.element.classes.join(', ')}]`);
    }
