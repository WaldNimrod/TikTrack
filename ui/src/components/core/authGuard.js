...
(function initAuthGuard() {
  ...
  const debugMode =
    ...
    localStorage.getItem('auth_guard_debug') === 'true';

  if (debugMode) {
    maskedLog('🔍 Auth Guard: DEBUG MODE ENABLED - No redirects will occur');
    maskedLog(
      '🔍 Auth Guard: Add ?debug=true to URL or set localStorage.auth_guard_debug=true',
    );
    ...
    try {
      ...
      const runCheck = async () => {
        try {
          ...
          if (debugMode) {
            logWithTimestamp(
              '🔍 DEBUG MODE: localStorage error, but continuing',
            );
          } else {
            logWithTimestamp(
              'localStorage not available, allowing access as fallback',
            );
          }
        }
      };
