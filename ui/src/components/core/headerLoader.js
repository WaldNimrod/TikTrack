...
(function loadHeader() {
  ...
  function initHeader() {
    ...
    try {
      window.__headerLoaderInit = window.__headerLoaderInit || Date.now();
    } catch (_) {}
    ...
    if (isAuthPage) {
      ...
      if (existingHeader) {
        safeLog(
          'Phoenix Header Loader: Auth page detected, removing existing header.',
          { path: currentPath },
        );
        existingHeader.remove();
      } else {
        safeLog(
          'Phoenix Header Loader: Auth page detected, skipping header load.',
          { path: currentPath },
        );
      }
      return;
    ...
    if (!document.body) {
      safeLog(
        'Phoenix Header Loader: document.body not available yet. Retrying...',
      );
    ...
    fetch(headerPath)
      ...
      .then((html) => {
        ...
        if (!header) {
          safeLog(
            'Phoenix Header Loader: header element not found in unified-header.html',
          );
          return;
        }
        ...
        try {
          ...
          if (pageWrapper && pageWrapper.parentNode) {
            try {
              ...
              safeLog(
                '[Header Loader] Header inserted before .page-wrapper (via parentNode)',
              );
              ...
              safeLog(
                '[Header Loader] parentNode.insertBefore failed for .page-wrapper, trying fallback',
                {
          ...
          if (document.body.firstChild) {
            ...
            if (firstChild && firstChild.parentNode === document.body) {
              try {
                ...
                safeLog(
                  '[Header Loader] insertBefore failed for firstChild, using appendChild',
                  {
                    error: insertError.message,
                  },
                );
          ...
          safeLog(
            '[Header Loader] All insertion strategies failed, using appendChild',
            {
          ...
          if (document.body) {
            try {
              document.body.appendChild(header);
              safeLog(
                '[Header Loader] Header appended to body (error recovery)',
              );
            } catch (appendError) {
