...
(function initNavigationHandler() {
  ...
  function initDropdownHandlers() {
    ...
    if (isDebugMode) {
      ...
      import('../../utils/maskedLog.js').then(({ maskedLog }) => {
        maskedLog('Navigation Handler: Found dropdown toggles', { count: dropdownToggles.length });
      }).catch(() => {
    ...
    if (isDebugMode) {
      // Import maskedLog dynamically to avoid module issues
      import('../../utils/maskedLog.js').then(({ maskedLog }) => {
        maskedLog('Navigation Handler: Initialized dropdown handlers and navigation links');
      }).catch(() => {
        // Fallback if import fails
      });
    }
