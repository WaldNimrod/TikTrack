...
export class DataStage extends StageBase {
  ...
  isAuthenticated() {
    try {
      ...
      maskedLog('[Data Stage] Error checking authentication:', {
  ...
  async execute() {
    try {
      ...
      if (!authenticated) {
        maskedLog(
          '[Data Stage] User not authenticated, skipping data loading',
          {
      ...
      try {
        ...
        maskedLog(
          '[Data Stage] Shared Services initialization failed, continuing with fallback:',
      ...
      if (config.dataLoader) {
        ...
        maskedLog(
          '[Data Stage] No data loader or apiEndpoint configured, skipping data loading',
        );
      }
      ...
      if (error.status === 401 || error.code === 'HTTP_401') {
        maskedLog(
          '[Data Stage] 401 Unauthorized - user not authenticated, skipping data loading',
          {
            pageType: config?.pageType,
            requiresAuth: config?.requiresAuth,
          },
  ...
  async fetchDataViaSharedServices(config) {
    try {
      ...
      maskedLog('[Data Stage] Data loaded via Shared Services:', {
        dataKeys: Object.keys(this.data),
      });
    } catch (error) {
      throw new Error(
        `Failed to fetch data via Shared Services: ${error.message}`,
      );
    }
  }
  ...
  async loadDataLoader(loaderPath) {
    try {
      ...
      if (
        ...
        !loaderPath.startsWith('http://') &&
        !loaderPath.startsWith('https://')
      ) {
      ...
      maskedLog('[Data Stage] Data loader loaded (ES Module):', {
        loaderPath,
        importPath,
      });
    } catch (error) {
      throw new Error(
        `Failed to load data loader: ${loaderPath} - ${error.message}`,
      );
    }
  }
  ...
  async fetchData(config) {
    try {
      ...
      const loaderFunction = this.findLoaderFunction(
        pageType,
        config.dataLoader,
      );
      ...
      if (!loaderFunction) {
        ...
        if (config.apiEndpoint) {
          ...
          throw new Error(
            `Data loader function not found for page type: ${pageType}`,
          );
        }
      } else {
        ...
        if (typeof loaderFunction === 'function') {
          ...
          throw new Error(
            `Invalid data loader format for page type: ${pageType}`,
          );
        }
      }

      maskedLog('[Data Stage] Data loaded successfully:', {
        dataKeys: Object.keys(this.data),
      });
    } catch (error) {
