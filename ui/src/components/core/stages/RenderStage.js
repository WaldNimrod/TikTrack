...
export class RenderStage extends StageBase {
  ...
  async initializeTable(pageType, config) {
    try {
      ...
      if (initFunction) {
        ...
        maskedLog('[Render Stage] Table init function not found for:', {
          pageType,
        });
      }
    } catch (error) {
      maskedLog('[Render Stage] Failed to initialize table:', {
  ...
  getTableInitPath(pageType) {
    const pathMap = {
      ...
      tradingAccounts:
        '/src/views/financial/tradingAccounts/tradingAccountsTableInit.js',
