    ...
    if (toolbar) {
      function updateActiveState() {
        toolbar.querySelectorAll('[data-rt-cmd]').forEach((btn) => {
          ...
          let active = false;
          if (cmd === 'phx-success')
            active = editor.isActive('phoenixRtSuccess');
          else if (cmd === 'phx-warning')
            active = editor.isActive('phoenixRtWarning');
          else if (cmd === 'phx-danger')
            active = editor.isActive('phoenixRtDanger');
          else if (cmd === 'phx-highlight')
            active = editor.isActive('phoenixRtHighlight');
      ...
      toolbar.querySelectorAll('[data-rt-cmd]').forEach((btn) => {
        btn.addEventListener('click', () => {
          const cmd = btn.getAttribute('data-rt-cmd');
          if (cmd === 'phx-success')
            editor.chain().focus().toggleMark('phoenixRtSuccess').run();
          else if (cmd === 'phx-warning')
            editor.chain().focus().toggleMark('phoenixRtWarning').run();
          else if (cmd === 'phx-danger')
            editor.chain().focus().toggleMark('phoenixRtDanger').run();
          else if (cmd === 'phx-highlight')
            editor.chain().focus().toggleMark('phoenixRtHighlight').run();
