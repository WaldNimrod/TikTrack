...
export const useAuthValidation = ({ schema, formData, options = {} }) => {
  ...
  const validateField = useCallback((fieldName, value, additionalData = {}) => {
    if (!schema || !schema[fieldName]) {
      if (DEBUG_MODE) {
        audit.log('AuthValidation', 'Field validator not found', { fieldName, availableValidators: Object.keys(schema || {}) });
      }
   ...
   */
  const validateForm = useCallback((customFormData = null) => {
    const dataToValidate = customFormData || formData;
    const errors = {};
    let isValid = true;

    if (!schema) {
      audit.error('AuthValidation', 'Schema not provided for form validation');
    ...
    Object.keys(schema).forEach(fieldName => {
      ...
      if (typeof validator === 'function') {
        ...
        } else if (fieldName === 'confirmNewPassword' && dataToValidate.newPassword) {
  ...
  const validationState = useMemo(() => ({
    hasErrors: Object.keys(fieldErrors).length > 0 || Object.keys(formErrors).length > 0,
