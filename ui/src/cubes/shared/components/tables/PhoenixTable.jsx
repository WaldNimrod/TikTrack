...
const PhoenixTable = ({
  ...
  const getSortIcon = (columnKey) => {
    if (sortState.primary.key === columnKey) {
      return sortState.primary.direction === 'ASC' ? 'asc' : 
             sortState.primary.direction === 'DESC' ? 'desc' : 'none';
    }
    if (sortState.secondary.key === columnKey) {
      return sortState.secondary.direction === 'ASC' ? 'asc' : 
             sortState.secondary.direction === 'DESC' ? 'desc' : 'none';
    }
    return 'none';
  };
  ...
  const sortedAndFilteredData = useMemo(() => {
    ...
    if (sortState.primary.key) {
      const primaryColumn = columns.find(col => col.key === sortState.primary.key);
      if (primaryColumn) {
        result.sort((a, b) => {
          ...
          // סידור לפי סוג השדה
          if (primaryColumn.type === 'numeric' || primaryColumn.type === 'currency') {
    ...
    if (sortState.secondary.key) {
      const secondaryColumn = columns.find(col => col.key === sortState.secondary.key);
      if (secondaryColumn) {
        result.sort((a, b) => {
          const aValue = a[sortState.secondary.key];
          const bValue = b[sortState.secondary.key];
          
          if (secondaryColumn.type === 'numeric' || secondaryColumn.type === 'currency') {
                ...
                <thead className="phoenix-table__head" role="rowgroup">
                  <tr className="phoenix-table__row" role="row">
                    {columns.map((column) => {
                      ...
                      const ariaSort = sortIcon === 'asc' ? 'ascending' : 
                                       sortIcon === 'desc' ? 'descending' : 'none';
                      ...
                      return (
                        <th
                          ...
                          onClick={column.sortable ? (e) => handleSortClick(column.key, e) : undefined}
                        ...
                        >
                          <span className="phoenix-table__header-text">{column.label}</span>
                          {column.sortable && (
                            <span className="phoenix-table__sort-indicator js-sort-indicator">
                              ...
                              >
                                ...
                                {sortIcon === 'none' && (
                                  <path d="M6 3L3 6H9L6 3Z M6 9L9 6H3L6 9Z" fill="currentColor" opacity="0.3" />
                                )}
                              </svg>
                            </span>
                          )}
                ...
                <tbody className="phoenix-table__body" role="rowgroup">
                  {sortedAndFilteredData.length === 0 ? (
                    ...
                    sortedAndFilteredData.map((row, rowIndex) => (
                      <tr key={rowIndex} className="phoenix-table__row" role="row">
                        {columns.map((column) => (
                          <td
                            key={column.key}
