...
export const validatePasswordChange = (newPassword, currentPassword) => {
  ...
  if (currentPassword && newPassword === currentPassword) {
    return { isValid: false, error: 'הסיסמה החדשה חייבת להיות שונה מהסיסמה הנוכחית' };
  }
...
export const validateIdentifier = (value) => {
  ...
  if (emailRegex.test(cleanValue)) {
    ...
    if (cleanValue.includes('@')) {
      return { isValid: false, error: 'כתובת אימייל לא תקינה', method: 'EMAIL' };
    } else {
      return { isValid: false, error: 'מספר טלפון לא תקין (פורמט E.164: +972501234567)', method: 'SMS' };
    }
  }
};
...
export const validateLoginForm = (formData) => {
  ...
  const usernameOrEmailResult = validateUsernameOrEmail(formData.usernameOrEmail);
...
export const validatePasswordChangeForm = (formData) => {
  ...
  const passwordChangeResult = validatePasswordChange(formData.newPassword, formData.currentPassword);
  if (!passwordChangeResult.isValid) {
    errors.newPassword = passwordChangeResult.error;
  }
  
  // Confirm password validation
  const confirmPasswordResult = validateConfirmPassword(formData.confirmPassword, formData.newPassword);
  if (!confirmPasswordResult.isValid) {
    errors.confirmPassword = confirmPasswordResult.error;
  }
...
export const validatePasswordResetRequestForm = (formData) => {
  ...
  return {
    ...
    errors: identifierResult.isValid ? {} : { identifier: identifierResult.error },
...
export const validatePasswordResetVerifyForm = (formData, method) => {
  ...
  // New password validation
  const passwordResult = validatePassword(formData.newPassword, { minLength: 8 });
  if (!passwordResult.isValid) {
