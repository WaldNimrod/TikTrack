...
async function fetchNotesSummary() {
  try {
    ...
    return {
      totalNotes: (response.total_notes != null ? response.total_notes : response.totalNotes) || 0,
      recentNotes: (response.recent_notes != null ? response.recent_notes : response.recentNotes) || 0,
      totalAttachments: (response.total_attachments != null ? response.total_attachments : response.totalAttachments) || 0,
      pinnedNotes: (response.pinned_notes != null ? response.pinned_notes : response.pinnedNotes) || 0,
      notesWithTags: (response.notes_with_tags != null ? response.notes_with_tags : response.notesWithTags) || 0,
      notesByParentType: (response.notes_by_parent_type != null ? response.notes_by_parent_type : response.notesByParentType) || {}
    };
  } catch (error) {
    maskedLog('[Notes Data Loader] Error fetching summary:', { errorCode: (error && error.code), status: (error && error.status) });
    return {
      totalNotes: 0,
      recentNotes: 0,
      totalAttachments: 0,
...
async function loadNotesData(filters = {}) {
  try {
    ...
    maskedLog('[Notes Data Loader] Error loading:', { errorCode: (error && error.code) });
    return {
      summary: { totalNotes: 0, recentNotes: 0, totalAttachments: 0, pinnedNotes: 0, notesWithTags: 0, notesByParentType: {} },
      notes: { data: [], total: 0 }
    };
  }
}
