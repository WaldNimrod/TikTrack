...
(function initCashFlowsTables() {
  ...
  function initTables() {
    ...
    if (
      ...
      const filterManager = new window.PhoenixTableFilterManager(
        cashFlowsTable,
      );
    ...
    const currencyConversionsTable = document.querySelector(
    ...
    if (
      ...
      const sortManager = new window.PhoenixTableSortManager(
        currencyConversionsTable,
      );
      const filterManager = new window.PhoenixTableFilterManager(
        currencyConversionsTable,
      ...
      // Listen to sort events
      currencyConversionsTable.addEventListener(
        'phoenix-table-sorted',
        function (e) {
          const { sortKey, sortDirection } = e.detail;
          if (sortKey && sortDirection) {
  ...
  function initPaginationHandlers() {
    ...
    const cashFlowsPageSizeSelect = document.querySelector(
      '.js-table-page-size[data-table-id="cashFlowsTable"]',
    );
    const cashFlowsPrevPageBtn = document.getElementById(
      'cashFlowsPrevPageBtn',
    );
    const cashFlowsNextPageBtn = document.getElementById(
      'cashFlowsNextPageBtn',
    );
    ...
    const currencyConversionsPageSizeSelect = document.querySelector(
      '.js-table-page-size[data-table-id="currencyConversionsTable"]',
    );
    const currencyConversionsPrevPageBtn = document.getElementById(
      'currencyConversionsPrevPageBtn',
    );
    const currencyConversionsNextPageBtn = document.getElementById(
      'currencyConversionsNextPageBtn',
    );

    if (currencyConversionsPageSizeSelect) {
      currencyConversionsPageSizeSelect.addEventListener(
        'change',
        function (e) {
    ...
    if (currencyConversionsNextPageBtn) {
      currencyConversionsNextPageBtn.addEventListener('click', function () {
        const totalPages = Math.ceil(
          currencyConversionsData.total / currencyConversionsPageSize,
        );
  ...
  async function loadCashFlowsTableData() {
    try {
      const flowType = document.getElementById('cashFlowsType')?.value || '';
      const dateRange = currentFilters?.dateRange;
      const tradingAccountId = normalizeTradingAccountId(
        currentFilters?.tradingAccount,
      );
      const search = currentFilters?.search || undefined;
      const dateFromFromRange = dateRange?.from || undefined;
      const dateToFromRange = dateRange?.to || undefined;
  ...
  function updateSummary(summary) {
    ...
    const tableCashFlowsCountEl = document.getElementById(
      'tableCashFlowsCount',
    );
    const currencyConversionsCountEl = document.getElementById(
    ...
    if (totalBalanceEl) {
      ...
      if (span) {
        ...
        span.className =
          balance >= 0 ? 'numeric-value-positive' : 'numeric-value-negative';
    ...
    if (monthlyDepositsEl) {
      ...
      if (span)
        span.textContent = formatCurrency(summary.monthlyDeposits || 0, 'USD');
    }
    ...
    if (monthlyConversionsEl)
      monthlyConversionsEl.textContent = summary.monthlyConversions || 0;
    if (tableCashFlowsCountEl)
      tableCashFlowsCountEl.textContent = `${cashFlowsData.total || 0} תנועות`;
    if (currencyConversionsCountEl)
      currencyConversionsCountEl.textContent = `${currencyConversionsData.total || 0} המרות`;
  }
  ...
  function updateCashFlowsTable(data) {
    ...
    if (!data || data.length === 0) {
      ...
      row.innerHTML =
        '<td colspan="9" class="phoenix-table__cell phoenix-table__cell--empty">אין נתונים להצגה</td>';
    ...
    data.forEach((flow) => {
      ...
      const tradeDisplay =
        flow.trade || flow.tradeId ? String(flow.trade || flow.tradeId) : '—'; // TODO: When trades linked - show ticker + ↗ (long) or ↘ (short)
      ...
      accountCell.textContent =
        flow.accountName || flow.account_name || flow.account || '—';
      ...
      amountCell.className =
        'phoenix-table__cell col-amount phoenix-table__cell--numeric phoenix-table__cell--currency';
      ...
      amountSpan.className =
        amount >= 0 ? 'numeric-value-positive' : 'numeric-value-negative';
      ...
      dateCell.className =
        'phoenix-table__cell col-date phoenix-table__cell--date';
      dateCell.setAttribute('data-field', 'date');
      dateCell.textContent = formatDate(
        flow.transactionDate || flow.transaction_date || flow.date || '',
      );
      ...
      actionsCell.innerHTML = `
        <div class="table-actions-tooltip">
          ...
          <div class="table-actions-menu">
            <button class="table-action-btn js-action-view" aria-label="צפה" title="צפה" data-flow-id="${flowId}">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                <path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path>
                <circle cx="12" cy="12" r="3"></circle>
              </svg>
            </button>
            <button class="table-action-btn js-action-edit" aria-label="ערוך" title="ערוך" data-flow-id="${flowId}">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path>
                <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path>
              </svg>
            </button>
            <button class="table-action-btn js-action-delete" aria-label="מחק" title="מחק" data-flow-id="${flowId}">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
  ...
  function updateCurrencyConversionsTable(data) {
    ...
    if (!data || data.length === 0) {
      ...
      row.className = 'phoenix-table__row';
      row.innerHTML =
        '<td colspan="7" class="phoenix-table__cell phoenix-table__cell--empty">אין נתונים להצגה</td>';
      tbody.appendChild(row);
      return;
    }
    ...
    data.forEach((conversion) => {
      ...
      fromCell.className =
        'phoenix-table__cell col-from phoenix-table__cell--numeric phoenix-table__cell--currency';
      ...
      fromSpan.textContent = formatCurrency(
        Math.abs(conversion.fromAmount || 0),
        conversion.fromCurrency || 'USD',
      );
      fromCell.appendChild(fromSpan);
      const fromCurrencySpan = document.createElement('span');
      fromCurrencySpan.style.cssText =
        'font-size: 0.85em; color: var(--apple-text-secondary);';
      ...
      toCell.className =
        'phoenix-table__cell col-to phoenix-table__cell--numeric phoenix-table__cell--currency';
      ...
      rateCell.className =
        'phoenix-table__cell col-estimated-rate phoenix-table__cell--numeric';
      ...
      actionsCell.innerHTML = `
        <div class="table-actions-tooltip">
          ...
          <div class="table-actions-menu">
            <button class="table-action-btn js-action-view" aria-label="צפה" title="צפה" data-conversion-id="${conversion.id || ''}">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                <path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path>
                <circle cx="12" cy="12" r="3"></circle>
              </svg>
            </button>
            <button class="table-action-btn js-action-edit" aria-label="ערוך" title="ערוך" data-conversion-id="${conversion.id || ''}">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path>
                <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path>
              </svg>
            </button>
            <button class="table-action-btn js-action-delete" aria-label="מחק" title="מחק" data-conversion-id="${conversion.id || ''}">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                <polyline points="3 6 5 6 21 6"></polyline>
                <path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path>
              </svg>
  ...
  function initCashFlowsActionHandlers() {
    const viewButtons = document.querySelectorAll(
      '#cashFlowsTableBody .js-action-view',
    );
    const editButtons = document.querySelectorAll(
      '#cashFlowsTableBody .js-action-edit',
    );
    const deleteButtons = document.querySelectorAll(
      '#cashFlowsTableBody .js-action-delete',
    );
  ...
  async function handleDeleteCashFlow(flowId) {
    ...
    try {
      ...
      const flow = cashFlowsData.data?.find(
        (f) => f.id === flowId || f.externalUlid === flowId,
      );
      ...
      // Reload table data
      await loadAllData();
      maskedLog('[Cash Flows] Cash flow deleted successfully', {
        flowId: idToUse,
      });
    } catch (error) {
  ...
  async function handleSaveCashFlow(flowId, flowData) {
    try {
      ...
      const apiData = {
        tradingAccountId:
          flowData.tradingAccountId || flowData.trading_account_id,
        ...
        externalReference:
          flowData.externalReference || flowData.external_reference || '',
      };
      ...
      if (idToUse) {
        ...
        maskedLog('[Cash Flows] Cash flow updated successfully', {
          flowId: idToUse,
        });
      } else {
        // Create new
        ...
        if (error.details && error.details.field) {
          ...
          const fieldName =
            error.details.field === 'trading_account_id'
              ? 'חשבון מסחר'
              : error.details.field === 'transaction_date'
                ? 'תאריך פעולה'
                : error.details.field === 'flow_type'
                  ? 'סוג תזרים'
  ...
  function initCurrencyConversionsActionHandlers() {
    const viewButtons = document.querySelectorAll(
      '#currencyConversionsTableBody .js-action-view',
    );
    const editButtons = document.querySelectorAll(
      '#currencyConversionsTableBody .js-action-edit',
    );
    const deleteButtons = document.querySelectorAll(
      '#currencyConversionsTableBody .js-action-delete',
    );
  ...
  async function handleViewCurrencyConversion(conversionId) {
    try {
      await sharedServices.init();
      const conversion = currencyConversionsData.data?.find(
        (c) => c.id === conversionId || c.externalUlid === conversionId,
      );
      if (conversion) {
        showCurrencyConversionModal(conversion, 'view');
      } else {
        const response = await sharedServices.get(
          `/cash_flows/currency_conversions/${conversionId}`,
        );
  ...
  async function handleDeleteCurrencyConversion(conversionId) {
    ...
    try {
      ...
      await sharedServices.delete(
        `/cash_flows/currency_conversions/${idToUse}`,
      );
      await loadAllData();
      maskedLog('[Cash Flows] Currency conversion deleted successfully', {
        conversionId: idToUse,
      });
    } catch (error) {
  ...
  function updateCashFlowsPagination() {
    ...
    const end = Math.min(
      cashFlowsPage * cashFlowsPageSize,
      cashFlowsData.total,
    );
  ...
  function updateCurrencyConversionsPagination() {
    const paginationInfo = document.getElementById(
      'currencyConversionsPaginationInfo',
    );
    const pageNumbers = document.getElementById(
      'currencyConversionsPageNumbers',
    );
    ...
    const start =
      (currencyConversionsPage - 1) * currencyConversionsPageSize + 1;
    const end = Math.min(
      currencyConversionsPage * currencyConversionsPageSize,
      currencyConversionsData.total,
    );
