...
(function initTradingAccountsTables() {
  ...
  function initActionHandlers() {
    ...
    if (accountsTable) {
      accountsTable.addEventListener('click', function(e) {
        const target = e.target.closest('.js-action-view, .js-action-edit, .js-action-delete');
        if (!target) return;
  ...
  async function handleViewTradingAccount(accountId) {
    try {
      await sharedServices.init();
      const response = await sharedServices.get(`/trading_accounts/${accountId}`);
  ...
  async function handleDeleteTradingAccount(accountId) {
    ...
    try {
      ...
      maskedLog('[Trading Accounts] Account deleted successfully', { accountId });
  ...
  function showTradingAccountModal(data, mode) {
    if (mode === 'view') {
      ...
      const balance = data?.balance || data?.cashBalance || data?.cash_balance || 0;
  ...
  async function handleSaveTradingAccount(accountId, accountData) {
    try {
      ...
      apiData.initialBalance = typeof accountData.initialBalance === 'number' 
        ? accountData.initialBalance 
        : parseFloat(accountData.initialBalance) || 0;
      apiData.currency = accountData.currency || 'USD';
      apiData.isActive = accountData.isActive !== undefined ? accountData.isActive : true;
      ...
      if (accountData.externalAccountId && accountData.externalAccountId.trim()) {
      ...
      if (accountId) {
        ...
        maskedLog('[Trading Accounts] Account updated successfully', { accountId });
      } else {
        // Create new
