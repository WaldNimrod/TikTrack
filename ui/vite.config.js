...
if (fs.existsSync(routesPath)) {
  try {
    ...
    console.log(
      '[Vite Config] ✅ Loaded routes from routes.json:',
      Object.keys(routeToHtmlMap).length,
      'routes',
    );
    console.log('[Vite Config] Route mappings:', routeToHtmlMap);
  } catch (error) {
    console.warn(
      '[Vite Config] ⚠️ Failed to parse routes.json, using fallback:',
      error.message,
    );
  ...
  console.warn(
    '[Vite Config] ⚠️ routes.json not found at',
    routesPath,
    '- using fallback',
  );
  // Fallback to default routes
    ...
    configureServer(server) {
      ...
      const htmlMiddleware = (req, res, next) => {
        ...
        if (routeToHtmlMap[url]) {
          ...
          if (
            htmlPath.startsWith('/') &&
            htmlPath.includes('.html') &&
            !htmlPath.includes('/views/')
          ) {
            // Map common routes to their actual file locations
            const routeToFileMap = {
              '/trading_accounts.html':
                '/views/financial/tradingAccounts/trading_accounts.html',
              '/brokers_fees.html':
                '/views/financial/brokersFees/brokers_fees.html',
              '/cash_flows.html': '/views/financial/cashFlows/cash_flows.html',
              '/tickers.html': '/views/management/tickers/tickers.html',
              '/user_tickers.html':
                '/views/management/userTicker/user_tickers.html',
              '/data_dashboard.html':
                '/views/data/dataDashboard/data_dashboard.html',
              '/system_management.html':
                '/views/management/systemManagement/system_management.html',
              '/trade_plans.html':
                '/views/planning/tradePlans/trade_plans.html',
              '/ai_analysis.html':
                '/views/planning/aiAnalysis/ai_analysis.html',
              '/watch_lists.html':
                '/views/tracking/watchLists/watch_lists.html',
              '/ticker_dashboard.html':
                '/views/tracking/tickerDashboard/ticker_dashboard.html',
              '/trading_journal.html':
                '/views/tracking/tradingJournal/trading_journal.html',
              '/trades.html': '/views/tracking/trades/trades.html',
              '/strategy-analysis.html':
                '/views/research/strategyAnalysis/strategy_analysis.html',
              '/trades_history.html':
                '/views/research/tradesHistory/trades_history.html',
              '/portfolio-state.html':
                '/views/research/portfolioState/portfolio_state.html',
              ...
              '/data_import.html':
                '/views/settings/dataImport/data_import.html',
              '/tag_management.html':
                '/views/settings/tagManagement/tag_management.html',
              '/preferences.html':
                '/views/settings/preferences/preferences.html',
            };
            actualFilePath = routeToFileMap[htmlPath] || htmlPath;
          }

          // Remove leading slash from actualFilePath for path.join
          const htmlPathNormalized = actualFilePath.startsWith('/')
            ? actualFilePath.substring(1)
            : actualFilePath;
          const filePath = path.join(__dirname, 'src', htmlPathNormalized);

          console.log(
            `[HTML Plugin] ✅ Mapped route: ${url} -> ${htmlPath} -> ${actualFilePath}`,
          );
          ...
          if (fs.existsSync(filePath) && fs.statSync(filePath).isFile()) {
            ...
            res.end(content);
            console.log(
              `[HTML Plugin] ✅✅✅ SERVED: ${url} -> ${actualFilePath}`,
            );
            ...
            for (const altPath of altPaths) {
              ...
              if (fs.existsSync(altPath) && fs.statSync(altPath).isFile()) {
                ...
                res.end(content);
                console.log(
                  `[HTML Plugin] ✅✅✅ SERVED from alternative path: ${altPath}`,
                );
                return;
              }
            }
        ...
        if (
          ...
          const resolvedHtmlFileName =
            urlToFileName[htmlFileName] || htmlFileName;
          const possiblePaths = [
            path.join(
              __dirname,
              'src',
          ...
          for (const filePath of possiblePaths) {
            if (fs.existsSync(filePath) && fs.statSync(filePath).isFile()) {
              ...
              res.end(content);
              console.log(
                `[HTML Plugin] ✅✅✅ SERVED DIRECT: ${url} -> ${filePath}`,
              );
              return; // Don't call next() - request handled
            }
          }
